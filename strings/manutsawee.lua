return {
    ACTIONFAIL = {
        ACTIVATE = {
            CARNIVAL_HOST_HERE = "I know I saw that fancy bird guy around here.",
            EMPTY_CATCOONDEN = "Drat, I thought for sure there'd be something good inside!",
            HOSTBUSY = "Excuse me. I will wait for your response.",
            KITCOON_HIDEANDSEEK_NOT_ENOUGH_HIDERS = "It would be too easy, perhaps if there were more of these little guys...",
            KITCOON_HIDEANDSEEK_NOT_ENOUGH_HIDING_SPOTS = "There aren't a lot of places around for them to hide.",
            KITCOON_HIDEANDSEEK_ONE_GAME_PER_DAY = "I think that's enough for one day.",
            LOCKED_GATE = "Aww. I want in there.",
            MANNEQUIN_EQUIPSWAPFAILED = "Nah, it looks better on me.",
            NOCARNIVAL = "Aww, looks like they all left.",
            NOTMYBERNIE = "Looks like it won't listen to me, I'm not its owner",
            PILLOWFIGHT_NO_HANDPILLOW = "Hey, don't I need a pillow first?",
        },
        APPRAISE = {
            NOTNOW = "Hey! Quit doing whatever you're doing and look at this!",
        },
        ATTUNE = {
            NOHEALTH = "Ughhhh, nooo... I don't feel good.",
        },
        BATHBOMB = {
            ALREADY_BOMBED = "Aww, someone did that one already!",
            GLASSED = "Pfft, that wouldn't work with glass in the way.",
        },
        BUILD = {
            HASPET = "I've already got one!",
            MOUNTED = "I can't place anything from atop this big lout!",
            TICOON = "Wait... no no!",
        },
        CARNIVALGAME_FEED = {
            TOO_LATE = "I wasn't too slow, it was too fast.",
        },
        CHANGEIN = {
            BURNING = "Changing near open flame is unsafe. Let's move first.",
            GENERIC = "Nah, too much effort.",
            INUSE = "Stop hogging it, I want to dress up too!",
            NOOCCUPANT = "Just a wild guess, but I might need to hitch up a beefalo first.",
            NOTENOUGHHAIR = "It might help if my beefalo had more hair.",
        },
        COMPARE_WEIGHABLE = {
            FISH_TOO_SMALL = "This thing's too small to even bother weighing.",
            OVERSIZEDVEGGIES_TOO_SMALL = "Nope. Not even going to bother.",
        },
        CONSTRUCT = {
            EMPTY = "I need something to build with first!",
            INUSE = "Ugh. Someone's already using it.",
            MISMATCH = "It's the wrong plans. Jeez!",
            NOTALLOWED = "It won't go in there.",
            NOTREADY = "The ground shaking's got me a bit distracted right now!",
        },
        COOK = {
            GENERIC = "I'm not too big on cooking.",
            INUSE = "Hey! What're you making? Can I have it?",
            TOOFAR = "It's all the way over thereeeee!",
        },
        DISMANTLE = {
            COOKING = "I'm not going to interrupt it while it's cooking.",
            INUSE = "But I want to use it!",
            NOTEMPTY = "Hey, there's still stuff in here!",
        },
        DRAW = {
            NOIMAGE = "But what should I even draw??",
        },
        FILL_OCEAN = {
            UNSUITABLE_FOR_PLANTS = "Aw, come on! A little seawater would toughen them up!",
        },
        FISH_OCEAN = {
            TOODEEP = "Stupid fish. Get on my hook!!",
        },
        GIVE = {
            ABIGAILHEART = "Ugh, just come back already! Geez!",
            BUSY = "It's busyyyy.",
            CANTSHADOWREVIVE = "It's not working.",
            CARNIVALGAME_ALREADY_PLAYING = "Come on, hurry up!",
            CARNIVALGAME_INVALID_ITEM = "I guess that won't work.",
            DEAD = "They won't fully appreciate my gift.",
            DUPLICATE = "Awww, we already know that one!",
            FOODFULL = "I have to wait for it to eat this one first.",
            GENERIC = "Noooope!",
            GHOSTHEART = "I'm not wasting this on them!",
            MUSHROOMFARM_NEEDSLOG = "Ughh, it doesn't need this! It needs a living log!",
            MUSHROOMFARM_NEEDSSHROOM = "Ughh, it doesn't need this! It needs a mushroom!",
            MUSHROOMFARM_NOMOONALLOWED = "These mushrooms will not grow under this moon condition.",
            NOMOON = "It won't work in here!",
            NOTAMONKEY = "Oh, don't stare at me!",
            NOTATRIUMKEY = "",
            NOTDISH = "I wouldn't serve that to a dog!",
            NOTGEM = "I can cram it in there but I do not know if I could get it back out.",
            NOTSCULPTABLE = "I'd like to see someone try to sculpt with that!",
            NOTSTAFF = "I don't have to jam stuff into every hole I see!",
            PIGKINGGAME_DANGER = "Not now! There's big one around.",
            PIGKINGGAME_MESSY = "Aww... You mean I have to clean up first?",
            PIGKINGGAME_TOOLATE = "Naw. It's too late.",
            QUEENBUSY = "Hey! Whatever else you're doing can wait.",
            SLEEPING = "Not right now. It's sleeping.",
            SLOTFULL = "Naw, can't make it fit.",
            SPIDERNOHAT = "Why does it need a hat when it's snug as a bug in my pocket?",
            TERRARIUM_COOLDOWN = "It's not taking it, is it broken? Where'd that tree go?",
            TERRARIUM_REFUSE = "What, that wasn't good enough for it?",
            WRONGGEM = "It's the wrong rock!",
            WRONGSHADOWFORM = "Nah. Skeleton's not right.",
        },
        GIVEALLTOPLAYER = {
            BUSY = "Hurry uppp! I have a sweet present for you!",
            DEAD = "They won't fully appreciate my gift.",
            FULL = "They can't carry any more things.",
            SLEEPING = "I could leave it under their pillow...?",
        },
        GIVETOPLAYER = {
            BUSY = "Hurry uppp! I have a sweet present for you!",
            DEAD = "They won't fully appreciate my gift.",
            FULL = "They can't carry any more things.",
            SLEEPING = "I could leave it under their pillow...?",
        },
        GIVE_TACKLESKETCH = {
            DUPLICATE = "Awww, we already know that one!",
        },
        HARVEST = {
            DOER_ISNT_MODULE_OWNER = "Ha! Scan THAT!",
        },
        HERD_FOLLOWERS = {
            WEBBERONLY = "These jerks won't listen to me.",
        },
        HITCHUP = {
            BEEF_HITCHED = "Already done.",
            INMOOD = "Ugh, there's just no reasoning with them when they get like this.",
            NEEDBEEF = "Might help if I had a beefalo...",
            NEEDBEEF_CLOSER = "I need to get that beefalo over here.",
        },
        LOWER_SAIL_FAIL = {
            "Lower, sail. Nice and steady.",
            "Oh man, I just love doing this over and over.",
            "Uuuuuuuuuugh.",
        },
        MARK = {
            ALREADY_MARKED = "No take backs!",
            NOT_PARTICIPANT = "I've got no steak in this contest.",
        },
        MOUNT = {
            INUSE = "They beat me to the hairy beast. Maybe it's for the best.",
            SLEEPING = "Oh good you're awake, now let's get going!",
            TARGETINCOMBAT = "I'll ride it when it settles down.",
        },
        OCEAN_FISHING_POND = {
            WRONGGEAR = "Seems like overkill.",
        },
        OPEN_CRAFTING = {
            PROFESSIONALCHEF = "I'm not THAT good at cooking.",
            SHADOWMAGIC = "Magic?",
        },
        PICKUP = {
            FULL_OF_CURSES = "That thing is demonic!",
            INUSE = "But I want to use it!",
            RESTRICTION = "I'm not using that!",
            _YOTC = {},
        },
        PLANTREGISTRY_RESEARCH_FAIL = {
            FERTILIZER = "Well, that's about all I ever want to know about this stuff.",
            GENERIC = "Pfft, I already know what I need to know.",
        },
        POUR_WATER = {
            OUT_OF_WATER = "Oh nooo I'm out of water, how terrible.",
        },
        POUR_WATER_GROUNDTILE = {
            OUT_OF_WATER = "Welp, I'm out of water.",
        },
        REPAIR = {
            WRONGPIECE = "We carried it all this way and it's not even the right one!",
        },
        REPLATE = {
            MISMATCH = "Ugh! I can't use that dish for that food!",
            SAMEDISH = "It's already on a dish!",
        },
        ROW_FAIL = {
            BAD_TIMING0 = "Don't think that was right.",
            BAD_TIMING1 = "Ugh, lost the rhythm.",
            BAD_TIMING2 = "Water's definitely not my thing.",
        },
        RUMMAGE = {
            GENERIC = "Make someone else do it!",
            INUSE = "There must be something good in there if you want it so bad!",
            NOTMASTERCHEF = "I'm not THAT good at cooking.",
        },
        SADDLE = {
            TARGETINCOMBAT = "I'll ride it when it settles down.",
        },
        SAIL = {
            REPAIR = "Why? It looks fine to me.",
        },
        SHAVE = {
            AWAKEBEEFALO = "Maybe I should wait for it to be distracted...",
            GENERIC = "Nu-uh.",
            NOBITS = "It's already shaved!",
            SOMEONEELSESBEEFALO = "Pfft, let someone else do it.",
        },
        SLAUGHTER = {
            TOOFAR = "Hey! Get back here!",
        },
        START_CARRAT_RACE = {
            NO_RACERS = "A race isn't any fun without someone to beat.",
        },
        STORE = {
            GENERIC = "It's full already.",
            INUSE = "I'll use it after they're done.",
            NOTALLOWED = "They won't let me.",
            NOTMASTERCHEF = "I'm not THAT good at cooking.",
        },
        TEACH = {
            CANTLEARN = "Ugh, whatever, I didn't want to know anyway.",
            KNOWN = "I knowwwww that already, geez!",
            MESSAGEBOTTLEMANAGER_NOT_FOUND = "No point trying to read this here.",
            STASH_MAP_NOT_FOUND = "Huh. These monkeys are painted in a mess",
            WRONGWORLD = "What the... this map isn't right at all!",
        },
        UNLOCK = {
            WRONGKEY = "",
        },
        USEITEMON = {
            BEEF_BELL_ALREADY_USED = "Someone already got to it first.",
            BEEF_BELL_HAS_BEEF_ALREADY = "One beefalo is enough to deal with.",
            BEEF_BELL_INVALID_TARGET = "Yeah... that's not going to work.",
        },
        USEKLAUSSACKKEY = {
            KLAUS = "But that big ugly thing is on my tail!",
            QUAGMIRE_WRONGKEY = "You mean there's another key?!",
            WRONGKEY = "Darn thing won't work!",
        },
        WRAPBUNDLE = {
            EMPTY = "I'm not just going to wrap air!",
        },
        WRITE = {
            GENERIC = "I can't write on it.",
            INUSE = "I want to write when you're done!",
        },
        YOTB_STARTCONTEST = {
            ALREADYACTIVE = "Guess he's somewhere else.",
            DOESNTWORK = "Hey weird guy! Helloooo?",
        },
        YOTB_UNLOCKSKIN = {
            ALREADYKNOWN = "Ugh, I already know this one!",
        },
    },
    ACTIONFAIL_GENERIC = "I can'tttttt.",
    ANNOUNCE_ACCOMPLISHMENT = "Move, arrow! MOVE!",
    ANNOUNCE_ACCOMPLISHMENT_DONE = "I DID IT!",
    ANNOUNCE_ACIDBATS = "",
    ANNOUNCE_ADVENTUREFAIL = "You win THIS time, Maxwell.",
    ANNOUNCE_ANTLION_SINKHOLE = {
        "The ground's cracking!",
        "Tread carefully!",
        "Earthquake!",
    },
    ANNOUNCE_ANTLION_TRIBUTE = {
        "Hey, I gotcha something!",
        "Dinnertime!",
        "Great Antlion! Take this tribute, or whatever.",
    },
    ANNOUNCE_ARCHIVE_NEW_KNOWLEDGE = "Useful study material. I'll commit it to memory.",
    ANNOUNCE_ARCHIVE_NO_POWER = "Well that was exciting.",
    ANNOUNCE_ARCHIVE_OLD_KNOWLEDGE = "I've covered this before. Good review, though.",
    ANNOUNCE_ATRIUM_DESTABILIZING = {
        "I probably shouldn't be here.",
        "This seems dangerous!",
        "What was that?!",
    },
    ANNOUNCE_ATTACH_BUFF_ATTACK = "Attack distance secured. I can press in.",
    ANNOUNCE_ATTACH_BUFF_ELECTRICATTACK = "Sparky! I like!",
    ANNOUNCE_ATTACH_BUFF_MOISTUREIMMUNITY = "Ha! Take that, water!",
    ANNOUNCE_ATTACH_BUFF_PLAYERABSORPTION = "Defensive posture set. I can receive safely.",
    ANNOUNCE_ATTACH_BUFF_SLEEPRESISTANCE = "Yeah! I'll stay awake forever!",
    ANNOUNCE_ATTACH_BUFF_WORKEFFECTIVENESS = "Where did all this motivation come from?!",
    ANNOUNCE_BECOMEGHOST = "oOoOooOo!!",
    ANNOUNCE_BEES = "BEEEEEEEEEEEEES!!!!",
    ANNOUNCE_BOAT_LEAK = "Gross, I hate water.",
    ANNOUNCE_BOAT_SINK = "I'm taking on water. I may drown.",
    ANNOUNCE_BOOMERANG = "I should try to catch that!",
    ANNOUNCE_BRAVERY_POTION = "Haha! Those bats don't scare me anymore!",
    ANNOUNCE_BURNT = "Burn damage confirmed. I'll manage the fire more strictly next time.",
    ANNOUNCE_CALL_BEEF = "Beefalo, come here please.",
    ANNOUNCE_CANFIX = "\nI think I can fix this!",
    ANNOUNCE_CANNOT_BUILD = {
        NO_INGREDIENTS = "I need more stuff.",
        NO_STATION = "I can't make it right now.",
        NO_TECH = "I'm  not sure how to make that yet...",
    },
    ANNOUNCE_CANTBUILDHERE_YOTB_POST = "How would the judge even see my beefalo this far away?",
    ANNOUNCE_CANT_ESCAPE_CURSE = "Wait, what's this doing back in my pocket? Didn't I just...?",
    ANNOUNCE_CARRAT_ERROR_FELL_ASLEEP = "Wake up, please.",
    ANNOUNCE_CARRAT_ERROR_STUNNED = "Not so good with the reflexes, huh?",
    ANNOUNCE_CARRAT_ERROR_WALKING = "Um, can we maybe go a bit faster?!",
    ANNOUNCE_CARRAT_ERROR_WRONG_WAY = {
        "Wrong lane. The finish line is the other way.",
        "Not that direction. This way, please.",
    },
    ANNOUNCE_CARRAT_START_RACE = "Alright, let's win that prize!",
    ANNOUNCE_CAVEIN = "I better protect my head!",
    ANNOUNCE_CHAIR_ON_FIRE = "Oops, so hot!",
    ANNOUNCE_CHARLIE = "No reckless movement. Confirming the situation first.",
    ANNOUNCE_CHARLIE_ATTACK = "Hey! You coward!",
    ANNOUNCE_COLD = "So cold!",
    ANNOUNCE_COMPASS_OUT = "Arrgh the needle is stuck!",
    ANNOUNCE_COZY_SLEEP = "Wow, I think I needed that!",
    ANNOUNCE_CRAFTING_FAIL = "I'm missing something.",
    ANNOUNCE_DAMP = "Uh oh!",
    ANNOUNCE_DEERCLOPS = "That sounded like a big mean monster man!",
    ANNOUNCE_DESPAWN = "A burning light!",
    ANNOUNCE_DETACH_BUFF_ATTACK = "My attacking edge dropped. Resetting my stance.",
    ANNOUNCE_DETACH_BUFF_ELECTRICATTACK = "Lightning gone.",
    ANNOUNCE_DETACH_BUFF_MOISTUREIMMUNITY = "Ugh. Back to watching out for puddles.",
    ANNOUNCE_DETACH_BUFF_PLAYERABSORPTION = "Might not be the best time to pick a fight.",
    ANNOUNCE_DETACH_BUFF_SLEEPRESISTANCE = "Focus is fading. Rest is part of discipline too.",
    ANNOUNCE_DETACH_BUFF_WORKEFFECTIVENESS = "Welp. The motivation didn't last.",
    ANNOUNCE_DIG_DISEASE_WARNING = "It looks better already.",
    ANNOUNCE_DUSK = "It's getting late. It will be dark soon.",
    ANNOUNCE_EAT = {
        GENERIC = "Yummy!",
        INVALID = "How would I even eat that!",
        PAINFUL = "Ugh! Nasty!",
        SPOILED = "That tasted terrible!",
        STALE = "Oops, it's so unpalatable.",
        YUCKY = "Eeeew, no way!",
    },
    ANNOUNCE_EATING_NOT_FEASTING = "Fiiiiiine, I'll share with everyone else.",
    ANNOUNCE_ENCUMBERED = {
        "Ughhh!",
        "This... is... the worst...",
        "I can feel the burn... I hate it!",
        "Exercise... sucks!",
        "Hnnfhg!",
        "Why am I doing this...?!",
        "Oooof!!",
        "Unghhh!",
        "There's... no way this is worth it!",
        "Left foot... Right foot...",
        "Ughhhh! I'm all sweaty!",
    },
    ANNOUNCE_ENTER_DARK = "It's so dark!",
    ANNOUNCE_ENTER_LIGHT = "I can see again!",
    ANNOUNCE_EYEOFTERROR_ARRIVE = "What is that- a giant floating eyeball?!",
    ANNOUNCE_EYEOFTERROR_FLYAWAY = "Get back here, I'm not finished with you yet!",
    ANNOUNCE_EYEOFTERROR_FLYBACK = "Finally!",
    ANNOUNCE_FAN_OUT = "Stupid thing broke!",
    ANNOUNCE_FERTILIZER_RESEARCHED = "Fundamentals matter. This study is not wasted.",
    ANNOUNCE_FIRENETTLE_TOXIN = {
        "Owie, that was not a nice plant!",
        "Ouch, that's hot!",
    },
    ANNOUNCE_FIRENETTLE_TOXIN_DONE = "Huh. I was kind of getting used to it.",
    ANNOUNCE_FLARE_SEEN = "Oh, someone's calling me to come over.",
    ANNOUNCE_FREEDOM = "Recovered. Moving to the next action.",
    ANNOUNCE_GHOSTDRAIN = "My head's all fuzzy...",
    ANNOUNCE_HAIR_TOO_SHORT = "My hair isn't long enough for this.",
    ANNOUNCE_HASRAIKIRI = "This is not a lightning catastrophe, it was raikiri who protected me",
    ANNOUNCE_HEALINGSALVE_ACIDBUFF_DONE = "",
    ANNOUNCE_HIGHRESEARCH = "So much information!",
    ANNOUNCE_HOT = "Very Hot!",
    ANNOUNCE_HOUNDS = "I hear something.",
    ANNOUNCE_HUNGRY = "I need food!",
    ANNOUNCE_HUNGRY_FASTBUILD = {
        "This's hungry work.",
        "I'm working up an appetite.",
        "That made me a lil hungry.",
        "I'll need some brainfood at this rate.",
    },
    ANNOUNCE_HUNGRY_SLOWBUILD = {
        "I can eat a pig in three bites now!",
        "Oof. I got the brainfog.",
        "Maybe I could take a snack break?",
        "I could sure go for some grub.",
    },
    ANNOUNCE_HUNT_AVOID_FORK = "I'm not scared, I'd just... rather go this way!",
    ANNOUNCE_HUNT_BEAST_NEARBY = "I'll find you!",
    ANNOUNCE_HUNT_LOST_TRAIL = "Ohh... he got away.",
    ANNOUNCE_HUNT_LOST_TRAIL_SPRING = "Mud broke the trail. I'll calmly track it again.",
    ANNOUNCE_HUNT_START_FORK = "I don't like the look of those marks... not that I'm scared or anything!",
    ANNOUNCE_HUNT_SUCCESSFUL_FORK = "Heh... I've got the drop on 'em!",
    ANNOUNCE_HUNT_WRONG_FORK = "Did you hear something? Er... must be my imagination...",
    ANNOUNCE_INSUFFICIENTFERTILIZER = "It looks slightly happier.",
    ANNOUNCE_INV_FULL = "I can only carry so much!!",
    ANNOUNCE_ISNIGHTMAREWILD = "I don't seem to feel well......",
    ANNOUNCE_IS_FEASTING = "Can't talk, must eat.",
    ANNOUNCE_KITCOON_FOUND_IN_THE_WILD = "I knew I saw something hiding over here!",
    ANNOUNCE_KITCOON_HIDANDSEEK_FOUND = {
        "Found you!",
        "There you are.",
        "I knew you'd be hiding there!",
        "I see you!",
    },
    ANNOUNCE_KITCOON_HIDANDSEEK_FOUND_LAST_ONE = "I found the last one!",
    ANNOUNCE_KITCOON_HIDANDSEEK_FOUND_LAST_ONE_TEAM = "{name} found the last one!",
    ANNOUNCE_KITCOON_HIDANDSEEK_FOUND_ONE_MORE = "Now where's that last one hiding?",
    ANNOUNCE_KITCOON_HIDANDSEEK_LOSEGAME = "I guess they don't want to play any more...",
    ANNOUNCE_KITCOON_HIDANDSEEK_TIME_ALMOST_UP = "These little guys must be getting impatient...",
    ANNOUNCE_KITCOON_HIDANDSEEK_TOOFAR = "They probably wouldn't hide this far away, would they?",
    ANNOUNCE_KITCOON_HIDANDSEEK_TOOFAR_RETURN = "The kitcoons should be nearby.",
    ANNOUNCE_KITCOON_HIDEANDSEEK_JOIN = "I'm joining. Let's proceed properly.",
    ANNOUNCE_KITCOON_HIDEANDSEEK_START = "Starting now. Bow first, then move.",
    ANNOUNCE_KLAUS_CALLFORHELP = "Aww man, here come those weird goat things!",
    ANNOUNCE_KLAUS_ENRAGE = "No forcing this. I'm resetting distance.",
    ANNOUNCE_KLAUS_UNCHAINED = "Why couldn't it just stay dead?!",
    ANNOUNCE_KNOCKEDOUT = "Argh, my little head!",
    ANNOUNCE_LIGHTFIRE = {
        "Ignition complete. I'll control the flame.",
        "The flame is beautiful, but handling comes first.",
        "Good, it's lit.",
        "Procedure complete. No issues.",
        "Source check complete. Staying careful.",
        "Don't stare too long. Next action.",
    },
    ANNOUNCE_LIGHTNING_DAMAGE_AVOIDED = "I was almost struck down!",
    ANNOUNCE_LNCOMPREHENSIBLE = "I don't understand, but I'm shocked.",
    ANNOUNCE_LOWRESEARCH = "Not enough data. I'll gather more.",
    ANNOUNCE_LUNAR_RIFT_MAX = "Hey, what was that weird light?",
    ANNOUNCE_MEGA_FLARE_SEEN = "That's a signal flare. Moving there now.",
    ANNOUNCE_MONKEY_CURSE_1 = "Brrr, what was that? I just got goosebumps.",
    ANNOUNCE_MONKEY_CURSE_CHANGE = "What the... HEY! I don't want to be a monkey!",
    ANNOUNCE_MONKEY_CURSE_CHANGEBACK = "Ugh, I still feel itchy all over...",
    ANNOUNCE_MOONALTAR_MINE = {
        GLASS_LOW = "I can see you!",
        GLASS_MED = "Don't worry, I'll get you out.",
        GLASS_REVEAL = "Gotcha!",
        IDOL_LOW = "I can see you!",
        IDOL_MED = "Don't worry, I'll get you out.",
        IDOL_REVEAL = "Gotcha!",
        SEED_LOW = "I can see you!",
        SEED_MED = "Don't worry, I'll get you out.",
        SEED_REVEAL = "Gotcha!",
    },
    ANNOUNCE_MOONPOTION_FAILED = "Huh. Alright then.",
    ANNOUNCE_MOSQUITOS = "Get away, you bloodsucking!",
    ANNOUNCE_MOUNT_LOWHEALTH = "This beast is looking pretty bad.",
    ANNOUNCE_NODANGERAFK = "I'm not leaving now!",
    ANNOUNCE_NODANGERGIFT = "The gift can wait. Safety first.",
    ANNOUNCE_NODANGERSIESTA = "This is not rest time. Mission first.",
    ANNOUNCE_NODANGERSLEEP = "No sleep now. Threats first.",
    ANNOUNCE_NODAYSLEEP = "It's too bright out.",
    ANNOUNCE_NODAYSLEEP_CAVE = "Too spooky down here to sleep.",
    ANNOUNCE_NOHUNGERSIESTA = "I can't take a siesta while my tummy is grumbling!",
    ANNOUNCE_NOHUNGERSLEEP = "My tummy is grumbling, I can't sleep!",
    ANNOUNCE_NOMOUNTEDGIFT = "I have to get down from this big oaf before opening that!",
    ANNOUNCE_NONIGHTSIESTA = "I couldn't get comfortable there.",
    ANNOUNCE_NONIGHTSIESTA_CAVE = "I'd really rather be inside.",
    ANNOUNCE_NOSLEEPHASPERMANENTLIGHT = "It's kind of hard to sleep with a robot glowing in my face.",
    ANNOUNCE_NOSLEEPONFIRE = "It's just the collapsing that concerns me.",
    ANNOUNCE_NOWARDROBEONFIRE = "There's an active fire. Changing outfits is unsafe.",
    ANNOUNCE_NO_TRAP = "Well, that was easy.",
    ANNOUNCE_OCEANFISHING_BADCAST = "This really isn't my thing.",
    ANNOUNCE_OCEANFISHING_GOTAWAY = "It got away. I'll time the pull more precisely next cast.",
    ANNOUNCE_OCEANFISHING_IDLE_QUOTE = {
        "Where are the fish?",
        "Hmmm",
        "Zzzzzzzzz",
        "Maybe I should find a better fishing spot.",
    },
    ANNOUNCE_OCEANFISHING_LINESNAP = "Line snapped. I'll inspect the gear and retry.",
    ANNOUNCE_OCEANFISHING_LINETOOLOOSE = "Better start reeling or it's going to get away!",
    ANNOUNCE_OCEAN_SILHOUETTE_INCOMING = "Something big's coming!",
    ANNOUNCE_OFF_SCRIPT = "Oh, no!",
    ANNOUNCE_PECKED = "No! Bad birdy!",
    ANNOUNCE_PETRIFED_TREES = "Something weird is happening...!",
    ANNOUNCE_PICK_DISEASE_WARNING = "Ugh, put it back!",
    ANNOUNCE_PIRATES_ARRIVE = "Uh, that sound's getting closer...",
    ANNOUNCE_PLANT_RANDOMSEED = "I guess we'll see what happens.",
    ANNOUNCE_PLANT_RESEARCHED = {
        "Plant knowledge helps in real work too. Logged.",
    },
    ANNOUNCE_POCKETWATCH_PORTAL = "Owww...",
    ANNOUNCE_QUAKE = "That sound probably doesn't mean good things.",
    ANNOUNCE_REPELLED = "Hey! That's not fair!",
    ANNOUNCE_RESEARCH = "That was useful.",
    ANNOUNCE_REVIVED_FROM_CORPSE = "Thanks for the hand!",
    ANNOUNCE_REVIVED_OTHER_CORPSE = "You got this!",
    ANNOUNCE_REVIVING_CORPSE = "Hey! Get back up!",
    ANNOUNCE_ROYALTY = {
        "",
        "",
        "",
    },
    ANNOUNCE_RUINS_RESET = "Reset confirmed. I'll reorganize and clear it again.",
    ANNOUNCE_SACREDCHEST_NO = "Rude.",
    ANNOUNCE_SACREDCHEST_YES = "That did it!",
    ANNOUNCE_SCRAPBOOK_FULL = "Looks like stuff I already know.",
    ANNOUNCE_SHADOW_RIFT_MAX = "Did anyone else see that weird glow?",
    ANNOUNCE_SHELTER = "Cover confirmed. Safety first.",
    ANNOUNCE_SNARED = "Gross, ew! Bones!",
    ANNOUNCE_SNARED_IVY = "Hey, knock it off!",
    ANNOUNCE_SOAKED = "Ugh, this is the WORST!",
    ANNOUNCE_SPOOKED = "Were those bats, or am I just seeing things.",
    ANNOUNCE_TALK_TO_PLANTS = {
        "Show me your condition for today.",
        "Checking moisture and sunlight.",
        "Quiet maintenance is still important work.",
        "I won't miss small changes.",
        "Good progress. Keep this pace.",
    },
    ANNOUNCE_THORNS = "Ouch!",
    ANNOUNCE_THURIBLE_OUT = "Awww! I liked that thing.",
    ANNOUNCE_TICOON_ABANDONED = "I'll find those little guys on my own.",
    ANNOUNCE_TICOON_DEAD = "Poor guy... Now where was he leading me?",
    ANNOUNCE_TICOON_GET_LEADER_ATTENTION = "He really wants me to follow him.",
    ANNOUNCE_TICOON_LOST_KITCOON = "Looks like someone else found what he was looking for.",
    ANNOUNCE_TICOON_NEAR_KITCOON = "He must have found something!",
    ANNOUNCE_TICOON_NOTHING_TO_TRACK = "Looks like he couldn't find anything.",
    ANNOUNCE_TICOON_START_TRACKING = "He's caught the scent!",
    ANNOUNCE_TICOON_WAITING_FOR_LEADER = "I should follow him!",
    ANNOUNCE_TOADESCAPED = "Lost it. I'll take the next round seriously.",
    ANNOUNCE_TOADESCAPING = "Escape attempt confirmed. Pursuing now.",
    ANNOUNCE_TOOL_SLIP = "Grip error. Correcting hand position.",
    ANNOUNCE_TOOL_TOOWEAK = "Ugh. I'm going to need something way stronger than this...",
    ANNOUNCE_TORCH_OUT = "My precious light is gone!",
    ANNOUNCE_TOWNPORTALTELEPORT = "Returned. Rejoining immediately.",
    ANNOUNCE_TRAP_WENT_OFF = "Aah!",
    ANNOUNCE_UNIMPLEMENTED = "This feature is not ready yet.",
    ANNOUNCE_WASHED_ASHORE = "Great, now I'm all wet.",
    ANNOUNCE_WEAK_RAT = "That thing's looking rather rough.",
    ANNOUNCE_WEIGHT = "Weight: {weight}",
    ANNOUNCE_WEIGHT_HEAVY = "Weight: {weight}\nUgh, this thing's heavy!",
    ANNOUNCE_WET = "This could be bad!",
    ANNOUNCE_WETTER = "I hate it!",
    ANNOUNCE_WINCH_CLAW_MISS = "Aw, come on! I was close enough!",
    ANNOUNCE_WINCH_CLAW_NO_ITEM = "Looks like I caught a whole lot of nothing.",
    ANNOUNCE_WINTERS_FEAST_BUFF = "Body feels light. I can sharpen my movement.",
    ANNOUNCE_WINTERS_FEAST_BUFF_OVER = "Effect ended. Back to regular discipline.",
    ANNOUNCE_WORMHOLE = "That was not a sane thing to do.",
    ANNOUNCE_WORMS = "I do NOT want to see what made that sound!",
    ANNOUNCE_YOTB_LEARN_NEW_PATTERN = "I think I could make a new beefalo costume now.",
    ANNOUNCE_YOTD_NOBOATS = "I seem to have drifted a bit too far from the starting point.",
    ANNOUNCE_YOTD_NOCHECKPOINTS = "Alas, there is nary a waypoint to guide me in this race!",
    ANNOUNCE_YOTD_NOTENOUGHBOATS = "Perhaps some space should be cleared around the Start Tower.",
    BATTLECRY = {
        DEER = "Fair and square, I take this point!",
        GENERIC = "With respect, let's settle this!",
        PIG = "No carelessness. Closing distance.",
        PREY = "I won't lose you. Pursuing.",
        SPIDER = "Stay calm and cut clean.",
        SPIDER_WARRIOR = "Hold the line, then slash through!",
    },
    COMBAT_QUIT = {
        GENERIC = "Retreating, are you? I'll finish it next time.",
        PIG = "No overchase. Resetting my form.",
        PREY = "I'll cut you off next round.",
        SPIDER = "No reckless pursuit. Safety first.",
        SPIDER_WARRIOR = "Next exchange, I win the cut.",
    },
    DESCRIBE = {
        ABIGAIL = {
            PROMPT = "There is nothing tenseiga can do.",
        },
        ABIGAIL_FLOWER = {
            GENERIC = "Pretty petals!",
            HAUNTED_GROUND = "I don't want to mess with that.",
            HAUNTED_POCKET = "Strong presence here. Handle with care.",
            LEVEL1 = "Ugh, it's weird!",
            LEVEL2 = "Do not provoke it. Observe first.",
            LEVEL3 = "Yeah sure, it's floating now, why not.",
            LONG = "I think it's listening to me!",
            MEDIUM = "Ugh, it's weird!",
            SOON = "No rushing. Follow procedure.",
        },
        ACCOMPLISHMENT_SHRINE = "I hate that arrow!",
        ACORN = "Hey there, tree seed.",
        ACORN_COOKED = "Looks like you won't become a tree after all.",
        ACORN_SAPLING = "You'll be a real tree soon.",
        ADVENTURE_PORTAL = "Maybe that leads home.",
        ALTERGUARDIANHAT = "It makes the inside of my head so noisy...",
        ALTERGUARDIANHATSHARD = "It was surprisingly hard to pull that thing apart.",
        ALTERGUARDIAN_CONTAINED = "What's that weird machine?",
        ALTERGUARDIAN_PHASE1 = {
            DEAD = "Heh. Take that moon, or whatever!",
            GENERIC = "We should have followed procedure with that unstable research.",
        },
        ALTERGUARDIAN_PHASE2 = {
            DEAD = "Got it for sure that time.",
            GENERIC = "You think you're so tough?!",
        },
        ALTERGUARDIAN_PHASE2SPIKE = "Ugh, get these things out of my way!",
        ALTERGUARDIAN_PHASE3 = "Beautiful, but I'd rather keep it intact.",
        ALTERGUARDIAN_PHASE3DEAD = "Guess it's finally done.",
        ALTERGUARDIAN_PHASE3DEADORB = "Uh, hey old guy? You might not want to get that close...",
        ALTERGUARDIAN_PHASE3TRAP = "Hey, that's just playing dirty!",
        AMULET = "I have no idea what it does, but it feels good to wear it!",
        ANCHOR = "Lowering it is easier than pulling it back up.",
        ANCHOR_ITEM = "Ooo, let's build an anchor.",
        ANCIENT_ALTAR = "Oooo! An afterlife intercom.",
        ANCIENT_ALTAR_BROKEN = "The dead can't get through here.",
        ANCIENT_STATUE = "Too dense to ignite.",
        ANIMAL_TRACK = "It leads to my new friend.",
        ANTCHOVIES = "They taste super salty and gross.",
        ANTLION = {
            GENERIC = "What do you want?!",
            UNHAPPY = "There's going to be tremors in our future.",
            VERYHAPPY = "You're in a good mood.",
        },
        ANTLIONHAT = "Looks unusual, but it's practical.",
        ANTLIONTRINKET = "Buncha junk.",
        ARCHIVE_CENTIPEDE = "Metal bug is angry.",
        ARCHIVE_CENTIPEDE_HUSK = "Is big pile of junk.",
        ARCHIVE_COOKPOT = {
            BURNT = "The pot got cooked.",
            COOKING_LONG = "This is going to take a while.",
            COOKING_SHORT = "It's almost done!",
            DONE = "Mmmmm! It's ready to eat!",
            EMPTY = "Let's dust off this old crockery, shall we?",
        },
        ARCHIVE_LOCKBOX = "Great. We turned on the mysterious thing and got another mysterious thing.",
        ARCHIVE_LOCKBOX_DISPENCER = {
            GENERIC = "Ugh, I feel a puzzle coming on...",
            POWEROFF = "Looks like it's busted.",
        },
        ARCHIVE_MOON_STATUE = "Ha! Wolfgang could carry big rock all by himself!",
        ARCHIVE_ORCHESTRINA_MAIN = "No ignition response. I'll need a different approach.",
        ARCHIVE_PORTAL = {
            GENERIC = "Shoulda known it wouldn't be that easy.",
            POWEROFF = "Another portal? You don't think...?",
        },
        ARCHIVE_RESONATOR = {
            GENERIC = "Point the way!",
            IDLE = "Nothing left to find, I guess.",
        },
        ARCHIVE_RESONATOR_ITEM = "It's humming to itself.",
        ARCHIVE_RUNE_STATUE = {
            LINE_1 = "Is nice statue, but is covered in scribblemarks.",
            LINE_2 = "Very fancy.",
            LINE_3 = "Is nice statue, but is covered in scribblemarks.",
            LINE_4 = "Very fancy.",
            LINE_5 = "Is nice statue, but is covered in scribblemarks.",
        },
        ARCHIVE_SECURITY_DESK = {
            GENERIC = "Yeah, that doesn't look suspicious.",
            POWEROFF = "Whatever it is, it's not working.",
        },
        ARCHIVE_SECURITY_PULSE = "Hey! Get back here!",
        ARCHIVE_SWITCH = {
            GEMS = "Something's missing here.",
            VALID = "I'm sure nobody would mind if I borrowed one of those gems.",
        },
        ARMORDRAGONFLY = "Another creature casing.",
        ARMORDREADSTONE = "It's fine I guess.",
        ARMORGRASS = "It just feels like more hair.",
        ARMORMARBLE = "If you're going to fight you might as well be protected.",
        ARMORRUINS = "Human beings probably shouldn't wear this.",
        ARMORSKELETON = "Can't hit me in this thing!",
        ARMORSLURPER = "Oh, ick! Ick! Ick! Ick! Eeeeeewwwwww!",
        ARMORSNURTLESHELL = "Less defense! More offense!",
        ARMORWAGPUNK = "It looks pretty fireproof, but there's only one way to know for sure...",
        ARMORWOOD = "Now I can take on the world!",
        ARMOR_BRAMBLE = "I think Wormwood made it.",
        ARMOR_LUNARPLANT = "I make it look good.",
        ARMOR_LUNARPLANT_HUSK = "",
        ARMOR_SANITY = "Like being wrapped in smoke.",
        ARMOR_VOIDCLOTH = "I think I look good in red.",
        ARROWSIGN_PANEL = {
            BURNT = "\"Don't play with matches.\"",
            GENERIC = "It says \"Thataway\".",
            UNWRITTEN = "The sign is currently blank.",
        },
        ARROWSIGN_POST = {
            BURNT = "\"Don't play with matches.\"",
            GENERIC = "It says \"Thataway\".",
            UNWRITTEN = "The sign is currently blank.",
        },
        ASH = {
            GENERIC = "Leftovers from a fire. I wish it was still here.",
            REMAINS_EYE_BONE = "I bet that eye thing was quite a sight wreathed in fire!",
            REMAINS_GLOMMERFLOWER = "I wish I saw the way that weird flower burned.",
            REMAINS_THINGIE = "I wish this thing was still burning, whatever it was.",
        },
        ASPARAGUS = "A vegetable.",
        ASPARAGUSSOUP = "Smells like it tastes.",
        ASPARAGUS_COOKED = "Not so chewy now, are you?",
        ASPARAGUS_OVERSIZED = "I have a feeling it'll be asparagus leftovers for a long time...",
        ASPARAGUS_SEEDS = "This'll make some food.",
        ATRIUM_GATE = {
            CHARGING = "It's gaining power.",
            COOLDOWN = "It needs time to recover. Me too.",
            DESTABILIZING = "The gateway is destabilizing.",
            OFF = "The essential components are still intact.",
            ON = "Back in working order.",
        },
        ATRIUM_KEY = "There is power emanating from it.",
        ATRIUM_LIGHT = {
            OFF = "Something must power it.",
            ON = "A truly unsettling light.",
        },
        ATRIUM_OVERGROWTH = "It's in some other language.",
        ATRIUM_RUBBLE = {
            LINE_1 = "It depicts an old civilization. The people look hungry and scared.",
            LINE_2 = "This tablet is too worn to make out.",
            LINE_3 = "Something dark creeps over the city and its people.",
            LINE_4 = "The people are shedding their skins. They look different underneath.",
            LINE_5 = "It shows a massive, technologically advanced city.",
        },
        ATRIUM_STATUE = "It doesn't seem fully real.",
        AXE = "It's very sharp.",
        BABYBEEFALO = {
            GENERIC = "khwai-noy.",
            SLEEPING = "Hello!",
        },
        BACKPACK = "You could fit like a million lighters in here.",
        BACONEGGS = "The yellow mucus-y part is gross, but the bacon is great!",
        BAKUSAIGA = "The power of a great yokai—this is proof of its strength.",
        BALLOON = "That's just asking to be popped.",
        BALLOONHAT = "Hey, a rabbit! Not bad!",
        BALLOONPARTY = "Hey, when are you going to make a hot air balloon? I could help!",
        BALLOONSPEED = {
            DEFLATED = "No flying away!",
            GENERIC = "Clownman has mighty lungs to make balloon so big!",
        },
        BALLOONS_EMPTY = "Is there going to be a party?!",
        BALLOONVEST = "Squeak-squeak-squeak! Ha ha where's Maxwell, this'll drive him crazy!",
        BANANABUSH = "That bush is bananas!",
        BANANAJUICE = "I... like it.",
        BANANAPOP = "This is the opposite of burning.",
        BANDAGE = "Eww, no!",
        BARNACLE = "Ew, it came from the water.",
        BARNACLEPITA = "Pita pita, barnacle eater.",
        BARNACLESTUFFEDFISHHEAD = "You know, I don't think I'm that hungry after all.",
        BARNACLESUSHI = "I can still see the barnacle hiding under there.",
        BARNACLE_COOKED = "Fire made it better.",
        BARNACLINGUINE = "Don't mind if I do.",
        BASALT = "It's too hard to break!",
        BAT = "Cute little guy!",
        BATBAT = "That thing's great for batting at enemies.",
        BATCAVE = "What would happen if I dropped a match in there?",
        BATHBOMB = "I still don't want to take a bath.",
        BATNOSE = "Ewww.",
        BATNOSEHAT = "Is that even a real hat?",
        BATNOSE_COOKED = "Is that supposed to make it better?",
        BATTLESONG_CONTAINER = "Whose song sheet is in the jar?",
        BATTLESONG_DURABILITY = "Hey look, fancy kindling!",
        BATTLESONG_FIRERESISTANCE = "Hey look, fancy kindling!",
        BATTLESONG_HEALTHGAIN = "Hey look, fancy kindling!",
        BATTLESONG_INSTANT_PANIC = "Boring words I don't care about.",
        BATTLESONG_INSTANT_REVIVE = "Isn't this just yelling?",
        BATTLESONG_INSTANT_TAUNT = "Heh. Okay, I'm using that one.",
        BATTLESONG_LUNARALIGNED = "Oh, you don't want to hear me sing, do you?",
        BATTLESONG_SANITYAURA = "Hey look, fancy kindling!",
        BATTLESONG_SANITYGAIN = "Hey look, fancy kindling!",
        BATTLESONG_SHADOWALIGNED = "No. I don't sing.",
        BATWING = "Yuck!",
        BATWING_COOKED = "Still yuck!",
        BEARDHAIR = "Clean up your gross hair guys! Ugh!",
        BEARGER = "Whoa! Niiiice bear...",
        BEARGERCORPSE = {
            BURNING = "It's so relaxing to watch it burn.",
            GENERIC = "Secure it properly, just in case.",
            REVIVING = "I knew I should've burnt it when I had the chance!!",
        },
        BEARGERFUR_SACK = "Can't believe we made a cooler out of you.",
        BEARGERVEST = "It's like swimming in fur.",
        BEARGER_FUR = "It's fur all the way down.",
        BEAST_BLUEPRINT = "My beefalo's already a lucky beast. He gets to be my friend!",
        BEDROLL_FURRY = "It's too frilly.",
        BEDROLL_STRAW = "Musty.",
        BEE = {
            GENERIC = "It's fat, but that stinger looks dangerous.",
            HELD = "Pocket full of bees!",
        },
        BEEBOX = {
            BURNT = "Smoked you out!",
            FULLHONEY = "Yay! Let's steal honey!",
            GENERIC = "Come on fat bees, make honey!",
            NOHONEY = "Nothing to see here.",
            READY = "Yay! Let's steal honey!",
            SOMEHONEY = "Patience.",
        },
        BEEBOX_HERMIT = {
            BURNT = "Smoked you out!",
            FULLHONEY = "She won't be mad if I steal a liiiittle honey, right?",
            GENERIC = "Wow, her bees are about as friendly as she is.",
            NOHONEY = "Nothing to see here.",
            READY = "She won't be mad if I steal a liiiittle honey, right?",
            SOMEHONEY = "Patience.",
        },
        BEEFALO = {
            DOMESTICATED = "khwai.. khwai.. khwai...",
            FOLLOWER = "Er, are you following me?",
            GENERIC = "khwai.. khwai.. khwai...",
            MYPARTNER = "We're khwai friends forever.",
            NAKED = "Aww...",
            ORNERY = "It looks deeply angry khwai.",
            PUDGY = "You need to burn some calories khwai.",
            RIDER = "Let's go! khwai!!",
            SLEEPING = "They look cute when they're sleeping.",
        },
        BEEFALOFEED = "I don't eat cow feed",
        BEEFALOHAT = "Beast hair to go over people hair!",
        BEEFALOTREAT = "Beefalo are pretty easy to please.",
        BEEFALOWOOL = "Thick fur.",
        BEEFALO_GROOMER = {
            BURNT = "Heh, well that was fun.",
            GENERIC = "Come on beefalo, it's dress-up time!",
            OCCUPIED = "Honestly, anything I do will be an improvement.",
        },
        BEEFALO_GROOMER_ITEM = "Uuuugh, why is everything so much work?",
        BEEF_BELL = "Wow, making friends is easy!",
        BEEGUARD = "Puffy flying!",
        BEEHAT = "This will keep the pokeys off of me.",
        BEEHIVE = "It's full of bees!",
        BEEMINE = "It buzzes when shaken.",
        BEEMINE_MAXWELL = "Mosquitoes inside. They don't sound happy.",
        BEEQUEEN = "No amount of honey is worth THAT!",
        BEEQUEENHIVE = {
            GENERIC = "It all came from a bee's butt.",
            GROWING = "It's expanding quickly. Control it early.",
        },
        BEEQUEENHIVEGROWN = "Dare you guys to smack it with a hammer.",
        BEESWAX = "That's none of my beeswax.",
        BEESWAX_SPRAY = "",
        BEETLETAUR = "Bet you think you're safe in all that armor.",
        BELL = "Is it New Year's Eve? Are there FIREWORKS?!",
        BERNIE_ACTIVE = "That teddy bear is moving!",
        BERNIE_BIG = "It's creepy and cute at the same time!!",
        BERNIE_INACTIVE = {
            BROKEN = "It's all busted up.",
            GENERIC = "A teddy bear.",
        },
        BERRIES = "Red berries taste the best.",
        BERRIES_COOKED = "I don't think heat improved them.",
        BERRIES_JUICY = "Extra tasty, though they won't last long.",
        BERRIES_JUICY_COOKED = "Better eat them before they spoil!",
        BERRYBUSH = {
            BARREN = "No growth. Add nutrients and monitor.",
            BURNING = "Not much I can do now.",
            DISEASED = "Disease detected. Remove and contain.",
            DISEASING = "Blech. You smell.",
            GENERIC = "Mmmmm. Berries.",
            PICKED = "But I want more berries!",
            WITHERED = "Nothing will grow in this heat.",
        },
        BERRYBUSH_JUICY = {
            BARREN = "Did I say you could stop growing?!",
            BURNING = "Not much I can do now.",
            DISEASED = "Disease detected. Remove and contain.",
            DISEASING = "Blech. You smell.",
            GENERIC = "Ready for picking!",
            PICKED = "Make mooooore!",
            WITHERED = "It's burning up.",
        },
        BERRYSAUCE = "This is the best part!",
        BIBINGKA = "Soft and spongy.",
        BIGFOOT = "What in the world!",
        BIRCHNUTDRAKE = "A mad little nut.",
        BIRDCAGE = {
            DEAD = "At least he's quiet now.",
            GENERIC = "Bird prison!",
            HUNGRY = "What's all the fuss about?",
            OCCUPIED = "Ha! Gotcha now!",
            SKELETON = "That should probably be cleaned up.",
            SLEEPING = "Stupid bird. Wake up!",
            STARVING = "He won't be quiet!",
        },
        BIRDTRAP = "Gonna catch those creepy birds.",
        BIRD_EGG = "It smells like a bird's butt.",
        BIRD_EGG_COOKED = "Yuck. The yellow part is all runny.",
        BIRD_MUTANT = "It looks like it wants to take a bite out of me.",
        BIRD_MUTANT_SPITTER = "Oh yeah?! Two can play at that game... PTOOEY!",
        BISHOP = "It's a bishop!",
        BISHOP_CHARGE_HIT = "Owwww!",
        BISHOP_NIGHTMARE = "Ahhh!",
        BLACKFLAG = "Looks like it's already charred.",
        BLOWDART_FIRE = "That's my favorite thing in the whole wide world.",
        BLOWDART_LAVA = "Ptoo! Now you're dead!",
        BLOWDART_LAVA2 = "Ooh, the firepower!",
        BLOWDART_PIPE = "I like a weapon you can shoot from far away. Preferably while running.",
        BLOWDART_SLEEP = "Just don't breathe in.",
        BLOWDART_YELLOW = "It's a shock to the system.",
        BLUEAMULET = "Boo to this.",
        BLUEGEM = "Ugh. This one is ugly.",
        BLUEMOONEYE = "That's not an eyeball! It's a rock!",
        BLUEPRINT = {
            COMMON = "This will save some experimentation.",
            RARE = "Woah. It doesn't burn!!",
        },
        BLUE_CAP = "It smells like a gym sock!",
        BLUE_CAP_COOKED = "Transformed by fire!",
        BLUE_MUSHROOM = {
            GENERIC = "Dumb mushroom.",
            INGROUND = "Hey! You! Get up here!",
            PICKED = "Maybe it will return some day.",
        },
        BOARDS = "Boards. They'll burn, same as other wood.",
        BOARON = "Keep that snout to yourself!",
        BOARRIOR = "He looks pretty haggard.",
        BOATFRAGMENT03 = "Welp, there goes that.",
        BOATFRAGMENT04 = "Welp, there goes that.",
        BOATFRAGMENT05 = "Welp, there goes that.",
        BOATPATCH = "Repairs are routine, but necessary.",
        BOATRACE_CHECKPOINT = "My brain is going into overload.",
        BOATRACE_CHECKPOINT_THROWABLE_DEPLOYKIT = "Strategic.",
        BOATRACE_PRIMEMATE = "Tryin' to throw a monkeywrench in my race",
        BOATRACE_SEASTACK = "It's trying to wreak havoc on my game",
        BOATRACE_SEASTACK_MONKEY = "Watch out for that buoy!",
        BOATRACE_SEASTACK_MONKEY_THROWABLE_DEPLOYKIT = "Into the water with ya!",
        BOATRACE_SEASTACK_THROWABLE_DEPLOYKIT = "Into the water with ya!",
        BOATRACE_SPECTATOR_DRAGONLING = "Clap! Clap if you believe in me!",
        BOATRACE_START = "Here we go?",
        BOATRACE_START_THROWABLE_DEPLOYKIT = "No, I don't want to play.",
        BOAT_BUMPER_KELP = "Ewww, it's slimy!",
        BOAT_BUMPER_KELP_KIT = "I guess building some bumpers is better than sinking.",
        BOAT_BUMPER_SHELL = "Uh-huh, nothing sturdier than seashells.",
        BOAT_BUMPER_SHELL_KIT = "I guess building some bumpers is better than sinking.",
        BOAT_BUMPER_YOTD = "Those teeth are just decoration.",
        BOAT_BUMPER_YOTD_KIT = "Let's paddle together~",
        BOAT_CANNON = {
            AMMOLOADED = "The only fun part of sailing is getting to fire the cannon.",
            GENERIC = "It isn't loaded? Booooring.",
            NOAMMO = "Darn it, I'm out... hey Wes, you always wanted to try being a human cannonball right?",
        },
        BOAT_CANNON_KIT = "If I build it, I get to fire it.",
        BOAT_GRASS_ITEM = "But it won't be as flammable if I put it in the water!",
        BOAT_ITEM = "That's the boat part of the boat.",
        BOAT_LEAK = "I should patch that up before we sink.",
        BOAT_MAGNET = {
            ACTIVATED = "Okay fiiiiine, I guess it works alright.",
            GENERIC = "A large magnet. Functional enough.",
        },
        BOAT_MAGNET_BEACON = {
            ACTIVATED = "I guess that means it's working.",
            GENERIC = "This will attract any strong magnets nearby.",
        },
        BOAT_MAGNET_KIT = "Wilson! How am I supposed to put all this junk together?!",
        BOAT_MSURFBOARD = "Ride the waves!",
        BOAT_ROTATOR = "Hehehe, let's see how fast we can spin without getting sick!",
        BOAT_ROTATOR_KIT = "Alright, I'll install the rudder properly.",
        BODYPILLOW_BEEFALOWOOL = "Bernie's cuddlier than a hairy old pillow.",
        BODYPILLOW_KELP = "Ew, this pillow's full of wet junk!",
        BODYPILLOW_PETALS = "I guess that's one use for flowers.",
        BODYPILLOW_STEELWOOL = "Pretty sure this is the worst pillow ever.",
        BOMB_LUNARPLANT = "Heh. Think fast!",
        BOMB_LUNARPLANT_WORMWOOD = "Wormwood must've been messing around with that one.",
        BONESHARD = "I don't think they make good tinder.",
        BONESOUP = "I would practically INHALE that!",
        BONESTEW = "Smells like Sunday supper.",
        BOOKSTATION = {
            BURNT = "Knowledge can't be treated like this",
            GENERIC = "Hey, who can remember what I learned in high school?",
        },
        BOOK_BEES = "Beeeeeeeeeeeeeeeeeeeeeeee!!!",
        BOOK_BIRDS = "Study first, action second.",
        BOOK_BRIMSTONE = "That's my favorite book!",
        BOOK_ELEMENTAL = "Looks interesting, for a book.",
        BOOK_FIRE = "I don't need books to teach me this!",
        BOOK_FISH = "It's as interesting as fishing.",
        BOOK_FOSSIL = "Looks like kindling to me.",
        BOOK_GARDENING = "Basic cultivation knowledge. Useful.",
        BOOK_HORTICULTURE = "Horticulture notes. Worth learning.",
        BOOK_HORTICULTURE_UPGRADED = "The knowledge within shall grant us bountiful harvests.",
        BOOK_LIGHT = "Sounds pretty mysterious!",
        BOOK_LIGHT_UPGRADED = "Let's keep this one close at hand, shall we?",
        BOOK_MOON = "I'm looking up at the moon~",
        BOOK_RAIN = "Ugh, I hate rain!",
        BOOK_RESEARCH_STATION = "More knowledge!",
        BOOK_SILVICULTURE = "Hey, it's a book about kindling!",
        BOOK_SLEEP = "Bo-oring!",
        BOOK_TEMPERATURE = "Yaaaawwwn.",
        BOOK_TENTACLES = "Looks like kindling to me!",
        BOOK_WEB = "Ew, who wants to read a book about spiders?",
        BOOMERANG = "It's not the most exciting weapon.",
        BOOTLEG = "Monkey pirates don't know how to wear shoes.",
        BROKEN_FORGEDITEM = "Welp, looks like it's busted.",
        BRUSH = "It smells like burnt hair.",
        BUGNET = "Doesn't fit over Webber's big head.",
        BULLKELP_PLANT = {
            GENERIC = "I bet we could eat that.",
            PICKED = "Nothing left to take.",
        },
        BULLKELP_ROOT = "Haha! This thing's great.",
        BUNDLE = "That oughta keep everything nice and fresh.",
        BUNDLEWRAP = "We could hide some gross things, I guess.",
        BUNNYMAN = "Unusual appearance, but I'll stay respectful.",
        BUNNYSTEW = "It warms from the inside out.",
        BURNT_MARSH_BUSH = "All burned up.",
        BUSHHAT = "Too passive!",
        BUTTER = "Tasty, and just a little bit insecty.",
        BUTTERFLY = {
            GENERIC = "Flutter away, butterfly!",
            HELD = "I want to squish it.",
        },
        BUTTERFLYMUFFIN = "Heehee, look at that butterfly stuck in the muffin.",
        BUTTERFLYWINGS = "No more flying for that butterfly!",
        BUZZARD = "Your neck is gross.",
        CABBAGEROLLS = "Yeah, I can roll with these.",
        CACTUS = {
            GENERIC = "Spines! My weakness! How did you know?",
            PICKED = "We'll call it a tie.",
        },
        CACTUS_FLOWER = "Yet another flower.",
        CACTUS_MEAT = "It still looks dangerous.",
        CACTUS_MEAT_COOKED = "It seems fine now.",
        CALIFORNIAROLL = "Oh, goody. Fish rolled in seaweed.",
        CAMPFIRE = {
            EMBERS = "That fire needs more fuel or it's going to go out.",
            GENERIC = "Sure beats darkness.",
            HIGH = "That fire is getting out of hand!",
            LOW = "The fire's getting a bit low.",
            NORMAL = "Nice and comfy.",
            OUT = "Well, that's over.",
        },
        CANARY = {
            GENERIC = "If it kicks the bucket I'm outta here.",
            HELD = "You still breathing? Just checking.",
        },
        CANARY_POISONED = "Uh, are you feeling okay?",
        CANDYBAG = "Treats, treats, treats!",
        CANE = "It's way easier to cover ground with this!",
        CANNONBALL_ROCK_ITEM = "Okay, but I still think fireballs would be better.",
        CARNIVALCANNON = {
            COOLDOWN = "I only wish it had a more fiery explosion.",
            GENERIC = "Let's liven things up a bit!",
        },
        CARNIVALCANNON_KIT = "Hehehe, this will be fun.",
        CARNIVALDECOR_BANNER = "Oh yeah, that'll catch the firelight nicely!",
        CARNIVALDECOR_BANNER_KIT = "Can somebody else put it together for me?",
        CARNIVALDECOR_EGGRIDE = "I could watch it for hours.",
        CARNIVALDECOR_EGGRIDE_KIT = "Can somebody else put it together for me?",
        CARNIVALDECOR_FIGURE = {
            GENERIC = "Aww, it's so cute!",
            RARE = "Wow, a rare one!",
            UNCOMMON = "If I gather enough, I can make a proper display.",
        },
        CARNIVALDECOR_FIGURE_KIT = "Ooooh what's in the box?",
        CARNIVALDECOR_FIGURE_KIT_SEASON2 = "Ooooh what's in the box?",
        CARNIVALDECOR_LAMP = "It makes light.",
        CARNIVALDECOR_LAMP_KIT = "Can somebody else put it together for me?",
        CARNIVALDECOR_PLANT = "Guess this tree never got its growth spurt.",
        CARNIVALDECOR_PLANT_KIT = "Can somebody else put it together for me?",
        CARNIVALFOOD_CORNTEA = "It keeps getting stuck in my teeth.",
        CARNIVALGAME_FEEDCHICKS_FOOD = "I don't need to chew them up first, do I?",
        CARNIVALGAME_FEEDCHICKS_KIT = "Aw come on, I've got to set it up myself?",
        CARNIVALGAME_FEEDCHICKS_NEST = "A little door to nowhere special.",
        CARNIVALGAME_FEEDCHICKS_STATION = {
            GENERIC = "I've uh... never actually been to a carnival before. Do I give it something?",
            PLAYING = "Heh. This actually looks rather fun.",
        },
        CARNIVALGAME_HERDING_CHICK = "Hey! Get back here!",
        CARNIVALGAME_HERDING_KIT = "Aw come on, I've got to set it up myself?",
        CARNIVALGAME_HERDING_STATION = {
            GENERIC = "I've uh... never actually been to a carnival before. Do I give it something?",
            PLAYING = "Run eggs, run!",
        },
        CARNIVALGAME_MEMORY_CARD = {
            GENERIC = "A little door to nowhere special.",
            PLAYING = "Yeah, that's the one!",
        },
        CARNIVALGAME_MEMORY_KIT = "Aw come on, I've got to set it up myself?",
        CARNIVALGAME_MEMORY_STATION = {
            GENERIC = "I've uh... never actually been to a carnival before. Do I give it something?",
            PLAYING = "Too much brain work, this one gets the torch.",
        },
        CARNIVALGAME_PUCKDROP_KIT = "Aw come on, I've got to set it up myself?",
        CARNIVALGAME_PUCKDROP_STATION = {
            GENERIC = "I've uh... never actually been to a carnival before. Do I give it something?",
            PLAYING = "Bounce that way! No, THAT way!!",
        },
        CARNIVALGAME_SHOOTING_BUTTON = {
            GENERIC = "I've uh... never actually been to a carnival before. Do I give it something?",
            PLAYING = "Fire! Fire!",
        },
        CARNIVALGAME_SHOOTING_KIT = "Aw come on, I've got to set it up myself?",
        CARNIVALGAME_SHOOTING_STATION = {
            GENERIC = "I've uh... never actually been to a carnival before. Do I give it something?",
            PLAYING = "Ooh, games where you get to hit something are my favorite!",
        },
        CARNIVALGAME_SHOOTING_TARGET = {
            GENERIC = "A little door to nowhere special.",
            PLAYING = "Think fast, bugs!",
        },
        CARNIVALGAME_WHEELSPIN_KIT = "Aw come on, I've got to set it up myself?",
        CARNIVALGAME_WHEELSPIN_STATION = {
            GENERIC = "I've uh... never actually been to a carnival before. Do I give it something?",
            PLAYING = "Wow, I'm great at this game!",
        },
        CARNIVAL_BALL = "Think fast, Maxwell!",
        CARNIVAL_CROWKID = "I wonder where they've been hiding all this time.",
        CARNIVAL_GAMETOKEN = "What is this, bird money?",
        CARNIVAL_HOST = "I like him.",
        CARNIVAL_PLAZA = {
            GENERIC = "Hm, the ground around it does looks kind of empty...",
            LEVEL_2 = "I think those weird birds like the decorations.",
            LEVEL_3 = "Alright, I'm bored of decorating. Moving on.",
        },
        CARNIVAL_PLAZA_KIT = "Will make nice tree for birds.",
        CARNIVAL_PRIZEBOOTH = {
            GENERIC = "Ooooh, prizes!",
        },
        CARNIVAL_PRIZEBOOTH_KIT = "Uuuugh, one more thing to build.",
        CARNIVAL_PRIZETICKET = {
            GENERIC = "Hey, kindling!",
            GENERIC_LARGESTACK = "It's a good thing we have extra arms to carry all these tickets!",
            GENERIC_SMALLSTACK = "Maybe now we can go get a prize!",
        },
        CARNIVAL_SEEDPACKET = "Ugh, bird food.",
        CARNIVAL_VEST_A = "Flaming red, just how I like it!",
        CARNIVAL_VEST_B = "It's like wearing my own shade tree.",
        CARNIVAL_VEST_C = "Breezy.",
        CARPENTRY_STATION = {
            BURNT = "See?",
            GENERIC = "I love stuff made out of wood!",
        },
        CARRAT = {
            DEAD = "Ew, it's dead.",
            GENERIC = "I want to roast it!",
            HELD = "It keeps squirming.",
            SLEEPING = "It's snoring.",
        },
        CARRAT_GHOSTRACER = "That's a weird looking Carrat.",
        CARROT = "Yuck. It's all vegetabley.",
        CARROT_COOKED = "Still vegetabley, but better for having been in fire.",
        CARROT_OVERSIZED = "A big old carrot.",
        CARROT_PLANTED = "Maybe its special carrot friend is in the ground.",
        CARROT_SEEDS = "Some seeds.",
        CARTOGRAPHYDESK = {
            BURNING = "Yes!!",
            BURNT = "Scouts don't need maps, anyway.",
            GENERIC = "I guess I could show everyone where I've been.",
        },
        CATCOON = "Meow.. Meow.. Meow...",
        CATCOONDEN = {
            EMPTY = "The nine lives thing is true!",
            GENERIC = "Trunk house.",
        },
        CATCOONHAT = "Meow hat.",
        CATTOY_MOUSE = "Mice with wheels..",
        CAVEIN_BOULDER = {
            GENERIC = "Let's just smash it up.",
            RAISED = "I can't get up there!",
        },
        CAVE_BANANA = "Bananas!",
        CAVE_BANANA_BURNT = "That's all I wanted!",
        CAVE_BANANA_COOKED = "Yum!",
        CAVE_BANANA_TREE = "Looks burnable!",
        CAVE_ENTRANCE = "Who plugged that hole?",
        CAVE_ENTRANCE_OPEN = {
            FULL = "There's too many people down there.",
            GENERIC = "I don't want to go in that gross hole!",
            OPEN = "I hope there's lava down there somewhere.",
        },
        CAVE_ENTRANCE_RUINS = "Who plugged that hole?",
        CAVE_EXIT = {
            FULL = "There's too many people up there.",
            GENERIC = "It's cooler down here, anyway.",
            OPEN = "It's too dark and stuffy down here.",
        },
        CAVE_FERN = "Swirly plants.",
        CEVICHE = "It's still slimy.",
        CHARCOAL = "Mmmm. Smells like fire.",
        CHARLIE_HAND = "You want something from me?",
        CHARLIE_HECKLER = "They're just here to stir up drama.",
        CHARLIE_LECTURN = "Is someone doing a play?",
        CHARLIE_NPC = "Have I seen her before...?",
        CHARLIE_STAGE_POST = "It's a setup! It feels too... staged.",
        CHESSJUNK1 = "Dead windup horsey.",
        CHESSJUNK2 = "Dead windup priest.",
        CHESSJUNK3 = "Dead windup castle.",
        CHESSPIECE_ANCHOR = "It's as heavy as it looks.",
        CHESSPIECE_ANTLION = "Can't shake anything up like that.",
        CHESSPIECE_BEARGER = "Those are some pointy teeth.",
        CHESSPIECE_BEARGER_MUTATED = "Somehow it seems crankier than the real one.",
        CHESSPIECE_BEEFALO = "Hey, they got his good side!",
        CHESSPIECE_BEEQUEEN = "Took the sting out of her stinger.",
        CHESSPIECE_BISHOP = {
            GENERIC = "A bishop piece.",
            STRUGGLE = "I don't want to see what comes outta that!",
        },
        CHESSPIECE_BUTTERFLY = "It looks nice, doesn't it?",
        CHESSPIECE_CARRAT = "Now all I can think of is roasted carrots.",
        CHESSPIECE_CATCOON = "Meow.. Meow.. Meow...",
        CHESSPIECE_CLAYHOUND = "It almost looks friendly.",
        CHESSPIECE_CLAYWARG = "It doesn't look so tough now.",
        CHESSPIECE_CRABKING = "Was the treasure worth it?",
        CHESSPIECE_DAYWALKER = "How come jerks always seem to get statues?",
        CHESSPIECE_DEERCLOPS = "We kicked its butt.",
        CHESSPIECE_DEERCLOPS_MUTATED = "Creepy, trembling all over.",
        CHESSPIECE_DRAGONFLY = "I think... I think I understand art.",
        CHESSPIECE_EYEOFTERROR = "Haha, Your soul is mine!",
        CHESSPIECE_FORMAL = "I'm not afraid of that guy. He doesn't even have arms!",
        CHESSPIECE_GUARDIANPHASE3 = "Ugh, I'd be happy never seeing that thing again.",
        CHESSPIECE_HORNUCOPIA = "Ouchhh! I think I chipped a tooth.",
        CHESSPIECE_KITCOON = "These ones are much easier to find.",
        CHESSPIECE_KLAUS = "Haha, you can't get me now.",
        CHESSPIECE_KNIGHT = {
            GENERIC = "A horse with no butt.",
            STRUGGLE = "I don't want to see what comes outta that!",
        },
        CHESSPIECE_MALBATROSS = "She was a pretty tough old bird.",
        CHESSPIECE_MANRABBIT = "Good times.",
        CHESSPIECE_MINOTAUR = "Pfft, I never needed a guardian. And I turned out fine!",
        CHESSPIECE_MOON = "I guess it's nice.",
        CHESSPIECE_MOOSEGOOSE = {
            "Looks as awkward as the real one.",
        },
        CHESSPIECE_MUSE = "Ewww, she doesn't have a head!",
        CHESSPIECE_PAWN = "What sort of peasant doesn't have a torch?!",
        CHESSPIECE_PIPE = "That was never really my thing.",
        CHESSPIECE_ROOK = {
            GENERIC = "That one looks like it could do some damage.",
            STRUGGLE = "I don't want to see what comes outta that!",
        },
        CHESSPIECE_STALKER = "Not so tough now, are you?",
        CHESSPIECE_TOADSTOOL = "What are you looking at?",
        CHESSPIECE_TWINSOFTERROR = "Now they'll always be together.",
        CHESSPIECE_WARG_MUTATED = "Now it won't summon puppies",
        CHESSPIECE_YOTD = "Meaningless reward.",
        CHESTER = "He's so fuzzy!",
        CHESTER_EYEBONE = {
            GENERIC = "It's rude to stare.",
            WAITING = "At least it's not looking at me anymore.",
        },
        CHESTUPGRADE_STACKSIZE = "",
        CHUM = "Ew, fish eat that nasty stuff?",
        CLAYHOUND = {
            GENERIC = "This dog is stone cold!",
            STATUE = "Welp, that's a weird shape for a rock.",
        },
        CLAYWARG = {
            GENERIC = "How'd you get your eyes to do that?!",
            STATUE = "Did someone carve this thing?",
        },
        COLDFIRE = {
            EMBERS = "That fire needs more fuel or it's going to go out.",
            GENERIC = "Sure beats darkness.",
            HIGH = "That fire is getting out of hand!",
            LOW = "The fire's getting a bit low.",
            NORMAL = "Nice and comfy.",
            OUT = "Well, that's over.",
        },
        COLDFIREPIT = {
            EMBERS = "I should put something on the fire before it goes out.",
            GENERIC = "Sure beats darkness.",
            HIGH = "Good thing it's contained!",
            LOW = "The fire's getting a bit low.",
            NORMAL = "Nice and comfy.",
            OUT = "At least I can start it up again.",
        },
        COLLAPSEDCHEST = "",
        COMPASS = {
            E = "East!",
            GENERIC = "Can't get a reading.",
            N = "North!",
            NE = "Northeast!",
            NW = "Northwest!",
            S = "South!",
            SE = "Southeast!",
            SW = "Southwest!",
            W = "West...",
        },
        COMPOST = "Nah, I'm good.",
        COMPOSTINGBIN = {
            BALANCED = "I guess it looks okay? For a pile of rotten dirt?",
            BURNT = "Nice.",
            DRY = "Is it supposed to be that dry?",
            GENERIC = "Ew, why do we have this?",
            WET = "It's all wet and gross.",
        },
        COMPOSTWRAP = "That's disgusting!",
        CONSTRUCTION_PLANS = "I'd rather break things down than build them up.",
        COOKBOOK = "Now i'll never forget a recipe!",
        COOKEDMANDRAKE = "Poor little guy.",
        COOKEDMEAT = "Unseasoned meat... great.",
        COOKEDMONSTERMEAT = "It's still gross.",
        COOKEDSMALLMEAT = "Gonna need a lot of appetizers to survive out here!",
        COOKIECUTTER = "Hey! What are you lookin' at?!",
        COOKIECUTTERHAT = "Matches my prickly personality.",
        COOKIECUTTERSHELL = "Ha, I stole its house.",
        COOKINGRECIPECARD = {
            GENERIC = "It records a lot of delicious food!",
        },
        COOKPOT = {
            BURNT = "The pot got cooked.",
            COOKING_LONG = "This is going to take a while.",
            COOKING_SHORT = "It's almost done!",
            DONE = "Mmmmm! It's ready to eat!",
            EMPTY = "It makes me hungry just to look at it.",
        },
        COONTAIL = "I've kept stranger things than that.",
        CORN = "A sweet vegetable, yum!",
        CORN_COOKED = "Ooh, this one explodes when it goes in fire!",
        CORN_OVERSIZED = "Let's get this thing on the fire!",
        CORN_SEEDS = "Some seeds.",
        COSTUME_BLACKSMITH_BODY = "It's a blacksmith costume.",
        COSTUME_DOLL_BODY = "It's a doll costume.",
        COSTUME_FOOL_BODY = "It's a fool's costume.",
        COSTUME_KING_BODY = "It's a King costume.",
        COSTUME_MIRROR_BODY = "It's a costume.",
        COSTUME_QUEEN_BODY = "It's a Queen costume.",
        COSTUME_TREE_BODY = "It's a tree costume.",
        COTL_TABERNACLE_LEVEL1 = {
            GENERIC = "Aww, you want something to burn, don't you?",
            LIT = "That's better.",
        },
        COTL_TABERNACLE_LEVEL2 = {
            GENERIC = "Aww, you want something to burn, don't you?",
            LIT = "See? Things just look better when they're on fire.",
        },
        COTL_TABERNACLE_LEVEL3 = {
            GENERIC = "Aww, you want something to burn, don't you?",
            LIT = "Flame is stable.",
        },
        COTL_TRINKET = "Finder's keepers.",
        COZY_BUNNYMAN = "They look comfortable.",
        CRABKING = {
            GENERIC = "He doesn't look too happy...",
            INERT = "I think it could use something glittery.",
        },
        CRABKING_CLAW = "Keep your claws off my boat!",
        CRITTERLAB = "Is there anything cute in there??",
        CRITTER_DRAGONLING = "We were made for each other.",
        CRITTER_EYEOFTERROR = "Hey, looking good today!",
        CRITTER_GLOMLING = "Aww, I could just squish your gross lil face!",
        CRITTER_KITTEN = "You're the perfect lapwarmer.",
        CRITTER_LAMB = "Look at those creepy little eyes. Aww.",
        CRITTER_LUNARMOTHLING = "She loves flames!",
        CRITTER_PERDLING = "I could just eat you up.",
        CRITTER_PUPPY = "Hah, you don't even know you're smelly.",
        CROW = {
            GENERIC = "I think it's waiting for me to die.",
            HELD = "Not so smart now, are you?",
        },
        CRUMBS = "Haha. That little guy is losing his parts.",
        CURSED_MONKEY_TOKEN = "I already have a curse more powerful than this. This one has no effect on me.",
        CUTGRASS = "Cut grass, ready for crafting.",
        CUTLESS = "At least it'll make good firewood.",
        CUTLICHEN = "Blah, tastes like sawdust.",
        CUTREEDS = "Cut reeds, ready for crafting.",
        CUTSTONE = "Perfectly squared for maximum enjoyment.",
        DAYWALKER = {
            GENERIC = "You jerk! I was just trying to help!",
            IMPRISONED = "Looks like somebody's having a bad day.",
        },
        DAYWALKER2 = {
            BURIED = "",
            GENERIC = "",
            HOSTILE = "",
        },
        DAYWALKER_PILLAR = {
            EXPOSED = "It won't break. I didn't try that hard, but still.",
            GENERIC = "How crazy would I have to be to try and break that pillar?",
        },
        DEADLYFEAST = "Fire didn't cure this dish of being nasty.",
        DEAD_SEA_BONES = "GROSS! What if I'd stepped on it by accident?",
        DECIDUOUSTREE = {
            BURNING = "What a waste of wood.",
            BURNT = "I feel like I could have prevented that.",
            CHOPPED = "Take that, nature!",
            GENERIC = "It's all leafy. Most of the time.",
            POISON = "It looks unhappy about me stealing those birchnuts!",
        },
        DECOR_CENTERPIECE = "Okay.",
        DECOR_FLOWERVASE = {
            EMPTY = "It's empty. Time to refill.",
            FRESHLIGHT = "Not as good as a fire, but at least we won't be in the dark.",
            GENERIC = "A simple flower arrangement.",
            OLDLIGHT = "We're going to be in the dark soon.",
            WILTED = "Those need replacing.",
        },
        DECOR_LAMP = "A torch works just as well.",
        DECOR_PICTUREFRAME = {
            GENERIC = "It's fine.",
            UNDRAWN = "My burning desire isn't to draw.",
        },
        DECOR_PORTRAITFRAME = "It's missing the fire in their eyes.",
        DEER = {
            ANTLER = "Don't think that horn makes you special, deer.",
            GENERIC = "I wish everyone'd stop fawning over that smelly thing!",
        },
        DEERCLOPS = "Holy crap!",
        DEERCLOPSCORPSE = {
            BURNING = "This is the best way to handle it...",
            GENERIC = "Quick, toss that Deerclops carcass into the fire, don't give them a chance!",
            REVIVING = "That's... not good.",
        },
        DEERCLOPSEYEBALL_SENTRYWARD = {
            GENERIC = "But how will anything burn?",
            NOEYEBALL = "There we go.",
        },
        DEERCLOPSEYEBALL_SENTRYWARD_KIT = "Looks like it's easy to assemble",
        DEERCLOPS_EYEBALL = "Stop staring at me!",
        DEER_ANTLER = "It's a big, weird antler.",
        DEER_GEMMED = "Marginally better smelling than most animals here.",
        DEPLETED_GRASS = {
            GENERIC = "What a sorry looking piece of... grass?",
        },
        DESERTHAT = "Not very stylish.",
        DEVTOOL = "",
        DEVTOOL_NODEV = "",
        DIRTPILE = "Who just leaves dirt lying around in the forest?",
        DIVININGROD = {
            COLD = "Buzz buzz buzz, What sound did you hear?",
            GENERIC = "This thing looks like it's full of electronic waste!",
            HOT = "Ugh! Stop beeping! It's so noisy.",
            WARM = "This thing is getting noisier and noisier.",
            WARMER = "It feels like we're almost there! What kind of treasure could it be?",
        },
        DIVININGRODBASE = {
            GENERIC = "Not sure what this does. Doesn't seem like it's fiery things.",
            READY = "Just needs to be unlocked with a key. Not fire, unfortunately.",
            UNLOCKED = "It's whirring now!",
        },
        DIVININGRODSTART = "I'll make something out of it.",
        DOCK_KIT = "Everything I need to build a dock for my boat.",
        DOCK_WOODPOSTS_ITEM = "Aha! I thought the dock was missing something.",
        DOLL_BLUEPRINT = "Awww, my beefalo's going to be so hideous and adorable!",
        DRAGONBOAT_KIT = "\"Dragon Boat\"",
        DRAGONBOAT_PACK = "\"Dragon Boat\" Kit",
        DRAGONBODYHAT = "I'm not sure I want to be in *another* belly.",
        DRAGONCHILISALAD = "I can't wait to eat it.",
        DRAGONFLY = "Oh, don't you just drag on.",
        DRAGONFLYCHEST = "Chest is not afraid of fire.",
        DRAGONFLYFURNACE = {
            GENERIC = "Let's turn it back on!",
            HAMMERED = "That's certainly a look.",
            HIGH = "I think I'm in love.",
            NORMAL = "Stick another gem in there!",
        },
        DRAGONFRUIT = "What a weird fruit.",
        DRAGONFRUIT_COOKED = "The fruit's still weird.",
        DRAGONFRUIT_OVERSIZED = "I wish it was a fire-breathing Dragonfruit.",
        DRAGONFRUIT_SEEDS = "It's a seed.",
        DRAGONHEADHAT = "The front part's sort of scary.",
        DRAGONPIE = "The dragonfruit is very filling.",
        DRAGONTAILHAT = "I like the tail!",
        DRAGON_SCALES = "They don't weigh much for scales.",
        DREADSTONE = "Eww, why does this rock feel so cold and clammy?",
        DREADSTONEHAT = "Do I have to wear it? On my head??",
        DREADSTONE_STACK = "It's growing from way down below.",
        DRIFTWOOD_LOG = "Dry wood. Good utility material.",
        DRIFTWOOD_TREE = {
            BURNING = "That driftwood's burning!",
            BURNT = "Well, that was fun.",
            CHOPPED = "There might still be something worth digging up.",
            GENERIC = "This tree looks super weird.",
        },
        DRUMSTICK = "Bang on the drum all day!",
        DRUMSTICK_COOKED = "Hmm... Satisfy hunger, or bang on the drum?",
        DUG_BANANABUSH = "That bush is bananas!",
        DUG_BERRYBUSH = "What's the matter, got no dirt?",
        DUG_BERRYBUSH_JUICY = "What's the matter, got no dirt?",
        DUG_GRASS = "What's the matter, got no dirt?",
        DUG_MARSH_BUSH = "What's the matter, got no dirt?",
        DUG_MONKEYTAIL = "It's so soft. And fuzzy.",
        DUG_SAPLING = "What's the matter, got no dirt?",
        DUG_TRAP_STARFISH = "I knew I didn't like you.",
        DUMBBELL = "Training gear should stay practical.",
        DUMBBELL_BLUEGEM = "Ugh! Why is it so cold?",
        DUMBBELL_GEM = "Did he really need to make it so fancy?",
        DUMBBELL_GOLDEN = "Did he really need to make it so fancy?",
        DUMBBELL_HEAT = "Let's get you all fired up for your workout, big guy!",
        DUMBBELL_MARBLE = "Did he really need to make it so fancy?",
        DUMBBELL_REDGEM = "Solid resistance for training.",
        DURIAN = "What a weird fruit.",
        DURIAN_COOKED = "The fruit's still weird.",
        DURIAN_OVERSIZED = "Of course THIS is the one that grows well...",
        DURIAN_SEEDS = "Some seeds.",
        DUSTMERINGUE = "Yeah, I don't think I'm THAT hungry.",
        DUSTMOTH = "They keep things clean all day. Diligent workers.",
        DUSTMOTHDEN = "Not that I tried or anything.",
        EARMUFFSHAT = "Smells like rabbit butt.",
        EEL = "This will make a delicious meal.",
        EEL_COOKED = "Smells great!",
        EGGPLANT = "Definitely not a bird.",
        EGGPLANT_COOKED = "Using fire on it made it a tastier eggplant.",
        EGGPLANT_OVERSIZED = "What am I supposed to do with all this eggplant?!",
        EGGPLANT_SEEDS = "Some seeds.",
        EMBERLIGHT = "How did it catch fire without any combustible material to support it?",
        ENDTABLE = {
            BURNT = "Alright, alright, so I was wrong about the hand!",
            EMPTY = "I'm telling you, there's a monsterhand under that thing!",
            FRESHLIGHT = "It's nice to have a little light.",
            GENERIC = "A neat bunch of flowers.",
            OLDLIGHT = "We're going to be in the dark soon.",
            WILTED = "Those need replacing.",
        },
        EVERGREEN = {
            BURNING = "What a waste of wood.",
            BURNT = "I feel like I could have prevented that.",
            CHOPPED = "Take that, nature!",
            GENERIC = "It's all piney.",
        },
        EVERGREEN_SPARSE = {
            BURNING = "What a waste of wood.",
            BURNT = "I feel like I could have prevented that.",
            CHOPPED = "Take that, nature!",
            GENERIC = "This sad tree has no cones.",
        },
        EYEBRELLAHAT = "Keeps the rain out your eyes and in someone else's.",
        EYEMASKHAT = "Ugly.",
        EYEOFTERROR = "Come! I'll Cut you down!",
        EYEOFTERROR_MINI = "I'm starting to feel self-conscious.",
        EYEOFTERROR_MINI_GROUNDED = "I think it's about to hatch...",
        EYEPLANT = "I think I'm being watched.",
        EYETURRET = "I hope it doesn't turn on me.",
        EYETURRET_ITEM = "It's no good until it's been placed.",
        FARMPLOT = {
            BURNT = "It was a nice finale.",
            GENERIC = "Sigh. It's a pile of dirt.",
            GROWING = "Hurry up, dirtpile. Feed me!",
            NEEDSFERTILIZER = "Stupid thing needs poop.",
        },
        FARM_HOE = "Farming is so much work...",
        FARM_PLANT = {
            BURNING = "Heh heh.",
            FULL = "Took long enough.",
            FULL_OVERSIZED = "Well that looks kind of unnatural.",
            FULL_WEED = "Wait, what kind of vegetable is that supposed to be?",
            GENERIC = "A plant. Track its growth.",
            GROWING = "Grow faster already!",
            ROTTEN = "Yeah... I'm not eating that.",
            ROTTEN_OVERSIZED = "A big pile of stinky mush.",
            SEED = "Ugh, this is going to take ages.",
        },
        FARM_PLOW = "Turn the soil evenly and cleanly.",
        FARM_PLOW_ITEM = "I should choose a suitable plot for planting.",
        FARM_SOIL_DEBRIS = "Get outta here!",
        FEATHERFAN = "I do not know... it could put out some...",
        FEATHERHAT = "A phoenix reborn!",
        FEATHERPENCIL = "Haha! It tickles!",
        FEATHER_CANARY = "Feather of yellow.",
        FEATHER_CROW = "Feather of black.",
        FEATHER_ROBIN = "Feather of red.",
        FEATHER_ROBIN_WINTER = "Feather of white.",
        FEM_PUPPET = "She looks scared half to death.",
        FENCE = "I'm not painting that.",
        FENCE_GATE = "I guess we could pen stuff in with it.",
        FENCE_GATE_ITEM = "No point just leaving it on the ground.",
        FENCE_ITEM = "No point just leaving it on the ground.",
        FENCE_JUNK = "",
        FENCE_ROTATOR = "Enguard! Re-post!",
        FERTILIZER = "A bucket full of nasty.",
        FESTIVEFISH = "It's just so festive!",
        FESTIVE_BLUEPRINT = "It needs more firecrackers before I'd call it \"festive\".",
        FIG = "Looks like old people fruit.",
        FIGATONI = "I won't turn down a plate of pasta.",
        FIGKABAB = "When I'm done with the food I can burn the stick!",
        FIG_COOKED = "As usual, fire made it better.",
        FIREBALLSTAFF = "It calls a meteor from above.",
        FIRECRACKERS = "Don't worry Ms. Wicker, we'll be careful.",
        FIREFLIES = {
            GENERIC = "I wish they didn't run away!",
            HELD = "They make my pocket glow!",
        },
        FIREHOUND = "I actually kind of like that one.",
        FIRENETTLES = "What do you mean it's a weed? Looks like an improvement to me!",
        FIREPEN = "Hey, I'm feeling really inspired to write!",
        FIREPIT = {
            EMBERS = "I should put something on the fire before it goes out.",
            GENERIC = "Sure beats darkness.",
            HIGH = "Good thing it's contained!",
            LOW = "The fire's getting a bit low.",
            NORMAL = "Nice and comfy.",
            OUT = "At least I can start it up again.",
        },
        FIRESTAFF = "That's my absolute favorite toy.",
        FIRESUPPRESSOR = {
            LOWFUEL = "Running low on fuel.",
            OFF = "It's off.",
            ON = "This would come in handy in a snowball fight.",
        },
        FISH = "Slippery fishy!",
        FISHINGNET = "I love throwing things to catch other things!",
        FISHINGROD = "Fishing for the answer with a hook, line and sinker.",
        FISHMEAT = "Once a fish, now food.",
        FISHMEAT_COOKED = "Grilled to perfection.",
        FISHMEAT_SMALL = "Not much more than a quick fishy snack.",
        FISHMEAT_SMALL_COOKED = "Fish and fire go well together.",
        FISHSTICKS = "What you see is what you get. Sticks of fish.",
        FISHTACOS = "Convenient taco-grip.",
        FISH_BOX = "Did we just... put a hole in the boat?",
        FISH_COOKED = "Flaky and moist. Delicious.",
        FLINT = "Aha, I've found some flint!",
        FLOWER = {
            GENERIC = "I don't have time to waste on flowers.",
            ROSE = "Bright red petals!",
        },
        FLOWERHAT = "A halo of flowers. Simple and tidy.",
        FLOWERSALAD = "I'd rather a bowl of flames.",
        FLOWER_CAVE = "It's burning inside.",
        FLOWER_EVIL = "Ugh, that smells terrible.",
        FLOWER_WITHERED = "Looks like good kindling.",
        FOLIAGE = "Soft and leafy.",
        FOOTBALLHAT = "Sports are hard.",
        FORGETMELOTS = "More flowers. Nice color balance.",
        FORMAL_BLUEPRINT = "Ugh, it looks so stuffy and proper.",
        FOSSIL_PIECE = "It's just nasty old bits of bones!",
        FOSSIL_STALKER = {
            COMPLETE = "Looks passable.",
            FUNNY = "That looks absolutely ridiculous.",
            GENERIC = "Needs more gross old bits.",
        },
        FRESHFRUITCREPES = "Ooo la la.",
        FRIENDLYFRUITFLY = "Hey, this one's actually helping!",
        FROG = {
            DEAD = "A hop too far.",
            GENERIC = "Ob.. Ob...",
            SLEEPING = "Oblivious amphibious.",
        },
        FROGFISHBOWL = "Woah, that's a ton of frogs.",
        FROGGLEBUNWICH = "If you close your eyes it's a little easier to get it down.",
        FROGLEGS = "It still twitches every now and then. Freaky.",
        FROGLEGS_COOKED = "Tastes like chicken.",
        FROGNEWTON = "All that fig almost masks the frog flavor.",
        FROZENBANANADAIQUIRI = "Ew, it's cold.",
        FRUITDRAGON = {
            GENERIC = "They're pretty cute.",
            RIPE = "Mmm... Roast dragonfruit.",
            SLEEPING = "Good night, little guy.",
        },
        FRUITFLY = "Please stay out of the crops.",
        FRUITFLYFRUIT = "Now I'm the the one giving the orders! To Fruit Flies, but still.",
        FRUITMEDLEY = "Yum, fruit!",
        FURTUFT = "Black and white and fuzzy all over!",
        FUSED_SHADELING = "Eww, it's so creepy!",
        FUSED_SHADELING_BOMB = "They sure go out with a bang.",
        GARGOYLE_HOUND = "A large stone ornament.",
        GARGOYLE_WEREPIG = "At least it doesn't smell anymore.",
        GARLIC = "It's just a couple cloves of garlic.",
        GARLIC_COOKED = "Fire made it better.",
        GARLIC_OVERSIZED = "I bet my breath would smell great if I ate the whole thing.",
        GARLIC_SEEDS = "It's just a bunch of seeds.",
        GAZPACHO = "I prefer hot food.",
        GEARS = "These must make them move.",
        GEMSOCKET = {
            GEMS = "It's so empty!",
            VALID = "I wonder how they hover?",
        },
        GESTALT = "It feels like they're speaking inside my head!",
        GESTALT_GUARD = "They sure hate those shadow creeps.",
        GHOST = "Resentment wakes it up. \n You can't kill what's already dead.",
        GHOSTFLOWER = "Great. Another spooky flower.",
        GHOSTLYELIXIR_ATTACK = "I not trust tiny bottles made by creepy girl.",
        GHOSTLYELIXIR_FASTREGEN = "I not trust tiny bottles made by creepy girl.",
        GHOSTLYELIXIR_RETALIATION = "I not trust tiny bottles made by creepy girl.",
        GHOSTLYELIXIR_SHIELD = "I not trust tiny bottles made by creepy girl.",
        GHOSTLYELIXIR_SLOWREGEN = "I not trust tiny bottles made by creepy girl.",
        GHOSTLYELIXIR_SPEED = "I not trust tiny bottles made by creepy girl.",
        GHOST_KILL = "Die again and you're free",
        GIFT = "It's for me right?!",
        GIFTWRAP = "I want to give someone something nice!",
        GINGERBREADHOUSE = "Let's eat it!",
        GINGERBREADPIG = "Hey get back here!",
        GINGERBREADWARG = "Desserts should not eat people!",
        GLASSBLOCK = "It's a big hunk of glass.",
        GLASSCUTTER = "Haha, this thing could take an arm off!",
        GLASSSPIKE = "That's a hazardous decoration.",
        GLOMMER = {
            GENERIC = "It's fuzzy! And slimy...",
            SLEEPING = "Alright, I guess it's kind of cute.",
        },
        GLOMMERFLOWER = {
            DEAD = "It smells worse now.",
            GENERIC = "It doesn't smell very nice.",
        },
        GLOMMERFUEL = "It's goopy and weird.",
        GLOMMERWINGS = "They're so small!",
        GLOWBERRYMOUSSE = "That's definitely going in my mouth.",
        GNARWAIL = {
            BROKENHORN = "Not so tough without your horn, are you?",
            BROKENHORN_FOLLOWER = "You look off-balance without your horn.",
            FOLLOWER = "Yeah, that's right! You work for ME now!",
            GENERIC = "Geez, look at the nose on that thing!",
        },
        GNARWAIL_HORN = "Ha ha, cool.",
        GOATMILK = "It's fuzzy with electricity. Yuck.",
        GOGGLESHAT = "What a great look!",
        GOLDENAXE = "Life is okay when you've got a golden axe.",
        GOLDENPICKAXE = "The finest pickaxe of them all.",
        GOLDENPITCHFORK = "We can do some fancy forking with this.",
        GOLDENSHOVEL = "We're going to dig so many holes.",
        GOLDEN_FARM_HOE = "Fancy garden hoe good for planting seeds!",
        GOLDNUGGET = "What should we spend it on??",
        GOOSE_FEATHER = "So snuggly!",
        GRASS = {
            BARREN = "It needs poop.",
            BURNING = "Fire! Wooo!",
            DISEASED = "Disease detected. Remove and contain.",
            DISEASING = "Blech. You smell.",
            GENERIC = "It's a grass.",
            PICKED = "Grass stubble is kind of useless.",
            WITHERED = "Nice, dry.",
        },
        GRASSGATOR = "E'Hia..., Ha.. Ha.. Ha...",
        GRASSGEKKO = {
            DISEASED = "Double ew!",
            GENERIC = "Kapom?",
        },
        GRASS_UMBRELLA = "Pretty as can be!",
        GRAVY = "Yeah! Pour on the gravy!",
        GREENAMULET = "I feel my mind opening when I wear it.",
        GREENGEM = "This one feels really light.",
        GREENMOONEYE = "A large green stone.",
        GREENSTAFF = "This will come in handy.",
        GREEN_CAP = "Plain, but useful.",
        GREEN_CAP_COOKED = "It's been transformed by fire!",
        GREEN_MUSHROOM = {
            GENERIC = "Green mushroom specimen.",
            INGROUND = "Hey! You! Get up here!",
            PICKED = "Maybe it will return some day.",
        },
        GROTTO_POOL_BIG = "Ugh, this whole place is damp and gross! I hate it!",
        GROTTO_POOL_SMALL = "Ugh, this whole place is damp and gross! I hate it!",
        GUACAMOLE = "Holy moley, that's tasty!",
        GUANO = "Ew. Nasty. But useful.",
        GUNPOWDER = "It looks like pepper.",
        HALLOWEENCANDY_1 = "Oh, Wendy! I'll trade you for your choco pigs!",
        HALLOWEENCANDY_10 = "Candy candy candy!",
        HALLOWEENCANDY_11 = "Mmm! Sweet revenge!",
        HALLOWEENCANDY_12 = "Wriggly, yet satisfying.",
        HALLOWEENCANDY_13 = "I like these a lot.",
        HALLOWEENCANDY_14 = "Gosh, it's spicy.",
        HALLOWEENCANDY_2 = "Haha ew! It's weird!",
        HALLOWEENCANDY_3 = "Haha, that's not candy!",
        HALLOWEENCANDY_4 = "We aren't totally comfortable with this.",
        HALLOWEENCANDY_5 = "I forgot what good things tasted like!",
        HALLOWEENCANDY_6 = "No worse than the other stuff we've eaten out here!",
        HALLOWEENCANDY_7 = "Oh, Ms. Wicker! I saved these for you!",
        HALLOWEENCANDY_8 = "Candy candy candy!",
        HALLOWEENCANDY_9 = "Gummy worms, yummy worms!",
        HALLOWEENPOTION_BRAVERY = "Makes us feel big and strong!",
        HALLOWEENPOTION_DRINKS_POTENT = "It's pretty powerful.",
        HALLOWEENPOTION_DRINKS_WEAK = "It's only a little powerful.",
        HALLOWEENPOTION_FIRE_FX = "Neat! It's like firecrackers.",
        HALLOWEENPOTION_MOON = "Should I drink it? Nah, probably not.",
        HALLOWEEN_ORNAMENT_1 = "Oooh. Spooky. I should decorate!",
        HALLOWEEN_ORNAMENT_2 = "Gosh. That almost looks real.",
        HALLOWEEN_ORNAMENT_3 = "Aw. It's not real. Let's hang it somewhere.",
        HALLOWEEN_ORNAMENT_4 = "Let's hang it somewhere for Halloween!",
        HALLOWEEN_ORNAMENT_5 = "I should put this guy in a tree!",
        HALLOWEEN_ORNAMENT_6 = "If I put it in a tree it'd look almost real.",
        HAMBAT = "This seems unsanitary.",
        HAMMER = "Stop! It's time! To hammer things!",
        HAMMER_MJOLNIR = "That weapon looks plain, but solid.",
        HANDPILLOW_BEEFALOWOOL = "Bernie's cuddlier than a hairy old pillow.",
        HANDPILLOW_KELP = "Ew, this pillow's full of wet junk!",
        HANDPILLOW_PETALS = "I guess that's one use for flowers.",
        HANDPILLOW_STEELWOOL = "The worst pillow ever. I can't wait to hit someone with it!",
        HARAKIRI = "For Honor!",
        HAREBALL = "Haha, nasty! Wait, I'm not the one who has to eat it, am I?",
        HAWAIIANSHIRT = "Grandpa's style, definitely.",
        HEALINGSALVE = "The stinging means that it's working.",
        HEALINGSALVE_ACID = "",
        HEALINGSTAFF = "I could take a crack at it.",
        HEATROCK = {
            COLD = "That's a cold stone.",
            FROZEN = "It's colder than ice.",
            GENERIC = "I could manipulate its temperature.",
            HOT = "Nice and toasty hot!",
            WARM = "It's quite warm and cuddly... for a rock!",
        },
        HEDGEHOUND = "It's an ambush!",
        HEDGEHOUND_BUSH = "It's a bush.",
        HERMITCRAB = "Heh. I rather like her.",
        HERMITHOUSE = {
            BUILTUP = "I never needed a home, but some people do I guess.",
            GENERIC = "What a dump.",
        },
        HERMIT_BUNDLE = " I was happy to help old lady!",
        HERMIT_BUNDLE_SHELLS = "I like the plink-plonk shells.",
        HERMIT_CRACKED_PEARL = "Oops...",
        HERMIT_PEARL = "Don't worry grams, it's safe with me!",
        HITOKIRI = "It's going to make me mad.",
        HIVEHAT = "That bee's head looks rather tasty, doesn't it?",
        HMINGOT = "it's hammer time!!",
        HOME = "Someone must live here.",
        HOMESIGN = {
            BURNT = "\"Don't play with matches.\"",
            GENERIC = "It says \"You are here\".",
            UNWRITTEN = "The sign is currently blank.",
        },
        HONEY = "Sweet and delicious!",
        HONEYCOMB = "It's waxy.",
        HONEYHAM = "Ham and honey go well together.",
        HONEYNUGGETS = "Honey-covered morsels.",
        HORN = "I can hear those hairy beasts inside.",
        HORRORFUEL = "That must be what nightmares have nightmares about.",
        HOTCHILI = "Now that's my kind of heat!",
        HOTSPRING = {
            BOMBED = "The bombs are fun to throw in.",
            EMPTY = "I'll just have to wait for it to fill up again.",
            GENERIC = "So toasty and warm!",
            GLASS = "Should we smash it up?",
        },
        HOUND = "It just a dog",
        HOUNDBONE = "Gross.",
        HOUNDCORPSE = {
            BURNING = "The best way to solve any problem.",
            GENERIC = "So... now what do we do with it?",
            REVIVING = "BURN ITTTT!",
        },
        HOUNDMOUND = "Uh oh, I don't like the look of that.",
        HOUNDSTOOTH = "It's sharp. I like it!",
        HOUNDSTOOTH_BLOWPIPE = "If only it shot flaming teeth.",
        HOUNDWHISTLE = "Heh heh. STAY.",
        HUMANMEAT = "Man eat man......will go down in history.",
        HUMANMEAT_COOKED = "I will never accept this kind of behavior, no matter what.",
        HUMANMEAT_DRIED = "I'm not in that era, nor in that place, I would never touch it.",
        HUTCH = "What's your angle?",
        HUTCH_FISHBOWL = {
            GENERIC = "It's too wet to burn.",
            WAITING = "It's still too wet to burn.",
        },
        ICE = "Chilly.",
        ICEBERG = {
            GENERIC = "That's a lot of ice.",
            MELTED = "It looks pretty melted.",
        },
        ICEBERG_MELTED = "It looks pretty melted.",
        ICEBOX = "I'm all for making things last longer.",
        ICECREAM = "Well, soooometimes cold things are okay.",
        ICEHAT = "It's a real mood dampener.",
        ICEHOUND = "Gross! Cold!",
        ICEPACK = "Fuzzy backpack!",
        ICESTAFF = "Booooring.",
        ICE_BLUEPRINT = "Don't beefalo already have hairy winter coats?",
        INSANITYROCK = {
            ACTIVE = "Am I seeing things?",
            INACTIVE = "I wonder what that does.",
        },
        INSPECTSELF = "Am I still in one piece?",
        IPECACSYRUP = "Eww, the smell's enough to make you sick!",
        IVY_SNARE = "Clear this weed safely.",
        JAMMYPRESERVES = "Wham, bam, jam! Thank you, ma'am.",
        JELLYBEAN = "Nothing better than a handful of jellybeans.",
        JUNK_PILE = "",
        JUNK_PILE_BIG = "",
        JUSTEGGS = "Eggs and more eggs.",
        KABOBS = "Foods on a stick!",
        KAGE = "At the end of its blueprint it reads:\n\"We are dust and shadows\"",
        KATANABLADE = "It has no hilt and will be uncomfortable to use.",
        KATANABODY = "Wrapped with rope and sharpened.",
        KELP = "HEY! Who dares me to eat it raw?",
        KELPHAT = "Yuck, it feels like snot.",
        KELP_COOKED = "Great, now I don't have to chew.",
        KELP_DRIED = "It's not half bad.",
        KILLERBEE = {
            GENERIC = "I like the cut of that bee's jib.",
            HELD = "Buzz!",
        },
        KITCOONDECOR1 = {
            BURNT = "Combustion!",
            GENERIC = "It's not a real bird, but the kits don't know that.",
        },
        KITCOONDECOR1_KIT = "It looks like there's some assembly required.",
        KITCOONDECOR2 = {
            BURNT = "It went up in flames.",
            GENERIC = "Those kits are so easily distracted. Now what was I doing again?",
        },
        KITCOONDECOR2_KIT = "It doesn't look too hard to build.",
        KITCOONDEN = {
            BURNT = "NOOOO!",
            GENERIC = "You'd have to be pretty small to fit in there.",
            PLAYING_HIDEANDSEEK = "Now where could they be...",
            PLAYING_HIDEANDSEEK_TIME_ALMOST_UP = "Not much time left! Where are they?!",
        },
        KITCOONDEN_KIT = "The whole kit and caboodle.",
        KITCOON_DECIDUOUS = "Meow.. Meow.. Meow...",
        KITCOON_DESERT = "Meow.. Meow.. Meow...",
        KITCOON_FOREST = "Meow.. Meow.. Meow...",
        KITCOON_GRASS = "Meow.. Meow.. Meow...",
        KITCOON_MARSH = "Meow.. Meow.. Meow...",
        KITCOON_MOON = "Meow.. Meow.. Meow...",
        KITCOON_NAMETAG = "I should think of some names!",
        KITCOON_ROCKY = "Meow.. Meow.. Meow...",
        KITCOON_SAVANNA = "Meow.. Meow.. Meow...",
        KITCOON_YOT = "Meow.. Meow.. Meow...",
        KLAUS = "A lump of coal would be really useful, actually!",
        KLAUSSACKKEY = "Ha! I wouldn't want to get smacked with that thing!",
        KLAUS_SACK = "Nothing says \"Open Me\" quite like a lock!!",
        KNIGHT = "It's a pony!",
        KNIGHT_NIGHTMARE = "It looks pretty worn down.",
        KOALEFANT_CARCASS = "it was killed by other beasts.",
        KOALEFANT_SUMMER = "Chang.. Chang.. Chang...",
        KOALEFANT_WINTER = "Chang.. Chang.. Chang...",
        KOALEFIG_TRUNK = "Sweet and rubbery.",
        KOSHIRAE = "Size doesn't matter.",
        KRAMPUS = "Stay back, you big one!",
        KRAMPUS_SACK = "The holidays came early this year!",
        LANTERN = "A more civilized light.",
        LATKES = "No skimping on the sour cream!",
        LAVAARENA_ARMOREXTRAHEAVY = "Nothing's going to get through that!",
        LAVAARENA_ARMORHEAVY = "That armor looks pretty safe.",
        LAVAARENA_ARMORLIGHT = "This armor isn't very good.",
        LAVAARENA_ARMORLIGHTSPEED = "It barely does anything.",
        LAVAARENA_ARMORMEDIUM = "This armor will stop a blow or two.",
        LAVAARENA_ARMORMEDIUMDAMAGER = "Good for smackin' stuff.",
        LAVAARENA_ARMORMEDIUMRECHARGER = "I could do neat stuff more often with that.",
        LAVAARENA_ARMOR_HP = "Woohoo! I should put that on.",
        LAVAARENA_BATTLESTANDARD = "Kill that flag!",
        LAVAARENA_BERNIE = "You're always there for me, Bernie.",
        LAVAARENA_BOARLORD = "Pfft, I bet he can't even fight.",
        LAVAARENA_CROWNDAMAGERHAT = "Dibs!",
        LAVAARENA_ELEMENTAL = "Woah! I love it!",
        LAVAARENA_EYECIRCLETHAT = "Ooo, give me!",
        LAVAARENA_FEATHERCROWNHAT = "It's a feather crown!",
        LAVAARENA_FIREBOMB = "FIRE! FIRE! FIRE!",
        LAVAARENA_HEALINGFLOWERHAT = "That's a great wreath!",
        LAVAARENA_HEALINGGARLANDHAT = "It makes you feel a little better when you wear it.",
        LAVAARENA_HEAVYBLADE = "No way I can use that!",
        LAVAARENA_KEYHOLE = "There's no key in it.",
        LAVAARENA_KEYHOLE_FULL = "Looks good.",
        LAVAARENA_LIGHTDAMAGERHAT = "So pointy!",
        LAVAARENA_LUCY = "That's Woodie's axe.",
        LAVAARENA_PORTAL = {
            GENERIC = "This is WAY better than our gate back home!",
            ON = "Time to go.",
        },
        LAVAARENA_RECHARGERHAT = "Ohh, I want it!",
        LAVAARENA_SPAWNER = "Hey! come out of it.",
        LAVAARENA_STRONGDAMAGERHAT = "I'd wear it if I felt like hitting stuff.",
        LAVAARENA_TIARAFLOWERPETALSHAT = "Comes with a matching staff!",
        LAVAE = "Why can't we be friends?",
        LAVAE_COCOON = "Aw, it lost its fiery personality.",
        LAVAE_EGG = {
            GENERIC = "There's a faint warmth coming from inside.",
        },
        LAVAE_EGG_CRACKED = {
            COLD = "It has a chill.",
            COMFY = "That egg looks happy.",
        },
        LAVAE_PET = {
            CONTENT = "My own little fire companion.",
            GENERIC = "She's... perfect.",
            HUNGRY = "I think she wants some burnings.",
            STARVING = "I can see her ribs!",
        },
        LAVAE_TOOTH = "I hope she's not a biter.",
        LAVASPIT = {
            COOL = "Cool drool, literally.",
            HOT = "The coolest drool!",
        },
        LAVA_POND = "Yes! Yessss!",
        LAVA_POND_ROCK = "Oh great, another stinkin' rock!",
        LEAFLOAF = "Uh, is it supposed to be green?",
        LEAFYMEATBURGER = "Does this burger taste weird to anyone else?",
        LEAFYMEATSOUFFLE = "What kind of monster would combine dessert with vegetables?",
        LEIF = "Where did that come from?!",
        LEIF_IDOL = "See how fun it is to burn trees? I knew he'd come around.",
        LEIF_SPARSE = "Where did that come from?!",
        LICHEN = "A gross, crusty plant.",
        LIFEINJECTOR = "Don't you dare stick that in me!",
        LIGHTBULB = "It's called a \"lightbulb\" but it's rather heavy.",
        LIGHTCRAB = "It's like there's a little fire burning inside.",
        LIGHTER = "Playing with fire will make you wet the bed.",
        LIGHTFLIER = "I'd rather use fire to light my way.",
        LIGHTNINGGOAT = {
            CHARGED = "You're crazy!",
            GENERIC = "Bouncy goat.",
        },
        LIGHTNINGGOATHORN = "I heard lightning when I held it to my ear.",
        LIGHTNING_ROD = {
            CHARGED = "It looks all glowy!",
            GENERIC = "All the lightning goes here!",
        },
        LITTLE_WALRUS = "He looks tasty.",
        LIVINGLOG = "It looks upset.",
        LIVINGTREE = "It might be alive.",
        LIVINGTREE_ROOT = "I should plant this somewhere.",
        LIVINGTREE_SAPLING = "It's a monster kid. Like me!",
        LOBSTERBISQUE = "I'm going to eat this entire thing!",
        LOBSTERDINNER = "Yummmmmmm!",
        LOG = {
            BURNING = "Log is burning steadily.",
            GENERIC = "That's a log of wood.",
        },
        LORDFRUITFLY = "You think you can barge in here and mess up the garden? That's my job!",
        LOST_TOY_1 = "We think those might belong to someone else.",
        LOST_TOY_10 = "We think those might belong to someone else.",
        LOST_TOY_11 = "We think those might belong to someone else.",
        LOST_TOY_14 = "We think those might belong to someone else.",
        LOST_TOY_18 = "We think those might belong to someone else.",
        LOST_TOY_19 = "We think those might belong to someone else.",
        LOST_TOY_2 = "We think those might belong to someone else.",
        LOST_TOY_42 = "We think those might belong to someone else.",
        LOST_TOY_43 = "We think those might belong to someone else.",
        LOST_TOY_7 = "We think those might belong to someone else.",
        LUCKY_GOLDNUGGET = "So shiny!",
        LUCY = "We could be good friends, you and I.",
        LUMPY_SAPLING = "I should burn that thing before it reaches full ugliness.",
        LUNARFROG = "Ew, ew, ew!! Too many eyes!!",
        LUNARPLANTHAT = "How am I supposed to see in that thing?",
        LUNARPLANT_HUSK = "Can we clear this safely now?",
        LUNARPLANT_KIT = "Do I look like the kind of person who fixes things? Ugh, fiiiiine...",
        LUNARRIFT_CRYSTAL = "I like the way it glitters.",
        LUNARRIFT_PORTAL = "Uh... is that supposed to be here?",
        LUNARTHRALL_PLANT = "Ew, that thing's creepy!",
        LUNARTHRALL_PLANT_VINE_END = "It's asking for a taste of my lighter.",
        LUNAR_FORGE = "What gives? That little fire doesn't even feel warm!",
        LUNAR_FORGE_KIT = "Right. Now I guess I have to build it.",
        LUNAR_GRAZER = "Ew, it's dripping its weird sleep gunk everywhere!",
        LUREPLANT = "What a colorful plant.",
        LUREPLANTBULB = "Gross! It's so meaty!",
        LUTEFISK = "It actually looks pretty tasty.",
        MADSCIENCE_LAB = "Wow. Look at it bubble.",
        MAGICIAN_CHEST = "What's in it? Do you have anything delicious?",
        MAID_HB = "Hmmm...",
        MALBATROSS = "Uh... nice bird?",
        MALBATROSS_BEAK = "Nasty.",
        MALBATROSS_FEATHER = "Ha! That thing was just a featherweight after all.",
        MALBATROSS_FEATHERED_WEAVE = "That took way too much effort.",
        MALE_PUPPET = "He looks scared half to death.",
        MANDRAKE = "Why did this plant have a face?",
        MANDRAKESOUP = "The face doesn't wash away!!",
        MANDRAKE_ACTIVE = "Now that's just creepy!",
        MANDRAKE_COOKED = "The meeping had to stop.",
        MANDRAKE_PLANTED = "That's not a normal plant.",
        MANRABBIT_TAIL = "The rabbits lost that argument.",
        MAPSCROLL = "Nothing's on it?",
        MARBLE = "It's heavy!",
        MARBLEBEAN = "I guess we just... plant it? In the dirt?",
        MARBLEBEAN_SAPLING = "That makes no sense!",
        MARBLEPILLAR = "I wonder if the rest burnt down.",
        MARBLESHRUB = "What sort of bush doesn't burn?!",
        MARBLETREE = "I hope it doesn't fall on me.",
        MARSH_BUSH = {
            BURNING = "It will be gone soon!",
            BURNT = "That burned way too quickly.",
            GENERIC = "It looks sharp.",
            PICKED = "Those thorns hurt!",
        },
        MARSH_PLANT = "It's all plant-y.",
        MARSH_TREE = {
            BURNING = "It's extra dangerous now!",
            BURNT = "I wish it was still burning.",
            CHOPPED = "Not so spiky now, are you?",
            GENERIC = "It looks dangerous!",
        },
        MASHEDPOTATOES = "I LOVE mashed potatoes.",
        MASK_BLACKSMITHHAT = "It's a blacksmith mask.",
        MASK_DOLLBROKENHAT = "It's a cracked doll mask.",
        MASK_DOLLHAT = "It's a doll mask.",
        MASK_DOLLREPAIREDHAT = "It's was a doll mask at one point.",
        MASK_FOOLHAT = "It's a fool's mask.",
        MASK_KINGHAT = "It's a King mask.",
        MASK_MIRRORHAT = "It's a mask, but it looks like a mirror.",
        MASK_QUEENHAT = "It's a Queen mask.",
        MASK_TREEHAT = "It's a tree mask.",
        MAST = "Are we allowed to climb it?",
        MASTUPGRADE_LAMP_ITEM = "I'm full of bright ideas.",
        MASTUPGRADE_LAMP_ITEM_YOTD = "It does lighten the mood considerably",
        MASTUPGRADE_LAMP_YOTD = "It does lighten the mood considerably",
        MASTUPGRADE_LIGHTNINGROD_ITEM = "That's where the lightning goes.",
        MAST_ITEM = "You can't call it \"sailing\" if you don't have a sail.",
        MAST_MALBATROSS = "Spread my wings and sail away!",
        MAST_MALBATROSS_ITEM = "You can really make a sail out of anything.",
        MAST_YOTD = "No different from ordinary ones",
        MAST_YOTD_ITEM = "No different from ordinary ones",
        MAXWELL = "He's so condescending.",
        MAXWELLHEAD = "He sure likes to talk.",
        MAXWELLLIGHT = "These are no fun. They light themselves.",
        MAXWELLLOCK = "It just needs a key.",
        MAXWELLPHONOGRAPH = "I like more exciting music.",
        MAXWELLTHRONE = "\"Theocratic Throne\"and it looks sticky.",
        MEAT = "This could be made better with FIRE!",
        MEATBALLS = "Meat made into spheres and improved with fire.",
        MEATRACK = {
            BURNT = "The rack got dried.",
            DONE = "It's ready!",
            DONE_NOTMEAT = "It's ready!",
            DRYING = "Come on meat, dry already!",
            DRYINGINRAIN = "Forget the rain! Dry!",
            DRYINGINRAIN_NOTMEAT = "Forget the rain! Dry!",
            DRYING_NOTMEAT = "How long does it take this stuff to dry?!",
            GENERIC = "I want to hang some meat!",
        },
        MEATRACK_HERMIT = {
            BURNT = "The rack got dried.",
            DONE = "Hey lady, your jerky's ready!",
            DONE_NOTMEAT = "It's ready!",
            DRYING = "Come on meat, dry already!",
            DRYINGINRAIN = "Forget the rain! Dry!",
            DRYINGINRAIN_NOTMEAT = "Forget the rain! Dry!",
            DRYING_NOTMEAT = "How long does it take this stuff to dry?!",
            GENERIC = "Maybe I'll leave some meat here before I leave...",
        },
        MEATYSALAD = "Wait... what ARE these leaves?",
        MEAT_DRIED = "Chewy, but satisfying.",
        MEGAFLARE = "Heheh... let's start some trouble!",
        MERM = "Eww, it's all swampy.",
        MERMGUARD = "I can't tell which way it's looking.",
        MERMHAT = "Yuck, who'd want a face like that?",
        MERMHEAD = {
            BURNT = "Double whammy!",
            GENERIC = "That's what you get for being so stinky!",
        },
        MERMHOUSE = {
            BURNT = "It's true, no one cares.",
            GENERIC = "No one would care if this burned down.",
        },
        MERMHOUSE_CRAFTED = {
            BURNT = "I feel a little bad.",
            GENERIC = "Is house for fish men!",
        },
        MERMKING = "Are you supposed to be important or something?",
        MERMTHRONE = {
            BURNT = "Did I do that? It's hard to keep track.",
            GENERIC = "Big sitting mat looks very inviting.",
        },
        MERMTHRONE_CONSTRUCTION = {
            BURNT = "Hahaha-! Uh, I mean, oh no...",
            GENERIC = "What's all this stuff for?",
        },
        MERMWATCHTOWER_NOKING = "A royal guard with no Royal to guard.",
        MERMWATCHTOWER_REGULAR = "They seem happy to have found a king.",
        MERM_PRINCE = "What makes that guy so special?",
        MESSAGEBOTTLE = "Hey! It's a bottle of emergency kindling!",
        MESSAGEBOTTLEEMPTY = "An old bottle, nothing special.",
        MIGHTY_GYM = {
            BURNT = "I don't know what Wolfgang's so upset about, I thought the fire effects were great!",
            GENERIC = "You know what this strongman show needs? Pyrotechnics!",
        },
        MILKYWHITES = "No thanks.",
        MINERHAT = "A handy light for our head.",
        MINGOT = "Bring it to fire and hit with hammer.",
        MINIBOATLANTERN = "The ocean could use a bit more light.",
        MINIFAN = "No fun, making me exercise to stay cool!",
        MINIFLARE = "I can't WAIT to light it!",
        MINISIGN = {
            GENERIC = "Haha oh man, who drew THAT?",
            UNDRAWN = "Looks rather bare.",
        },
        MINISIGN_ITEM = "It's like a sign, but smaller.",
        MINOTAUR = "Poor thing. Trapped in this maze.",
        MINOTAURCHEST = "I want big horns like that!",
        MINOTAURHORN = "I wonder if the rings are an indication of age.",
        MMIKO_ARMOR = "This robe for miko outfit.",
        MNAGINATA = "Nagigaga~.",
        MOLE = {
            ABOVEGROUND = "Coming up for a peek!",
            HELD = "Out of the ground.",
            UNDERGROUND = "Hiding from the light, huh?",
        },
        MOLEBAT = "Haha gross! I just saw it eat something through its nose!",
        MOLEBATHILL = "Ew, is it sleeping in its own snot?",
        MOLEHAT = "Mole skin is stretchy.",
        MOLEHILL = "Mole hole.",
        MONKEY = "Joke's on you! I can cut this poo.",
        MONKEYBARREL = "Did you hear something?",
        MONKEYHUT = {
            BURNT = "Heh. This is probably why nobody lets me into their tree forts.",
            GENERIC = "Ugh, nobody ever lets me into their tree forts.",
        },
        MONKEYISLAND_PORTAL = "Um, should I be worried about that?",
        MONKEYISLAND_PORTAL_DEBRIS = "What's with all this metal junk all over the place?",
        MONKEYPILLAR = "A real pillar of the community.",
        MONKEYQUEEN = "Being in charge looks like a pretty easy job.",
        MONKEYTAIL = "It's so soft. And fuzzy.",
        MONKEY_MEDIUMHAT = "I took the monkey's hat.",
        MONKEY_SMALLHAT = "It rather itches...",
        MONSTERLASAGNA = "No thanks.",
        MONSTERMEAT = "No thanks",
        MONSTERMEAT_DRIED = "It's dry and smells strange.",
        MONSTERTARTARE = "Gross!",
        MOONBASE = {
            BROKEN = "Oh look, a bunch of smashed up rocks!",
            GENERIC = "It's expecting something from me.",
            MOONSTAFF = "Ughh! It's doing the exact opposite of burning!",
            STAFFED = "Charging. Please complete soon.",
            WRONGSTAFF = "That's obviously completely wrong.",
        },
        MOONBUTTERFLY = {
            GENERIC = "Hey, come back here!",
            HELD = "Got you now.",
        },
        MOONBUTTERFLYWINGS = "They came off a dead butterfly, gross.",
        MOONBUTTERFLY_SAPLING = "Just an un-burned baby tree.",
        MOONDIAL = {
            CAVE = "Doesn't work down here.",
            GENERIC = "What the heck? I can still see the moon!",
            GLASSED = "I feel like it's staring at me.",
            NIGHT_FULL = "That's as full as it gets.",
            NIGHT_NEW = "A new moon! Great, I hated the old one!",
            NIGHT_WANE = "The moon's outta here!",
            NIGHT_WAX = "The moon is waxing, unlike me.",
        },
        MOONGLASS = "It's very sharp.",
        MOONGLASSAXE = "The moon HATES trees.",
        MOONGLASS_CHARGED = "I should put this to scientific use before the energy fades!",
        MOONGLASS_ROCK = "That's a big chunk of glass!",
        MOONGLASS_WOBSTER_DEN = "We should try smoking them out.",
        MOONROCKCRATER = "Great. A rock with a hole in it.",
        MOONROCKIDOL = "What is this?",
        MOONROCKNUGGET = "Just another rock to me.",
        MOONROCKSEED = "Neat, it's a ball that floats by itself!",
        MOONROCK_PIECES = "Oh look! ROCKS! Ughhhhh!",
        MOONSPIDERDEN = "This den needs clearing.",
        MOONSTORM_GLASS = {
            GENERIC = "Hey, I can see my reflection!",
            INFUSED = "It's like there's a little fire inside.",
        },
        MOONSTORM_GOGGLESHAT = "At least I'll be wearing it in the storm where nobody can see me.",
        MOONSTORM_SPARK = "I'm going to touch it.",
        MOONSTORM_STATIC = "Don't get zapped, old man.",
        MOONSTORM_STATIC_ITEM = "It crackles, almost like a fire.",
        MOON_ALTAR = {
            GENERIC = "Doesn't it feel nice to be back in one piece?",
            MOON_ALTAR_WIP = "Almost done, I swear.",
        },
        MOON_ALTAR_ASTRAL = "Back together again.",
        MOON_ALTAR_COSMIC = "Why do I get the feeling we aren't done yet?",
        MOON_ALTAR_CROWN = "Y'know, this would be a lot easier if you weren't so heavy.",
        MOON_ALTAR_GLASS = "Yeah, of course I'll lug you over there.",
        MOON_ALTAR_ICON = "Alright, alright, let's get going.",
        MOON_ALTAR_IDOL = "Okay, let me take you home.",
        MOON_ALTAR_LINK = "Aw come on! What is it?!",
        MOON_ALTAR_ROCK_GLASS = "Something inside is calling for me.",
        MOON_ALTAR_ROCK_IDOL = "Something inside is calling for me.",
        MOON_ALTAR_ROCK_SEED = "Something inside is calling for me.",
        MOON_ALTAR_SEED = "Where do you want to go?",
        MOON_ALTAR_WARD = "You're right, I really am the best for helping you out.",
        MOON_CAP = "What a weird looking mushroom.",
        MOON_CAP_COOKED = "Once again, fire makes it better.",
        MOON_DEVICE = {
            CONSTRUCTION1 = "Uuuugh, why can't that old guy just build this thing himself?",
            CONSTRUCTION2 = "I guess I could just cut it and be done with it, but now I'm rather curious...",
            GENERIC = "Great! So what does it do?",
        },
        MOON_FISSURE = {
            GENERIC = "Ew, I think it's whispering to me.",
            NOLIGHT = "I do not know, maybe we can jam stuff in it.",
        },
        MOON_FISSURE_PLUGGED = "Ohh, so that's how she keeps those creeps away.",
        MOON_TREE = {
            BURNING = "The tree is burning!",
            BURNT = "I don't feel as bad as I thought I would.",
            CHOPPED = "It's not going to grow back.",
            GENERIC = "What a pretty tree!",
        },
        MOON_TREE_BLOSSOM = "It fell from the moon tree.",
        MOOSE = "What in the world...",
        MOOSEEGG = "It's huuuuge!",
        MOOSE_NESTING_GROUND = "Ugh, it smells like bird butts!",
        MOQUECA = "It looks soooo good!",
        MORTALBLADE = "Trick.",
        MOSQUITO = {
            GENERIC = "So annoying!",
            HELD = "Keep that mouth away from me!",
        },
        MOSQUITOSACK = "Blood must bubble in currents muggy and thick.",
        MOSSLING = "Its feathers are frazzled.",
        MOUND = {
            DUG = "Sorry, please lend me the funerary items first..",
            GENERIC = "I think, don't disturb it.",
        },
        MSURFBOARD_ITEM = "I learned to surf in Hawaii.",
        MULLEDDRINK = "It warms you from the inside out.",
        MULTIPLAYER_PORTAL = "It's pretty... pretty scary!",
        MULTIPLAYER_PORTAL_MOONROCK = "It's made of some kind of rock.",
        MULTITOOL_AXE_PICKAXE = "It's so useful!",
        MUSHGNOME = "Ew, it's dropping spores everywhere!",
        MUSHROOMBOMB = "She's going to blow!",
        MUSHROOMHAT = "I don't like having my head messed with.",
        MUSHROOMSPROUT = {
            BURNT = "How could that not work?!",
            GENERIC = "A big gross mushroom!",
        },
        MUSHROOM_FARM = {
            BURNT = "Mold problem's taken care of.",
            EMPTY = "It's just a log.",
            LOTS = "Gross, they're taking over!",
            ROTTEN = "Nasty. Let's burn the rot out.",
            SNOWCOVERED = "It can grow in this cold.",
            SOME = "There's mushrooms growing in it now.",
            STUFFED = "Geez, who even needs that many mushrooms?",
        },
        MUSHROOM_LIGHT = {
            BURNT = "C'mon, you saw that coming.",
            OFF = "This mushroom isn't lighting up.",
            ON = "Not as nice as a warm fire.",
        },
        MUSHROOM_LIGHT2 = {
            BURNT = "Now what?",
            OFF = "I've got a few ideas on how to light it. Heh heh.",
            ON = "Pfft, well that was a bright idea.",
        },
        MUSHTREE_MEDIUM = {
            ACIDCOVERED = "",
            BLOOM = "It's spreading junk everywhere.",
            GENERIC = "Gross. It smells like leprechaun butt.",
        },
        MUSHTREE_MOON = "This mushroom tree is clearly stranger than the rest.",
        MUSHTREE_SMALL = {
            ACIDCOVERED = "",
            BLOOM = "Ew, I don't want to get too close.",
            GENERIC = "Gross. It's all mushroomy.",
        },
        MUSHTREE_TALL = {
            ACIDCOVERED = "",
            BLOOM = "Ack! It stinks!",
            GENERIC = "Gross. This tree is sick all over.",
        },
        MUSHTREE_TALL_WEBBED = "That one got what it deserves.",
        MUTATEDBEARGER = "Did I ever mention my best friend is a bear?",
        MUTATEDDEERCLOPS = "Er... no hard feelings, right?",
        MUTATEDHOUND = {
            DEAD = "Good riddance.",
            GENERIC = "YUCK! What is that?!",
            SLEEPING = "Ewww I can see it breathing.",
        },
        MUTATEDWARG = "Don't worry, you're still going to go down in flames.",
        MUTATED_PENGUIN = {
            DEAD = "Ew, it's dead.",
            GENERIC = "Cut IT.",
            SLEEPING = "I don't want to wake it.",
        },
        MUTATOR_DROPPER = "Eww, there's legs sticking out of it!",
        MUTATOR_HEALER = "Eww, there's legs sticking out of it!",
        MUTATOR_HIDER = "This seems to be for spiders to eat.",
        MUTATOR_MOON = "This seems to be for spiders to eat.",
        MUTATOR_SPITTER = "Eww, there's legs sticking out of it!",
        MUTATOR_WARRIOR = "This seems to be for spiders to eat.",
        MUTATOR_WATER = "This seems to be for spiders to eat.",
        M_FOXMASK = "Now I'm a fox!.",
        M_PANTSU = "Hentai.",
        M_SCARF = "Makes you warm and fuzzy inside. Outside, too.",
        NATURE_BLUEPRINT = "Once the flowers dry out they'll make nice kindling.",
        NIGHTCAPHAT = "I've got a nightcap, now where's my candlestick?",
        NIGHTLIGHT = "It's like fire, but purple!",
        NIGHTMAREFUEL = "Eww, it's still warm!",
        NIGHTMAREGROWTH = "That's probably not... great...",
        NIGHTMARELIGHT = "I wonder what function this served.",
        NIGHTMAREPIE = "I'm sure he wouldn't mind if I stole a nibble.",
        NIGHTMARE_TIMEPIECE = {
            CALM = "Looks normal to me.",
            DAWN = "I guess it's nearly over.",
            NOMAGIC = "I don't think it's working.",
            STEADY = "It's almost humming.",
            WANING = "I think it's turning off.",
            WARN = "It's warning me.",
            WAXING = "The fuel is coming to life!",
        },
        NIGHTSTICK = "It's electric!",
        NIGHTSWORD = "It's like dreams that can hurt real things!",
        NITRE = "There are tiny explosions trapped inside.",
        NITRE_FORMATION = "I don't know how anything could grow in that weird rain.",
        NUTRIENTSGOGGLESHAT = "Wow, I always wanted dirt vision... said nobody ever.",
        OAR = "Rowing is slower. A sail would be more efficient.",
        OAR_DRIFTWOOD = "Ugh. Can it be someone else's turn to row?",
        OAR_MONKEY = "Hey, maybe rowing can be fun after all!",
        OASISLAKE = {
            EMPTY = "It's a bunch of dirt.",
            GENERIC = "It's a bunch of water.",
        },
        OCEANFISHABLEFLOTSAM = "Just a big hunk of yuck.",
        OCEANFISHINGBOBBER_BALL = "The fish will have a ball with this.",
        OCEANFISHINGBOBBER_CANARY = "Say y'ello to my little friend!",
        OCEANFISHINGBOBBER_CROW = "I'd rather eat fish than crow.",
        OCEANFISHINGBOBBER_GOOSE = "You're going down, fish!",
        OCEANFISHINGBOBBER_MALBATROSS = "Where there's a quill, there's a way.",
        OCEANFISHINGBOBBER_NONE = "A bobber might improve its accuracy.",
        OCEANFISHINGBOBBER_OVAL = "Those fish won't give me the slip this time!",
        OCEANFISHINGBOBBER_ROBIN = "Hopefully it won't attract any red herrings.",
        OCEANFISHINGBOBBER_ROBIN_WINTER = "The snowbird quill helps me stay frosty.",
        OCEANFISHINGLURE_HERMIT_DROWSY = "My brain is protected by a thick layer of hard science!",
        OCEANFISHINGLURE_HERMIT_HEAVY = "This feels a bit heavy handed.",
        OCEANFISHINGLURE_HERMIT_RAIN = "Soaking myself might help me think like a fish...",
        OCEANFISHINGLURE_HERMIT_SNOW = "The fish won't snow what hit them!",
        OCEANFISHINGLURE_SPINNER_BLUE = "Some fish might find this a-luring!",
        OCEANFISHINGLURE_SPINNER_GREEN = "Some fish might find this a-luring!",
        OCEANFISHINGLURE_SPINNER_RED = "Some fish might find this a-luring!",
        OCEANFISHINGLURE_SPOON_BLUE = "Some smaller fish might find this a-luring!",
        OCEANFISHINGLURE_SPOON_GREEN = "Some smaller fish might find this a-luring!",
        OCEANFISHINGLURE_SPOON_RED = "Some smaller fish might find this a-luring!",
        OCEANFISHINGROD = "Now this is the reel deal!",
        OCEANFISH_MEDIUM_1 = "Ew, that thing looks nasty!",
        OCEANFISH_MEDIUM_2 = "Quit looking at me like that!",
        OCEANFISH_MEDIUM_3 = "What's so dandy about it?",
        OCEANFISH_MEDIUM_4 = "Ha! I'm not scared of a little bad luck.",
        OCEANFISH_MEDIUM_5 = "What... is it?",
        OCEANFISH_MEDIUM_6 = "It's kind of pretty for something that lives in the water.",
        OCEANFISH_MEDIUM_7 = "It's kind of pretty for something that lives in the water.",
        OCEANFISH_MEDIUM_8 = "I dare someone to lick it!",
        OCEANFISH_MEDIUM_9 = "Another fish for the log.",
        OCEANFISH_SMALL_1 = "A small fish specimen.",
        OCEANFISH_SMALL_2 = "What the-? It's so small!",
        OCEANFISH_SMALL_3 = "Hey! I thought you were going to be a big fish!",
        OCEANFISH_SMALL_4 = "Looks more like a shrimp to me.",
        OCEANFISH_SMALL_5 = "Weird...",
        OCEANFISH_SMALL_6 = "I get the feeling it would make great kindling.",
        OCEANFISH_SMALL_7 = "You've uh... got something growing on your forehead.",
        OCEANFISH_SMALL_8 = "It's so sad... a fellow firebug forced to live its life underwater!",
        OCEANFISH_SMALL_9 = "Hey! Go spit at somebody else!",
        OCEANTREE = "I think these trees are a little lost.",
        OCEANTREENUT = "Ugh, it's still wet.",
        OCEANTREE_PILLAR = "Hey, it's going to stop the sun from burning things!",
        OCEANVINE = "Oh look, a really long wick!",
        OCEANVINE_COCOON = "Ewww look at it just dangling up there!",
        OCEANVINE_COCOON_BURNT = "Fire is a quick redecorator.",
        OCEANWHIRLPORTAL = "Can't we take the scenic route instead?",
        OCEAN_TRAWLER = {
            CAUGHT = "A decent haul.",
            ESCAPED = "You're not supposed to get away!",
            FIXED = "That should do it.",
            GENERIC = "It does all the work for me!",
            LOWERED = "I bet it's catching lots of fish right now.",
        },
        OCEAN_TRAWLER_KIT = "Maybe I can bribe one of the kids to build it for me.",
        ONEMANBAND = "I can do the pyrotechnics too!",
        ONION = "It's so bulbous.",
        ONION_COOKED = "Roasting. My favorite cooking method.",
        ONION_OVERSIZED = "I've got to figure out what I'm going to do with all this onion...",
        ONION_SEEDS = "It's just a bunch of seeds.",
        OPALPRECIOUSGEM = "It's glittery and mesmerizing.",
        OPALSTAFF = "Gotta hold it with my sleeve so my hands don't get cold.",
        ORANGEAMULET = "It picks up burning materials!",
        ORANGEGEM = "It'll make your fingers tingle.",
        ORANGEMOONEYE = "It's a rock. Ugh!",
        ORANGESTAFF = "It gives me a headache.",
        PALMCONETREE = "Kind of piney, for a palm tree.",
        PALMCONE_SAPLING = "It has big dreams of being a tree one day.",
        PALMCONE_SCALE = "If trees had toenails, I imagine they'd look like this.",
        PALMCONE_SEED = "The very beginnings of a tree.",
        PANDORASCHEST = "Kind of tacky.",
        PANETTONE = "Am I just hungry, or does everything look extra tasty?",
        PANFLUTE = "Not my preferred style of music.",
        PAPYRUS = "Is like paper.",
        PAVLOVA = "It's so delicate looking, I just want to smush it.",
        PEGHOOK = "It's got a weaponized butt!",
        PENGUIN = "Run away, tiny dancers.",
        PEPPER = "Good. These are the hot ones.",
        PEPPERPOPPER = "My mouth is on fire!",
        PEPPER_COOKED = "First rule of vegetables: fire makes it better.",
        PEPPER_OVERSIZED = "That's a lot of hot stuff!",
        PEPPER_SEEDS = "It's just a bunch of seeds.",
        PERD = "Evil bird! Get away from those yummy berries!",
        PERDFAN = "It's so big!!",
        PERDSHRINE = {
            BURNT = "That won't do at all.",
            EMPTY = "The pot's empty.",
            GENERIC = "I want to give it something!",
        },
        PEROGIES = "Tasty things.",
        PETALS = "Stupid flowers. They're almost useless.",
        PETALS_EVIL = "Ew, they're sticky.",
        PETRIFIED_TREE = "It's all stone and no bark.",
        PHLEGM = "It's a boogie!",
        PHONOGRAPH = "Oh, Maxwell will love this.",
        PICKAXE = "It's very pointy.",
        PICKAXE_LUNARPLANT = "The plant still serves a purpose as a tool.",
        PICKLEDHERRING = "How does it smell so good?",
        PIGELITE1 = "All washed up.",
        PIGELITE2 = "You, I like.",
        PIGELITE3 = "Eat dirt!",
        PIGELITE4 = "Sure does like using those signs.",
        PIGELITEFIGHTER1 = "All washed up.",
        PIGELITEFIGHTER2 = "You, I like.",
        PIGELITEFIGHTER3 = "Eat dirt!",
        PIGELITEFIGHTER4 = "Sure does like using those signs.",
        PIGGUARD = "I like his attitude!",
        PIGGYBACK = "It's a backpack made out of butts!",
        PIGHEAD = {
            BURNT = "It can always get worse, I guess.",
            GENERIC = "I guess I have it alright.",
        },
        PIGHOUSE = {
            BURNT = "Nice redecorating job!",
            FULL = "He's doing pig things in there.",
            GENERIC = "These pigs have questionable taste in architecture.",
            LIGHTSOUT = "Hey! Let me in!",
        },
        PIGKING = "Blech. What a slob.",
        PIGMAN = {
            DEAD = "I wonder how they taste.",
            FOLLOWER = "Ick. It's following me.",
            GENERIC = "Ugh. They're fragrant.",
            GUARD = "He's not the boss of me!",
            WEREPIG = "Bring it on, piggie!",
        },
        PIGSHRINE = {
            BURNT = "Burnt. I like it.",
            EMPTY = "Needs meat.",
            GENERIC = "Where's the shrine to me?",
        },
        PIGSKIN = "Ha ha. A pig's butt.",
        PIGTENT = "It smells terrible!",
        PIGTORCH = "These pigs sure know how to have a good time.",
        PIG_COIN = "Finally, someone to do my dirty work.",
        PIG_TOKEN = "It might be worth something.",
        PINECONE = "Hey there, tree seed.",
        PINECONE_SAPLING = "You'll be a real tree soon.",
        PIRATE_FLAG_POLE = "Ahoy!",
        PIRATE_STASH = "Treasure! Heh, I'm pretty good at this.",
        PITCHFORK = "Three times the pointiness.",
        PLANTMEAT = "Eww, it's all slimy.",
        PLANTMEAT_COOKED = "It smells kind of rotten.",
        PLANTREGISTRYHAT = "Nooo I don't want my head full of plant nerd facts!",
        PLANT_NORMAL = {
            GENERIC = "I'd eat it if I had to.",
            GROWING = "Steady growth. Keep conditions stable.",
            READY = "Oh boy. Vegetables.",
            WITHERED = "It's crackly and dry. Good kindling!",
        },
        PLAYBILL_THE_DOLL = "\"Authored by C.W.\"",
        PLAYER = {
            ATTACKER = "%s looks shifty...",
            FIRESTARTER = "I'm sure you had a very clever reason for that fire.",
            GENERIC = "Hi %s!",
            GHOST = "I better get a heart for %s.",
            MURDERER = "Murderer confirmed. Stay alert.",
            REVIVER = "Who do ghosts call? %s!",
        },
        POCKETWATCH_DISMANTLER = "A bunch of little tools.",
        POCKETWATCH_HEAL = {
            GENERIC = "Why worry about the past or future? It'll all go up in flames eventually.",
            RECHARGING = "It's not doing much right now.",
        },
        POCKETWATCH_PARTS = "Some weird clock junk.",
        POCKETWATCH_PORTAL = {
            GENERIC = "Why worry about the past or future? It'll all go up in flames eventually.",
            RECHARGING = "It's not doing much right now.",
        },
        POCKETWATCH_PORTAL_ENTRANCE = {
            DIFFERENTSHARD = "Am I really just going to jump into any old portal I see? Yep!",
            GENERIC = "Am I really just going to jump into any old portal I see? Yep!",
        },
        POCKETWATCH_PORTAL_EXIT = "It's fun to watch OTHER people fall on their butts.",
        POCKETWATCH_RECALL = {
            GENERIC = "Why worry about the past or future? It'll all go up in flames eventually.",
            RECHARGING = "It's not doing much right now.",
        },
        POCKETWATCH_REVIVE = {
            GENERIC = "Why worry about the past or future? It'll all go up in flames eventually.",
            RECHARGING = "It's not doing much right now.",
        },
        POCKETWATCH_WARP = {
            GENERIC = "Why worry about the past or future? It'll all go up in flames eventually.",
            RECHARGING = "It's not doing much right now.",
        },
        POCKETWATCH_WEAPON = {
            GENERIC = "That looks dangerous. I want to try!",
        },
        POCKET_SCALE = "Hey, now I can weigh my fish... not that I care.",
        POLISHCOOKIE = "Yup, I'll be having ten more of those.",
        POLLY_ROGERS = "Polly want to bring me stuff?",
        POLLY_ROGERSHAT = "Now that's stylish.",
        POMEGRANATE = "Is it supposed to have this many parts?",
        POMEGRANATE_COOKED = "Fire always makes things better.",
        POMEGRANATE_OVERSIZED = "I bet it looks super gross inside!",
        POMEGRANATE_SEEDS = "Some seeds.",
        POND = "This pond will definitely not ignite. How dull.",
        PONDEEL = "I don't like the look it's giving me!",
        PONDFISH = "Slippery fishy!",
        POND_ALGAE = "It must need a lot of water.",
        POOP = "Ew. Nasty. But useful.",
        PORTABLEBLENDER_ITEM = "Shakes things up a bit.",
        PORTABLECOOKPOT_ITEM = {
            COOKING_LONG = "Ughhh, this is taking forever!",
            COOKING_SHORT = "Come on fire, do your thing!",
            DONE = "Good. Let's eat!",
            EMPTY = "Stupid empty pot.",
            GENERIC = "A portable pot.",
        },
        PORTABLESPICER_ITEM = {
            DONE = "All that work for just a little bit of spice?",
            GENERIC = "All that work better be worth it.",
        },
        PORTABLETENT = "Nice and sturdy!",
        PORTABLETENT_ITEM = "I was never any good at setting these things up.",
        POTATO = "We grew it in that gross muck.",
        POTATOSACK = "I could make it a roasted potato sack, just say the word!",
        POTATOSOUFFLE = "MY stuff never comes out looking like THIS!",
        POTATOTORNADO = "Woah, I didn't know potatoes came in that shape!",
        POTATO_COOKED = "Cooking just means sticking stuff in fire.",
        POTATO_OVERSIZED = "Let's roast this thing!",
        POTATO_SEEDS = "It's just a bunch of seeds.",
        POTTEDFERN = "A fern in a pot.",
        POWCAKE = "This never ends well.",
        POWDER_MONKEY = "Hey! Hands off my stuff!",
        PREMIUMWATERINGCAN = {
            EMPTY = "Reassuringly dry.",
            GENERIC = "Simple design, but functional.",
        },
        PRIME_MATE = "I don't like the way he swings that paddle around.",
        PUFFIN = {
            GENERIC = "A bird! I wonder if it's tasty.",
            HELD = "Haha, I got you.",
            SLEEPING = "I bet I could sneak up on it now.",
        },
        PUMPKIN = "I wonder what would happen if I applied fire.",
        PUMPKINCOOKIE = "Cookies!!!",
        PUMPKINPIE = "Yep, I'm eating this whole thing.",
        PUMPKIN_COOKED = "Fire on the outside turned out pretty well.",
        PUMPKIN_LANTERN = "Fire on the inside is amazing!",
        PUMPKIN_OVERSIZED = "I could stick the biggest candle in there! It'll be great!",
        PUMPKIN_SEEDS = "Some seeds.",
        PUNCHINGBAG = "Think fast, dummy!",
        PUREBRILLIANCE = "It's pretty, but it'll never beat fire.",
        PURPLEAMULET = "Has science gone too far?",
        PURPLEGEM = "Weird!",
        PURPLEMOONEYE = "Some sort of weird rock.",
        QUAGMIRE_ALTAR = {
            FULL = "I wonder where all the food goes. Probably somewhere gross.",
            GENERIC = "Don't. Look. Up.",
        },
        QUAGMIRE_ALTAR_BOLLARD = "It's nice enough.",
        QUAGMIRE_ALTAR_IVY = "Ivy grows everywhere.",
        QUAGMIRE_ALTAR_QUEEN = "Wow. She looks beautiful.",
        QUAGMIRE_ALTAR_STATUE1 = "Look at this goat statue.",
        QUAGMIRE_ALTAR_STATUE2 = "A lotta goat statues around here.",
        QUAGMIRE_BEEFALO = "Take it easy, grandpa.",
        QUAGMIRE_BERRYBUSH = "Aww... all the berries are gone now.",
        QUAGMIRE_BOWL_SILVER = "Now I don't have to use my hands.",
        QUAGMIRE_CARROT_SEEDS = "It's just a bunch of seeds.",
        QUAGMIRE_CASSEROLEDISH = "Fancy.",
        QUAGMIRE_CASSEROLEDISH_SMALL = "It's rather small.",
        QUAGMIRE_COIN1 = "Wow. A coin. Pfft.",
        QUAGMIRE_COIN2 = "I can probably buy something good with this.",
        QUAGMIRE_COIN3 = "I'm rich!",
        QUAGMIRE_COIN4 = "Guess that thing liked what we gave it.",
        QUAGMIRE_CRABMEAT = "No imitations here.",
        QUAGMIRE_CRABMEAT_COOKED = "I bet it's tasty.",
        QUAGMIRE_CRABTRAP = "That'll show those crabs.",
        QUAGMIRE_CRATE = "",
        QUAGMIRE_FERN = "I can take my pick of them.",
        QUAGMIRE_FLOUR = "Can you just eat flour?",
        QUAGMIRE_FOLIAGE_COOKED = "Leaf this to a professional.",
        QUAGMIRE_FOOD = {
            GENERIC = "Maybe that thing up there'll eat it.",
            MATCH = "This should shut that thing up for a while.",
            MATCH_BUT_SNACK = "This won't shut it up for long.",
            MISMATCH = "I think the Gnaw wants something different.",
        },
        QUAGMIRE_FOOD_BURNT = "Looks good to me.",
        QUAGMIRE_GARLIC = "It's just a couple cloves of garlic.",
        QUAGMIRE_GARLIC_COOKED = "Fire made it better.",
        QUAGMIRE_GARLIC_SEEDS = "It's just a bunch of seeds.",
        QUAGMIRE_GOATKID = "He'll eat just about anything.",
        QUAGMIRE_GOATMILK = "What? We needed milk.",
        QUAGMIRE_GOATMUM = "Her eyes creep me out.",
        QUAGMIRE_GRILL = "Now all I need is a backyard to put it in.",
        QUAGMIRE_GRILL_ITEM = "I'll have to grill someone about this.",
        QUAGMIRE_GRILL_SMALL = "Barbecurious.",
        QUAGMIRE_GRILL_SMALL_ITEM = "For grilling small meats.",
        QUAGMIRE_HOE = "You mean I have to do work?",
        QUAGMIRE_KEY = "Cool! I wonder what it unlocks?",
        QUAGMIRE_KEY_PARK = "I want to go to the park.",
        QUAGMIRE_LAMP_POST = "It's the kind of lamp they have in the city.",
        QUAGMIRE_LAMP_SHORT = "We're almost as tall as this lamp.",
        QUAGMIRE_MEALINGSTONE = "I can smash stuff down with it.",
        QUAGMIRE_MERMHOUSE = "That's what passes for a village around here, I guess.",
        QUAGMIRE_MERM_CART1 = "Do they have any toys?",
        QUAGMIRE_MERM_CART2 = "That's full of good stuff.",
        QUAGMIRE_MUSHROOMS = "It's totally raw. Fire could fix that!",
        QUAGMIRE_MUSHROOMSTUMP = {
            GENERIC = "They're so hideous they're almost cute.",
            PICKED = "Hey! Give me more mushrooms!",
        },
        QUAGMIRE_ONION = "It's so bulbous.",
        QUAGMIRE_ONION_COOKED = "Roasting. My favorite cooking method.",
        QUAGMIRE_ONION_SEEDS = "It's just a bunch of seeds.",
        QUAGMIRE_OVEN = "Now we're cooking.",
        QUAGMIRE_OVEN_ITEM = "I can't wait to use it!",
        QUAGMIRE_PARKSPIKE = "That looks sharp.",
        QUAGMIRE_PARK_ANGEL = "Gross.",
        QUAGMIRE_PARK_ANGEL2 = "Ugly.",
        QUAGMIRE_PARK_FOUNTAIN = "A fountain with no water. Maybe that's a metaphor?",
        QUAGMIRE_PARK_GATE = {
            GENERIC = "Didn't even have to melt the locks.",
            LOCKED = "It's locked. I think we need a key.",
        },
        QUAGMIRE_PARK_OBELISK = "It's a monument or something.",
        QUAGMIRE_PARK_URN = "Cremation is the only way to do it.",
        QUAGMIRE_PEBBLECRAB = "Don't worry, I won't kick you.",
        QUAGMIRE_PIGEON = {
            DEAD = "It's dead.",
            GENERIC = "I haven't got any crumbs for you.",
            SLEEPING = "It's taking a nap.",
        },
        QUAGMIRE_PLATE_SILVER = "Ohh. Look who's fancy now!",
        QUAGMIRE_POND_SALT = "Blech! Tastes like ocean.",
        QUAGMIRE_PORTAL = "I'm just going to keep jumping through portals til I figure out a way home.",
        QUAGMIRE_PORTAL_KEY = "Fancy key.",
        QUAGMIRE_POT = "This pot holds more ingredients.",
        QUAGMIRE_POTATO = "We grew it in that gross muck.",
        QUAGMIRE_POTATO_COOKED = "Cooking just means sticking stuff in fire.",
        QUAGMIRE_POTATO_SEEDS = "It's just a bunch of seeds.",
        QUAGMIRE_POT_HANGER = "It has hang-ups.",
        QUAGMIRE_POT_HANGER_ITEM = "For suspension-based cookery.",
        QUAGMIRE_POT_SMALL = "Let's get cooking!",
        QUAGMIRE_POT_SYRUP = "I need to sweeten this pot.",
        QUAGMIRE_ROTTEN_CROP = "That one stayed in the ground too long.",
        QUAGMIRE_RUBBLE_BIKE = "Aww... It's broken.",
        QUAGMIRE_RUBBLE_CARRIAGE = "I wish i could ride in that.",
        QUAGMIRE_RUBBLE_CATHEDRAL = "I can't even fix it.",
        QUAGMIRE_RUBBLE_CHIMNEY = "What's a chimney without a fireplace?",
        QUAGMIRE_RUBBLE_CHIMNEY2 = "Aw. That's too bad.",
        QUAGMIRE_RUBBLE_CLOCK = "Does it still tell the right time?",
        QUAGMIRE_RUBBLE_CLOCKTOWER = "I don't think that clock works anymore.",
        QUAGMIRE_RUBBLE_HOUSE = {
            "I wonder what happened here?",
            "Where did everyone go?",
            "Looks like everyone left.",
        },
        QUAGMIRE_RUBBLE_PUBDOOR = "It doesn't lead anywhere anymore.",
        QUAGMIRE_RUBBLE_ROOF = "Too bad.",
        QUAGMIRE_SAFE = {
            GENERIC = "Ha! Cracked it.",
            LOCKED = "I can't get it to open.",
        },
        QUAGMIRE_SALMON = "Look at it, flopping around like that.",
        QUAGMIRE_SALMON_COOKED = "Ready for the dinner table.",
        QUAGMIRE_SALT = "I need a pinch or two on all my food.",
        QUAGMIRE_SALTROCK = "I licked one. Had to.",
        QUAGMIRE_SALT_RACK = {
            GENERIC = "Grow faster, salt crystals!",
            READY = "Gotta dredge the salt up.",
        },
        QUAGMIRE_SALT_RACK_ITEM = "I can use it to get some cooking salt.",
        QUAGMIRE_SAP = "Mmm! Pure sugar!",
        QUAGMIRE_SAPBUCKET = "Now we can get sap out of the trees.",
        QUAGMIRE_SAPLING = "It's going to take too long to grow back.",
        QUAGMIRE_SAP_SPOILED = "That's unfortunate.",
        QUAGMIRE_SEEDPACKET = "Aw...I won't know what these are until I plant them.",
        QUAGMIRE_SLAUGHTERTOOL = "I don't want to use this.",
        QUAGMIRE_SPOTSPICE_GROUND = "Spices... the true spice of life.",
        QUAGMIRE_SPOTSPICE_SHRUB = {
            GENERIC = "It smells kind of pepper-y.",
            PICKED = "That's not growing back.",
        },
        QUAGMIRE_SPOTSPICE_SPRIG = "It's a piece of that pepper-y bush.",
        QUAGMIRE_SUGARWOODTREE = {
            GENERIC = "It's full of delicious, delicious sap.",
            STUMP = "It's rather useless now.",
            TAPPED_BUGS = "Stupid bugs.",
            TAPPED_EMPTY = "It's taking forever!",
            TAPPED_READY = "Sap's ready!",
            WOUNDED = "You don't look so hot, tree.",
        },
        QUAGMIRE_SWAMPIG = "Not the prettiest face.",
        QUAGMIRE_SWAMPIGELDER = {
            GENERIC = "They're not so different from the pigs back at our place.",
            SLEEPING = "It's taking a nap.",
        },
        QUAGMIRE_SWAMPIG_HOUSE = "That structure looks unstable.",
        QUAGMIRE_SWAMPIG_HOUSE_RUBBLE = "Hey pigs! Your house blew down!",
        QUAGMIRE_SYRUP = "Mmmm...",
        QUAGMIRE_TOMATO = "Cutting these is a pain.",
        QUAGMIRE_TOMATO_COOKED = "It just needed a couple minutes on the fire.",
        QUAGMIRE_TOMATO_SEEDS = "It's just a bunch of seeds.",
        QUAGMIRE_TRADER_MERM = "Hey, what'dya got for me?",
        QUAGMIRE_TRADER_MERM2 = "Nice hat. Are you selling that, too?",
        QUAGMIRE_TURNIP = "Haha gross, I hate turnips.",
        QUAGMIRE_TURNIP_COOKED = "Take that, turnip.",
        QUAGMIRE_TURNIP_SEEDS = "It's just a bunch of seeds.",
        QUAGMIRE_WHEAT = "How on earth do you eat this?",
        QUAGMIRE_WHEAT_SEEDS = "It's just a bunch of seeds.",
        RABBIT = {
            GENERIC = "He looks tasty.",
            HELD = "I have him right where I want him!",
        },
        RABBITHOLE = {
            GENERIC = "Stupid rabbits. Come out so I can eat you.",
            SPRING = "Stupid rabbits must be stuck down there.",
        },
        RABBITHOUSE = {
            BURNT = "Ha! Good result.",
            GENERIC = "Ugh. Stupid rabbits.",
        },
        RAIKIRI = "Once it used to split a bolt of lightning.",
        RAINCOAT = "That'll do the trick.",
        RAINHAT = "Anything to keep the water away.",
        RAINOMETER = {
            BURNT = "Take that, rain!",
            GENERIC = "It measures cloudiness.",
        },
        RATATOUILLE = "Vegetables. So many vegetables.",
        RAZOR = "What's it even for?",
        RECORD = "Hey Max, it's your favorite!",
        RECORD_CREEPYFOREST = "Music helps me keep rhythm while I work.",
        RECORD_DANGER = "There's something so nostalgic about the way it pops and crackles.",
        RECORD_DAWN = "Music helps me keep rhythm while I work.",
        RECORD_DRSTYLE = "There's something so nostalgic about the way it pops and crackles.",
        RECORD_DUSK = "Music helps me keep rhythm while I work.",
        RECORD_EFS = "There's something so nostalgic about the way it pops and crackles.",
        RECORD_END = "Music helps me keep rhythm while I work.",
        RECORD_MAIN = "There's something so nostalgic about the way it pops and crackles.",
        RECORD_WORKTOBEDONE = "Music helps me keep rhythm while I work.",
        REDGEM = "So pretty!",
        REDLANTERN = "Our own personal night light!",
        REDMOONEYE = "It can see and be seen for miles!",
        REDPOUCH = "I'am so lucky!",
        RED_CAP = "I like the color.",
        RED_CAP_COOKED = "It's been transformed by fire!",
        RED_MUSHROOM = {
            GENERIC = "Pretty!",
            INGROUND = "Hey! You! Get up here!",
            PICKED = "Maybe it will return some day.",
        },
        REEDS = {
            BURNING = "Those burn quickly!",
            GENERIC = "It's a burnable clump of reeds.",
            PICKED = "It's just reed stubble.",
        },
        REFINED_DUST = "Great. What am I supposed to do with this stuff?",
        REFLECTIVEVEST = "Vests are so in.",
        RELIC = "Old furniture.",
        RESEARCHLAB = {
            BURNT = "There's no learning from that now.",
            GENERIC = "Even I don't know everything... yet.",
        },
        RESEARCHLAB2 = {
            BURNT = "There's no learning from that now.",
            GENERIC = "Even I don't know everything... yet.",
        },
        RESEARCHLAB3 = {
            BURNT = "Now it's dark and not very powerful.",
            GENERIC = "A dark and powerful energy radiates from it.",
        },
        RESEARCHLAB4 = {
            BURNT = "Double, double toil and... oh. It's burned.",
            GENERIC = "I can use the hat like a cauldron!",
        },
        RESKIN_TOOL = "Oh yeah, I'm going to have fun with this.",
        RESURRECTIONSTATUE = {
            BURNT = "Our policy was canceled.",
            GENERIC = "That's our insurance policy.",
        },
        RESURRECTIONSTONE = "I'll touch it when I'm good and ready.",
        REVIVER = "I expected it to be blacker.",
        RHINODRILL = "Pfft. Whatever Rhinocebros.",
        ROASTTURKEY = "What an enormous turkey leg!",
        ROBIN = {
            GENERIC = "Does that mean winter is gone?",
            HELD = "He likes my pocket.",
        },
        ROBIN_WINTER = {
            GENERIC = "Life in the frozen wastes.",
            HELD = "It's so soft.",
        },
        ROBOT_BLUEPRINT = "Uh, is this really a sewing pattern?",
        ROBOT_PUPPET = "They're trapped!",
        ROCK = "It's, like, a rock.",
        ROCKS = "What's the point of collecting these again?",
        ROCKY = "We don't have much in common.",
        ROCK_AVOCADO_BUSH = {
            BARREN = "It's not going to grow fruit anymore.",
            BURNING = "Yesss!",
            DISEASED = "EW, it smells!!",
            DISEASING = "I think it's getting sick.",
            GENERIC = "It's a fruit bush of some sort.",
            PICKED = "Someone took the fruit already.",
            WITHERED = "Pfft, it's barely even hot out.",
        },
        ROCK_AVOCADO_FRUIT = "I bet it'll taste good when it's ripe.",
        ROCK_AVOCADO_FRUIT_RIPE = "It's ripe!",
        ROCK_AVOCADO_FRUIT_RIPE_COOKED = "It's actually soft enough to eat now.",
        ROCK_AVOCADO_FRUIT_SPROUT = "Ewww it's got a little arm.",
        ROCK_ICE = {
            GENERIC = "Ice can be useful.",
            MELTED = "Puddle!",
        },
        ROCK_ICE_MELTED = "Puddle!",
        ROCK_LIGHT = {
            GENERIC = "There's heat in there, just waiting to get out!",
            LOW = "The lava is cooling.",
            NORMAL = "Nice and comfy.",
            OUT = "Aww, it's cooled off.",
        },
        ROCK_MOON = "Just another rock to me.",
        ROCK_PETRIFIED_TREE = "It's all stone and no bark.",
        ROCK_PETRIFIED_TREE_OLD = "It's all stone and no bark.",
        ROOK = "It's a castle!",
        ROOK_NIGHTMARE = "It has a nasty smile.",
        ROPE = "What should we tie up??!",
        ROTTENEGG = "Ew! Why? Oh Why?!",
        ROYAL_JELLY = "It's so sweet!",
        RUBBLE = "Broken furniture.",
        RUINSHAT = "It seems like there's something flowing through it.",
        RUINS_BAT = "This will keep the nasties at bay.",
        RUINS_RUBBLE = "That could probably be fixed up.",
        SACRED_CHEST = {
            GENERIC = "A very very old chest.",
            LOCKED = "I think it's judging me.",
        },
        SADDLEHORN = "I bet it stinks under the saddle.",
        SADDLE_BASIC = "It's so uncomfortable.",
        SADDLE_RACE = "Was it worth it? I think it was worth it.",
        SADDLE_WAR = "I'm going to raze some villages!",
        SADDLE_WATHGRITHR = "Goin' for that understated look, I see.",
        SALSA = "Hey, this is great!",
        SALTBOX = "It'll keep my stuff from rotting, for a little while.",
        SALTLICK = "It's a big block of slobbery salt.",
        SALTROCK = "What a weird rock.",
        SALTSTACK = {
            GENERIC = "Huh. That's... weird.",
            GROWING = "Hey, it's growing back!",
            MINED_OUT = "Someone got all the salt already.",
        },
        SANDBLOCK = "Things are getting gritty!",
        SANDSPIKE = "Hit and a miss!",
        SANITYROCK = {
            ACTIVE = "I wonder what these markings mean.",
            INACTIVE = "Where'd it go?",
        },
        SAPLING = {
            BURNING = "So bright!",
            DISEASED = "Maybe it needs some chicken soup?",
            DISEASING = "Blech. You smell.",
            GENERIC = "It'd be worth it to pick that.",
            PICKED = "Poor little limp tree.",
            WITHERED = "I think the heat broke it.",
        },
        SCANDATA = "hmmm..?",
        SCARECROW = {
            BURNING = "It's a burning man.",
            BURNT = "Just burnt straw.",
            GENERIC = "Ha! He thinks he's people!",
        },
        SCORCHED_SKELETON = "Ewwwwwwwwwww!",
        SCRAPBOOK_PAGE = "Another page for the emergency kindling supply- I mean scrapbook.",
        SCRAPHAT = "",
        SCRAP_MONOCLEHAT = "",
        SCULPTINGTABLE = {
            BLOCK = "I'm going to carve a stone butt.",
            BURNT = "Haha!",
            EMPTY = "I always wanted pottery lessons!",
            SCULPTURE = "I guess that's good, too.",
        },
        SCULPTURE_BISHOPBODY = {
            COVERED = "Not burnable. Not interested.",
            FINISHED = "Humpty Dumpty's back together again.",
            READY = "Ew! It's wriggling!",
            UNCOVERED = "The crank doesn't even turn!",
        },
        SCULPTURE_BISHOPHEAD = "Gross. I don't think that was supposed to come off.",
        SCULPTURE_KNIGHTBODY = {
            COVERED = "It's a marble statue, I guess.",
            FINISHED = "He didn't even say \"thank you\".",
            READY = "Ew! It's wriggling!",
            UNCOVERED = "That thing looks terrible.",
        },
        SCULPTURE_KNIGHTHEAD = "Great, now we've irritated some marble crime boss!",
        SCULPTURE_ROOKBODY = {
            COVERED = "Looks like a bunch of rubble to me.",
            FINISHED = "I'm more used to destroying things than fixing them.",
            READY = "Ew! It's wriggling!",
            UNCOVERED = "What a creepy load of junk!",
        },
        SCULPTURE_ROOKNOSE = "Ha, falling to pieces. Rookie mistake.",
        SEAFARING_PROTOTYPER = {
            BURNT = "Heh heh. Whoops.",
            GENERIC = "Watery nerd stuff.",
        },
        SEAFOODGUMBO = "Ew, what IS this?",
        SEASTACK = "A large sea rock.",
        SECURITY_PULSE_CAGE = "It's empty. So what?",
        SECURITY_PULSE_CAGE_FULL = "Great, it glows. But does it burn?",
        SEEDPOUCH = "I was getting tired of carrying loose seeds in my pockets.",
        SEEDS = "Farming takes patience.",
        SEEDS_COOKED = "No good for farming now.",
        SENTRYWARD = "Woah! I bet it sees so far!",
        SEWING_KIT = "Tool care is a basic duty. I'll repair it properly.",
        SEWING_MANNEQUIN = {
            BURNT = "All burnt up and nowhere to go.",
            GENERIC = "All dressed up and nowhere to go.",
        },
        SEWING_TAPE = "At least something around here can hold it together.",
        SHADOWDANCER = "Yep, that's pure nightmare fuel.",
        SHADOWDIGGER = "Eww, it's even creepier than the real one.",
        SHADOWHEART = "Oh, ewww! Who would ever touch that?!",
        SHADOWRIFT_PORTAL = "Nope. Not going near that.",
        SHADOWTHRALL_HANDS = "What has two hands and no feet? Whatever that thing is.",
        SHADOWTHRALL_HORNS = "Go snack on somebody else!",
        SHADOWTHRALL_WINGS = "You don't scare me! Okay, maybe a little.",
        SHADOW_FORGE = "Alright, let's get weird!",
        SHADOW_FORGE_KIT = "I have to put it together myself?",
        SHARK = "Uhh... nice fishy?",
        SHARKBOI = "Fluffy Shark.",
        SHELL_CLUSTER = "Just a bunch of shells.",
        SHIELDOFTERROR = "At least there's no eye gunk on it.",
        SHINAI = "Perfect for drills. Check cut line and footwork.",
        SHIRASAYA = "Clean blade geometry. I'll refine draw and sheathing form.",
        SHOVEL = "Not great for fighting.",
        SHOVEL_LUNARPLANT = "Alright, time to find some stuff to dig up!",
        SHROOMBAIT = "",
        SHROOMCAKE = "Cake's cake, pass it here.",
        SHROOM_SKIN = "Ew!",
        SHUSUI = "emmmmm......",
        SIESTAHUT = {
            BURNT = "It won't provide much shade now.",
            GENERIC = "They taught us how to make these in Girl Scouts.",
        },
        SILK = "Mmmmmm. Smooth.",
        SINGINGSHELL_OCTAVE3 = {
            GENERIC = "Ew, it's making noises!",
        },
        SINGINGSHELL_OCTAVE4 = {
            GENERIC = "Those things freak me out.",
        },
        SINGINGSHELL_OCTAVE5 = {
            GENERIC = "Nope. No thanks.",
        },
        SISTURN = {
            GENERIC = "A pyre would've been more fun.",
            LOTS_OF_FLOWERS = "I guess it's kind of pretty...",
            SOME_FLOWERS = "I will find more flowers for tiny house!",
        },
        SKELETON = "I hope you at least went out in a blaze of glory.",
        SKELETONHAT = "I've got a headache just looking at it.",
        SKELETON_NOTPLAYER = "",
        SKELETON_PLAYER = {
            DEFAULT = "Oh no, %s! %s must have really hurt them!",
            FEMALE = "Oh no, %s! %s must have really hurt her!",
            MALE = "Oh no, %s! %s must have really hurt him!",
            ROBOT = "Oh no, %s! %s must have really hurt them!",
        },
        SKETCH = "Trading diagrams is for dorks.",
        SKULLCHEST = "Ooooh, spooky!",
        SLEEPBOMB = "Who's ready for a nap!",
        SLINGSHOT = "Nung Stick!.",
        SLINGSHOTAMMO_FREEZE = "Shots to be slinged.",
        SLINGSHOTAMMO_GOLD = "Shots to be slinged.",
        SLINGSHOTAMMO_MARBLE = "Shots to be slinged.",
        SLINGSHOTAMMO_POOP = "Poop projectiles.",
        SLINGSHOTAMMO_ROCK = "Shots to be slinged.",
        SLINGSHOTAMMO_SLOW = "Shots to be slinged.",
        SLINGSHOTAMMO_THULECITE = "Shots to be slinged.",
        SLURPER = "No! No! Stay off head!",
        SLURPERPELT = "Eeewwwww, it's still alive!",
        SLURPER_PELT = "Eeewwwww, it's still alive!",
        SLURTLE = "I want to blow it up!",
        SLURTLEHAT = "It's perfectly head-shaped.",
        SLURTLEHOLE = "There's something gross in there.",
        SLURTLESLIME = "Eww!",
        SLURTLE_SHELLPIECES = "Heh. It broke.",
        SMALLBIRD = {
            GENERIC = "Not quite a phoenix, but still cute. I guess.",
            HUNGRY = "Are you hungry?",
            SLEEPING = "It's asleep for now.",
            STARVING = "Okay, okay! I get it, you're hungry.",
        },
        SMALLGHOST = "What are you looking at, pipsqueak?",
        SMALLMEAT = "A couple more'll make a morsel meal!",
        SMALLMEAT_DRIED = "Chewy, but satisfying.",
        SNAPPER = "Jeez, what's his crocodeal?",
        SNURTLE = "Kaboom!",
        SOIL_AMENDER = {
            GENERIC = "Gross.",
            SPOILED = "Let's hope this works as well as it stinks.",
            STALE = "Yup, stuff's happening in there alright.",
        },
        SOIL_AMENDER_FERMENTED = "Yeesh, plants actually like this stuff?",
        SPAT = "I don't like your face!",
        SPEAR = "That looks good for jabbin'!",
        SPEAR_GUNGNIR = "I don't want to use that.",
        SPEAR_LANCE = "Looks cumbersome.",
        SPEAR_WATHGRITHR = "It feels very stabby.",
        SPEAR_WATHGRITHR_LIGHTNING = "I still like my Rachel",
        SPICEPACK = "I'm sure Warly won't mind if I borrow this.",
        SPICE_CHILI = "I like the way it sets my mouth on fire.",
        SPICE_GARLIC = "Powdered stink.",
        SPICE_SALT = "People say I'M salty.",
        SPICE_SUGAR = "It's like drinking candy.",
        SPIDER = {
            DEAD = "Rest in peace...",
            GENERIC = "Just swing and die.",
            SLEEPING = "I could take him.",
        },
        SPIDERDEN = "That's just nasty.",
        SPIDERDEN_BEDAZZLER = "He's certainly got a creative streak, doesn't he?",
        SPIDEREGGSACK = "Tons of tiny disgusting spiders.",
        SPIDERGLAND = "Eeeeew, it's slimy and stinky!",
        SPIDERHAT = "Who's your mommy!",
        SPIDERHOLE = "It's full of spiders.",
        SPIDERHOLE_ROCK = "It's full of spiders.",
        SPIDERQUEEN = "I will cut it down!",
        SPIDER_DROPPER = "They come from above.",
        SPIDER_HEALER = "Ew, it stinks!",
        SPIDER_HEALER_ITEM = "Ewww it's all goopy!",
        SPIDER_HIDER = "Spider Spider and Spiders!",
        SPIDER_MOON = {
            DEAD = "Confirm neutralization and clear it safely.",
            GENERIC = "High threat target. Engage carefully.",
            SLEEPING = "Best timing for a controlled strike.",
        },
        SPIDER_REPELLENT = "Well it might not work, but at least it makes noise!",
        SPIDER_SPITTER = "Get over here!",
        SPIDER_WARRIOR = {
            DEAD = "Rest in peace...",
            GENERIC = "I hate this one.",
            SLEEPING = "Maybe I should just leave that one alone.",
        },
        SPIDER_WATER = "Get outta the water so I can cut you!",
        SPIDER_WHISTLE = "Nu-uh, I'm not putting that thing anywhere near my face.",
        SPOILED_FISH = "Gross.",
        SPOILED_FOOD = "Hmmm.",
        SPORE_MEDIUM = {
            GENERIC = "It's like floating fire!",
            HELD = "If I stare at it I'll turn to stone!",
        },
        SPORE_MOON = "Alright, I get the message! I'll stay away!",
        SPORE_SMALL = {
            GENERIC = "It has no idea where it's going.",
            HELD = "If I lick it I'll turn to wood!",
        },
        SPORE_TALL = {
            GENERIC = "It's like colorful sparks!",
            HELD = "If I eat it I'll turn to water!",
        },
        SQUID = "No substitute for a good torch.",
        STAFFCOLDLIGHT = "Boo! Hissss!",
        STAFFLIGHT = "It's so beautiful!",
        STAFF_LUNARPLANT = "This magic stuff is easy!",
        STAFF_TORNADO = "Always whirling. Powerful area control.",
        STAGEHAND = {
            AWAKE = "Gross! I told you we shoulda burned it!",
            HIDING = "What's a weird table doing out here?",
        },
        STAGEUSHER = {
            SITTING = "Something's odd here, but I can't put my finger on it.",
            STANDING = "Just keep your hand to yourself, alright?",
        },
        STALAGMITE = "A standard rock formation.",
        STALAGMITE_TALL = "Another tall rock formation.",
        STALKER = "I brought you back so I could beat you up!",
        STALKER_ATRIUM = "It's just bones and shadow.",
        STALKER_MINION = "Yuck, it's barely even alive.",
        STASH_MAP = "The monkeys left a map to their stash.",
        STATUEGLOMMER = {
            EMPTY = "Looks about the same.",
            GENERIC = "Is that supposed to be something?",
        },
        STATUEHARP = "Such a nice statue. Shame if something were to happen to it.",
        STATUEHARP_HEDGESPAWNER = "The flowers grew back, but the head didn't.",
        STATUEMAXWELL = "Carefully constructed cults of personality are vulnerable.",
        STATUE_MARBLE = {
            GENERIC = "It's an okay statue, I guess.",
            TYPE1 = "Yikes. I'd cover that up too if I were her.",
            TYPE2 = "She hasn't put her face on for the day yet.",
            TYPE3 = "A brazier would be nicer.",
        },
        STEELWOOL = "Scratchy metal fibers.",
        STEERINGWHEEL = "Check out how fast I can spin it!!",
        STEERINGWHEEL_ITEM = "Can't sail without that.",
        STINGER = "It's pokey!",
        STORAGE_ROBOT = {
            BROKEN = "",
            GENERIC = "",
        },
        STRAWHAT = "A hat made of straw. To think, it could've been tinder.",
        STUFFEDEGGPLANT = "It's still not a bird, but it's sure stuffed like one!",
        STUFFING = "Out of the turkey and into my stomach!",
        SUCCULENT_PICKED = "It got picked.",
        SUCCULENT_PLANT = "I think it's a cactus.",
        SUCCULENT_POTTED = "A succulent in a pot.",
        SUNKENCHEST = {
            GENERIC = "The real treasure is the treasure we found along the way.",
            LOCKED = "Aw, come on!",
        },
        SUPERTACKLECONTAINER = "Bet I could cram a lot more stuff in there.",
        SUPPORT_PILLAR = "I don't want to be here when that thing finally breaks.",
        SUPPORT_PILLAR_BROKEN = "I guess I'll have to keep an eye out for falling rocks again.",
        SUPPORT_PILLAR_COMPLETE = "It's nice not having to worry about rocks falling on my head.",
        SUPPORT_PILLAR_DREADSTONE = "I don't want to be here when that thing finally breaks.",
        SUPPORT_PILLAR_DREADSTONE_BROKEN = "Somebody should really repair this.",
        SUPPORT_PILLAR_DREADSTONE_COMPLETE = "Cool. Wonder if it will burn.",
        SUPPORT_PILLAR_DREADSTONE_SCAFFOLD = "Hmm. That looks suspiciously like work.",
        SUPPORT_PILLAR_SCAFFOLD = "Hmm. That looks suspiciously like work.",
        SURFNTURF = "Perfect for people who can't make up their mind.",
        SWEATERVEST = "It itches and fits funny.",
        SWEETPOTATO = "Don't mind if I do.",
        SWEETTEA = "What's so great about it? It's just leaf juice.",
        SWORD_LUNARPLANT = "It's okay, but now I really want a fire sword.",
        TABLE_WINTERS_FEAST = {
            BURNT = "Now I'm really feeling the holiday spirit!",
            GENERIC = "Are we having company?",
            HAS_FOOD = "Enough food for everyone!",
            WRONG_TYPE = "Not the season for this.",
        },
        TACKLECONTAINER = "I usually like my stuff in a state of organized chaos.",
        TACKLESKETCH = "Who even reads fishing magazines? Uh, apart from me right now...",
        TACKLESTATION = "Gross, it looks full of bugs.",
        TAFFY = "Candy!",
        TALLBIRD = "I don't think it wants to be friends.",
        TALLBIRDEGG = "Will it hatch?",
        TALLBIRDEGG_COOKED = "Delicious and nutritious.",
        TALLBIRDEGG_CRACKED = {
            COLD = "Is it shivering or am I?",
            GENERIC = "Looks like it's hatching!",
            HOT = "Are eggs supposed to sweat?",
            LONG = "I have a feeling this is going to take a while...",
            SHORT = "It should hatch any time now.",
        },
        TALLBIRDNEST = {
            GENERIC = "That's quite an egg!",
            PICKED = "The nest is empty.",
        },
        TALLEGGS = "It still has that birdy crunch to it.",
        TAMALES = "If I eat much more I'm going to start feeling a bit husky.",
        TEENBIRD = {
            GENERIC = "I feel like he understands.",
            HUNGRY = "He sure eats a lot.",
            SLEEPING = "All you do is eat and sleep.",
            STARVING = "Don't look at me! Get your own food.",
        },
        TELEBASE = {
            GEMS = "It needs something more.",
            VALID = "I can feel the magic!",
        },
        TELEPORTATO_BASE = {
            ACTIVE = "With this I can surely pass through space and time!",
            GENERIC = "This appears to be a nexus to another world!",
            LOCKED = "There's still something missing.",
            PARTIAL = "Soon, the invention will be complete!",
        },
        TELEPORTATO_BOX = "This may control the polarity of the whole universe.",
        TELEPORTATO_CRANK = "Tough enough to handle the most intense experiments.",
        TELEPORTATO_POTATO = "This metal potato contains great and fearful power...",
        TELEPORTATO_RING = "A ring that could focus dimensional energies.",
        TELESTAFF = "It's quite a rush to use.",
        TENSEIGA = "I've only seen it in books, \"Tenseiga\"",
        TENT = {
            BURNT = "Well, I've still got my badges.",
            GENERIC = "I got all the badges in Girl Scouts.",
        },
        TENTACLE = "Not at all cuddly.",
        TENTACLESPIKE = "It's pointy and slimy.",
        TENTACLESPOTS = "Ewwwww.",
        TENTACLE_GARDEN = "I hear little digging sounds.",
        TENTACLE_PILLAR = "It's quivering.",
        TENTACLE_PILLAR_ARM = "Awwww, they want hugs!",
        TENTACLE_PILLAR_HOLE = "What a smell! Light a match next time!",
        TERRARIUM = {
            COOLDOWN = "It needs to cool down after that.",
            CRIMSON = "I have a nasty feeling about this...",
            ENABLED = "Am I on the other side of the rainbow?!",
            GENERIC = "How did tiny tree get inside?",
            SPAWN_DISABLED = "I shouldn't be bothered by anymore prying eyes now.",
            WAITING_FOR_DARK = "What could it be?",
        },
        TERRARIUMCHEST = {
            BURNT = "Is burned away to dust.",
            GENERIC = "Is looking like normal chest without sparkles.",
            SHIMMER = "That seems a bit out of place...",
        },
        THULECITE = "I wonder where this is from?",
        THULECITE_PIECES = "This Thulecite crumbled to pieces.",
        THURIBLE = "It smells like burnt hair!",
        TICOON = {
            ABANDONED = "I'm sure I can find them on my own.",
            GENERIC = "Meow.. Meow.. Meow...",
            LOST_TRACK = "Someone else found them first.",
            NEARBY = "Looks like there's something nearby.",
            NOTHING_TO_TRACK = "It doesn't look like there's anything left to find.",
            SUCCESS = "Hey, he found one!",
            TARGET_TOO_FAR_AWAY = "They might be too far away for him to sniff out.",
            TRACKING = "I should follow his lead.",
            TRACKING_NOT_MINE = "He's leading the way for someone else.",
        },
        TILLWEED = "Stupid weed.",
        TILLWEEDSALVE = "I guess that weed was good for something.",
        TOADSTOOL = {
            GENERIC = "It's covered in nasty warts and fungus!!",
            RAGE = "Woah! It's packing a punch now!",
        },
        TOADSTOOL_CAP = {
            EMPTY = "It's a hole, what do you want from me?",
            GENERIC = "A mutant mushroom! I want it!",
            INGROUND = "What is that? It's smelly.",
        },
        TOKIJIN = {
            GENERIC = "I found it at an abandoned shrine near my home.",
            RESENTMENT = "Evil spirit, resentment, hatred, obsession, and sadness are gathered together.",
        },
        TOMATO = "Cutting these is a pain.",
        TOMATO_COOKED = "It just needed a couple minutes on the fire.",
        TOMATO_OVERSIZED = "Ugh, it's going to be even harder to cut now...",
        TOMATO_SEEDS = "It's just a bunch of seeds.",
        TOPHAT = "I'd look like quite the gentlewoman in that!",
        TOPHAT_MAGICIAN = "Aww, I thought I saw smoke, but it's just Max's shadow gunk.",
        TORCH = "Something to hold back the night.",
        TOURTIERE = "Get into my pie hole!",
        TOWNPORTAL = {
            ACTIVE = "Walking's for suckers.",
            GENERIC = "Magic stuff.",
        },
        TOWNPORTALTALISMAN = {
            ACTIVE = "I didn't want to walk, anyway.",
            GENERIC = "Yuck. It smells like sulfur.",
        },
        TRAILMIX = "Crunch crunch crunch.",
        TRAILS = "You can't push me around.",
        TRANSISTOR = "Electrical doo-dah, doo-dah.",
        TRAP = "I wove it real tight.",
        TRAP_BRAMBLE = "It's got really sharp thorns.",
        TRAP_STARFISH = {
            CLOSED = "Jeez, that was rude!",
            GENERIC = "Do starfish usually have so many teeth?",
        },
        TRAP_TEETH = "This is a nasty surprise.",
        TRAP_TEETH_MAXWELL = "I'll want to avoid stepping on that!",
        TREASURECHEST = {
            BURNT = "The trunk burned nicely.",
            GENERIC = "It's a trunk for my junk.",
            UPGRADED_STACKSIZE = "",
        },
        TREASURECHEST_TRAP = "Dirty trick!",
        TREECLUMP = "Moooooooooooooove.",
        TREEGROWTHSOLUTION = "Eww tree gunk!",
        TRIDENT = "It's three times as pokey.",
        TRINKET_1 = "We could still play with these, if we're extra creative.",
        TRINKET_10 = "Just like grandpa wears!",
        TRINKET_11 = "Beep boop!",
        TRINKET_12 = "Feels leathery.",
        TRINKET_13 = "It's watching us.",
        TRINKET_14 = "I want some hot cocoa.",
        TRINKET_15 = "I don't know how to play this game.",
        TRINKET_16 = "I don't know how to play this game.",
        TRINKET_17 = "Spork. Spork. Spork. Hahaha!",
        TRINKET_18 = "I like toys.",
        TRINKET_19 = "This toy doesn't work very well.",
        TRINKET_2 = "A voiceless instrument.",
        TRINKET_20 = "Can i dig in the dirt with this?",
        TRINKET_21 = "Mom had one of these.",
        TRINKET_22 = "It's rather like our webbing!",
        TRINKET_23 = "Are we supposed to blow on it?",
        TRINKET_24 = "No cookies. Yet!",
        TRINKET_25 = "It's stinky.",
        TRINKET_26 = "You're our cuppy cup!",
        TRINKET_27 = "Not very practical.",
        TRINKET_28 = "Maybe Maxwell will teach us how to play.",
        TRINKET_29 = "Maybe Maxwell will teach us how to play.",
        TRINKET_3 = "It won't come undone!",
        TRINKET_30 = "I can't follow the rules if i don't know them.",
        TRINKET_31 = "I can't follow the rules if i don't know them.",
        TRINKET_32 = "It's not bouncy. What's the point?",
        TRINKET_33 = "It's a friend for our finger!!",
        TRINKET_34 = "The monkey probably needed that.",
        TRINKET_35 = "I rather want to drink what's left, but he won't let me.",
        TRINKET_36 = "I've already got some, thanks.",
        TRINKET_37 = "Maybe i should hide this before someone gets hurt.",
        TRINKET_38 = "Haha! Everything looks so small!",
        TRINKET_39 = "Not very interesting.",
        TRINKET_4 = "It's watching us.",
        TRINKET_40 = "Haha, it looks like a snail shell.",
        TRINKET_41 = "Haha! Weird!",
        TRINKET_42 = "Neat!!",
        TRINKET_43 = "C'mon little croc! Let's adventure!",
        TRINKET_44 = "The plant is so pretty!",
        TRINKET_45 = "It doesn't get any good channels.",
        TRINKET_46 = "What's it for?",
        TRINKET_5 = "Yay, a new toy!",
        TRINKET_6 = "Maybe i'll find a use for these.",
        TRINKET_7 = "Another toy!",
        TRINKET_8 = "I miss bath toys.",
        TRINKET_9 = "None of them match!",
        TROPHYSCALE_FISH = {
            BURNING = "Heh... wasn't sure if that would work.",
            BURNT = "Nice.",
            GENERIC = "Let's record the fish weight properly.",
            HAS_ITEM = "Weight: {weight}\nCaught by: {owner}",
            HAS_ITEM_HEAVY = "Weight: {weight}\nCaught by: {owner}\nHow does it even fit in there?",
            OWNER = "Weight: {weight}\nCaught by: {owner}\nPfft, that wasn't even hard.",
            OWNER_HEAVY = "Weight: {weight}\nCaught by: {owner}\nRead it and weep!",
        },
        TROPHYSCALE_OVERSIZEDVEGGIES = {
            BURNING = "Mmm, what's cooking?",
            BURNT = "Aw, over already?",
            GENERIC = "I used to see these at the fair!",
            HAS_ITEM = "Weight: {weight}\nHarvested on day: {day}\nWow. Exciting.",
            HAS_ITEM_HEAVY = "Weight: {weight}\nHarvested on day: {day}\nHa! Very mighty indeed!",
            HAS_ITEM_LIGHT = "Hey is this thing broken? It's not even showing the weight!",
        },
        TRUE_HITOKIRI = "It's going to make me mad.",
        TRUE_KOSHIRAE = "Size doesn't matter.",
        TRUE_RAIKIRI = "Once it used to split a bolt of lightning.",
        TRUE_SHIRASAYA = "This blade tracks beautifully through the tip line.",
        TRUNKVEST_SUMMER = "It's so puffy!",
        TRUNKVEST_WINTER = "Winter survival gear.",
        TRUNK_COOKED = "It still doesn't look totally edible.",
        TRUNK_SUMMER = "Well, part of him is still cuddly.",
        TRUNK_WINTER = "It's so soft and squishy!",
        TUMBLEWEED = "Who knows what that tumbleweed has picked up.",
        TURFCRAFTINGSTATION = "I guess I could use a change in scenery.",
        TURF_ARCHIVE = "The ground is plain.",
        TURF_BADLANDS = "The ground is plain.",
        TURF_BEARD_RUG = "Ew ew ew EW!!",
        TURF_CARPETFLOOR = "The ground is plain.",
        TURF_CARPETFLOOR2 = "The ground is plain.",
        TURF_CAVE = "The ground is plain.",
        TURF_CHECKERFLOOR = "The ground is plain.",
        TURF_COTL_BRICK = "A bunch of bricks.",
        TURF_COTL_GOLD = "It reflects the firelight nicely.",
        TURF_DECIDUOUS = "The ground is plain.",
        TURF_DESERTDIRT = "The ground is plain.",
        TURF_DIRT = "The ground is plain.",
        TURF_DRAGONFLY = "This might actually be useful.",
        TURF_FOREST = "The ground is plain.",
        TURF_FUNGUS = "The ground is plain.",
        TURF_FUNGUS_GREEN = "The ground is plain.",
        TURF_FUNGUS_MOON = "The ground is plain.",
        TURF_FUNGUS_RED = "The ground is plain.",
        TURF_GRASS = "The ground is plain.",
        TURF_MARSH = "The ground is plain.",
        TURF_METEOR = "The ground is plain.",
        TURF_MONKEY_GROUND = "The ground is plain.",
        TURF_MOSAIC_BLUE = "The ground is plain.",
        TURF_MOSAIC_GREY = "The ground is plain.",
        TURF_MOSAIC_RED = "The ground is plain.",
        TURF_MUD = "The ground is plain.",
        TURF_PEBBLEBEACH = "The ground is plain.",
        TURF_ROAD = "The ground is plain.",
        TURF_ROCKY = "The ground is plain.",
        TURF_RUINSBRICK = "The ground is plain.",
        TURF_RUINSBRICK_GLOW = "The ground is plain.",
        TURF_RUINSTILES = "The ground is plain.",
        TURF_RUINSTILES_GLOW = "The ground is plain.",
        TURF_RUINSTRIM = "The ground is plain.",
        TURF_RUINSTRIM_GLOW = "The ground is plain.",
        TURF_SANDY = "The ground is plain.",
        TURF_SAVANNA = "The ground is plain.",
        TURF_SHELLBEACH = "The ground is plain.",
        TURF_SINKHOLE = "The ground is plain.",
        TURF_UNDERROCK = "The ground is plain.",
        TURF_WOODFLOOR = "The ground is plain.",
        TURKEYDINNER = "A feast of burned bird!",
        TURNIP = "Haha gross, I hate turnips.",
        TURNIP_COOKED = "Take that, turnip.",
        TURNIP_SEEDS = "It's just a bunch of seeds.",
        TURTILLUS = "Hey! How am I s'pose to hit you with all that armor?",
        TWIGGYTREE = {
            BURNING = "What a waste of wood.",
            BURNT = "I feel like I could have prevented that.",
            CHOPPED = "Take that, nature!",
            DISEASED = "It looks sick. More so than usual.",
            GENERIC = "It's all stick-y.",
        },
        TWIGGY_NUT = "There's a stick-y tree inside it that wants to get out.",
        TWIGGY_NUT_SAPLING = "It doesn't need any help to grow.",
        TWIGGY_OLD = "That tree looks pretty wimpy.",
        TWIGS = "A bunch of small twigs.",
        TWINOFTERROR1 = "ROBO! ROBOROBOROBOROBOROBO...",
        TWINOFTERROR2 = "ROBO! ROBOROBOROBOROBOROBO...",
        UMBRELLA = "I love the color!",
        UNAGI = "Unagi daisuki-desu.",
        UNIMPLEMENTED = "It's not finished. But I bet it still burns.",
        VEGGIEOMLET = "Hey, there's vegetables hiding underneath!",
        VEGGIE_OVERSIZED_ROTTEN = "A big pile of stinky mush.",
        VEGSTINGER = "Can you use the celery as a straw?",
        VICTORIAN_BLUEPRINT = "Ha! This is going to look hilarious!",
        VOIDCLOTH = "Finders keepers!",
        VOIDCLOTHHAT = "You never know who might really be hiding under that hood.",
        VOIDCLOTH_KIT = "Do I look like the kind of person who fixes things? Ugh, fiiiiine...",
        VOIDCLOTH_SCYTHE = "Farming just became more efficient.",
        VOIDCLOTH_UMBRELLA = "At least it keeps the rain off.",
        VOLTGOATJELLY = "HEY WARLY! Can I eat this?",
        VSCODE = "Can you change the world?",
        WAFFLES = "Hi, waffles!",
        WAGPUNKBITS_KIT = "Nice doodad.",
        WAGPUNKHAT = "Aww, it's a hat that teaches nerds how to fight!",
        WAGPUNK_BITS = "What am I supposed to do with that?",
        WAGSTAFF_MACHINERY = "Someone really should burn this scrap heap.",
        WAGSTAFF_MUTATIONS_NOTE = "Looks important. I'll keep it for review.",
        WAGSTAFF_NPC = "Uuuugh, old people always seem to need help with something or other.",
        WAGSTAFF_NPC_MUTATIONS = "Hey, suspicious guy, do you think I have all the time in the world for your schemes?",
        WAGSTAFF_NPC_WAGPUNK = "Are you in a hurry to die?",
        WAGSTAFF_TOOL_1 = "I've got a feeling this belongs to that old guy.",
        WAGSTAFF_TOOL_2 = "Ugh, couldn't that old guy have just gotten it himself?",
        WAGSTAFF_TOOL_3 = "Yeah, that looks like a Whaddayacallit! Or was it a Whatchamahoosit...",
        WAGSTAFF_TOOL_4 = "I found a tool! Maybe it's the one he's looking for?",
        WAGSTAFF_TOOL_5 = "Looks like some weird science junk. Maybe that's what the old guy's looking for?",
        WALKINGPLANK = "So, who're we going to make walk it?",
        WALKINGPLANK_GRASS = "So, who are we going to make walk it?",
        WALKINGPLANK_YOTD = "It's not much fun, but a least it's pretty.",
        WALKING_STICK = "That'll make a nice poker stick.",
        WALL_DREADSTONE = "Couldn't we have made walls out of literally anything else?",
        WALL_DREADSTONE_ITEM = "Uuuuugh, building is so much wooork...",
        WALL_HAY = {
            BURNT = "It didn't keep the fire out.",
            GENERIC = "This will keep all sorts of things out!",
        },
        WALL_HAY_ITEM = "This seems like a bad idea.",
        WALL_MOONROCK = "Safe for now.",
        WALL_MOONROCK_ITEM = "I guess I could carry it.",
        WALL_RUINS = "And they'll huff and they'll puff!",
        WALL_RUINS_2 = "And they'll huff and they'll puff!",
        WALL_RUINS_ITEM = "Will those fit in my pockets?",
        WALL_SCRAP = "",
        WALL_SCRAP_ITEM = "",
        WALL_STONE = "Eh. I guess that's okay.",
        WALL_STONE_2 = "Eh. I guess that's okay.",
        WALL_STONE_ITEM = "These are surprisingly heavy.",
        WALL_WOOD = {
            BURNT = "Burnt!",
            GENERIC = "Pointy!",
        },
        WALL_WOOD_ITEM = "I hate hiding.",
        WALRUS = "Stop following me!",
        WALRUSHAT = "I kind of like the look of it.",
        WALRUS_CAMP = {
            EMPTY = "I'm not going in there. Yuck!",
            GENERIC = "Why does everyone have a nicer house than me?",
        },
        WALRUS_TUSK = "Nom nom nom.",
        WALTER = {
            ATTACKER = "That goody-goody finally snapped!",
            FIRESTARTER = "Oh... s-should i start making a fire too?",
            GENERIC = "Hi %s!",
            GHOST = "I think he's actually having fun.",
            MURDERER = "Murderer confirmed. Stand down.",
            REVIVER = "Alright, I'm sorry for calling you a goody-goody.",
        },
        WALTERHAT = "It doesn't suit me.",
        WANDA = {
            ATTACKER = "Hey, watch it!",
            FIRESTARTER = "Stop it, %s! You're going to get in trouble!",
            GENERIC = "Hi %s!",
            GHOST = "Uh... I'm guessing something went wrong.",
            MURDERER = "Murderer confirmed. Keep your distance.",
            REVIVER = "%s is always there when we need her.",
        },
        WARDROBE = {
            BURNING = "Fire fire fire!",
            BURNT = "The fire has gone out.",
            GENERIC = "It holds dark, forbidden secrets...",
        },
        WARG = "You're a big mean!",
        WARGCORPSE = {
            BURNING = "Controlled burn in progress.",
            GENERIC = "Ugh. I can't just leave that thing lying around.",
            REVIVING = "Oh come ON!!",
        },
        WARGLET = "N-nice puppy...",
        WARGSHRINE = {
            BURNING = "That's what we were supposed to do, right?",
            BURNT = "I may've misunderstood.",
            EMPTY = "I need to put a torch in it.",
            GENERIC = "That looks WAY better!",
        },
        WARLY = {
            ATTACKER = "%s has been serving up knuckle sandwiches.",
            FIRESTARTER = "I don't think you were supposed to start that fire.",
            GENERIC = "Hi %s!",
            GHOST = "Did you mean to do that?",
            MURDERER = "Stay back. Keep a safe distance.",
            REVIVER = "%s would never leave anyone behind.",
        },
        WAR_BLUEPRINT = "Ooooh, looks dangerous!",
        WASPHIVE = "I think those bees are mad.",
        WATERBALLOON = "Boo! Hissss!",
        WATERINGCAN = {
            EMPTY = "That's more like it.",
            GENERIC = "Ugh, who put this water here?!",
        },
        WATERMELON = "Sticky sweet.",
        WATERMELONHAT = "Well, that's one use for this fruit.",
        WATERMELONICLE = "Just the thing for a hot summer day.",
        WATERMELON_COOKED = "Juicy and warm.",
        WATERMELON_OVERSIZED = "Ugh, way too much water!",
        WATERMELON_SEEDS = "I can grow these with enough time.",
        WATERPLANT = "A large aquatic flower.",
        WATERPLANT_BABY = "It's small now, but it'll get bigger.",
        WATERPLANT_BOMB = "Noted. I underestimated this one.",
        WATERPLANT_PLANTER = "Do I really want to grow more of these things?",
        WATERPUMP = "Hey... is that a fish inside?",
        WATERTREE_PILLAR = "The bigger the tree.",
        WATERTREE_ROOT = "Is big tree root.",
        WATHGRITHR = {
            ATTACKER = "Take it down a notch, %s!",
            FIRESTARTER = "You're supposed to put fires out when you're finished.",
            GENERIC = "Hi %s!",
            GHOST = "Hey %s, I'll get you a heart if you let me wear your helm!",
            MURDERER = "Murderer confirmed. Maintain formation.",
            REVIVER = "%s doesn't let anyone fall in battle!",
        },
        WATHGRITHRHAT = "It's got a name scratched on the inside... \"W\", uh...",
        WATHGRITHR_IMPROVEDHAT = "Not suitable for me",
        WATHGRITHR_SHIELD = "That shield looks pretty durable.",
        WAXED_PLANT = "",
        WAXPAPER = "Heh, we rubbed a bunch of paper in beeswax.",
        WAXWELL = {
            ATTACKER = "There's evil in you, huh %s?!",
            FIRESTARTER = "%s, no!! Fire can hurt you!",
            GENERIC = "Hi %s!",
            GHOST = "Have a heart, %s! Heh heh.",
            MURDERER = "%s, hostile actions confirmed.",
            REVIVER = "Hahaha %s, you care about us!",
        },
        WAXWELLJOURNAL = "Would make a great bedtime story... for nightmares!",
        WEBBER = {
            ATTACKER = "Hey %s, you're not venomous, are you?",
            FIRESTARTER = "We need to have a group meeting about fire safety.",
            GENERIC = "Hi %s!",
            GHOST = "Don't cry, %s, I'm getting you a heart.",
            MURDERER = "Hostile behavior confirmed. Keep distance.",
            REVIVER = "Who do ghosts call? %s!",
        },
        WEBBERSKULL = "It's not myyy way, but a burial will have to do.",
        WEBBER_SPIDER_MINION = "That spider's so tiny!",
        WEED_IVY = "It's all spiny.",
        WENDY = {
            ATTACKER = "I've never seen you so, uh, passionate... %s.",
            FIRESTARTER = "Uhh, uh-oh, let's play a different game, %s.",
            GENERIC = "Hi %s!",
            GHOST = "Isn't death kind of your thing, %s?",
            MURDERER = "Hostile behavior confirmed. Keep distance.",
            REVIVER = "That girl really likes ghosts!",
        },
        WEREITEM_BEAVER = "It looks just like you, Mr. Woodie!",
        WEREITEM_GOOSE = "Mr. Woodie, are you alright?",
        WEREITEM_MOOSE = "Can you make a toy without meat in it?",
        WES = {
            ATTACKER = "That mime punch was really convincing! Haha, ow!",
            FIRESTARTER = "Wait, don't tell me. You lit a fire.",
            GENERIC = "Hi %s!",
            GHOST = "Just tell me whatcha need and I'll get it for you. Heheh!",
            MURDERER = "Hostile behavior confirmed.",
            REVIVER = "Who do ghosts call? %s!",
        },
        WETGOOP = "I never claimed to be good at cooking!",
        WETPAPER = "I can think of a real quick way to dry it.",
        WETPOUCH = "There's something inside.",
        WHIP = "This means I'm the boss now.",
        WICKERBOTTOM = {
            ATTACKER = "Your bun too tight, %s?",
            FIRESTARTER = "I'm sure you had a very clever reason for that fire.",
            GENERIC = "Hi %s!",
            GHOST = "Did your heart give out, %s? Just kidding! Hey!",
            MURDERER = "Murderer confirmed. Proceed carefully.",
            REVIVER = "%s is a big softie!",
        },
        WILLOW = {
            ATTACKER = "You're makin' us look nuts, %s!",
            FIRESTARTER = "Another fire? Well, as long as you're happy...",
            GENERIC = "Hey! That's my face, %s! Give it back!",
            GHOST = "Is that really what my ghost hair looks like?",
            MURDERER = "Murderer confirmed. Do not approach.",
            REVIVER = "Haha, nice one %s.",
        },
        WILLOW_EMBER = "Magic?",
        WILSON = {
            ATTACKER = "Why, %s... that fire in your eyes!",
            FIRESTARTER = "Burning that wasn't very scientific, %s.",
            GENERIC = "Hi %s!",
            GHOST = "I better get a heart for %s.",
            MURDERER = "%s is hostile. Maintain distance.",
            REVIVER = "%s won't leave anyone behind.",
        },
        WINCH = {
            GENERIC = "It's only good for grabbing junk from the bottom of the ocean.",
            HOLDING_ITEM = "I'm not that interested.",
            RETRIEVING_ITEM = "Come on alreadyyyy...",
        },
        WINONA = {
            ATTACKER = "Too rough, %s! Jeez!",
            FIRESTARTER = "Don't light fires, %s!",
            GENERIC = "Hi %s!",
            GHOST = "A heart sure would come in handy!",
            MURDERER = "Murderer confirmed.",
            REVIVER = "%s never gives up on anyone.",
        },
        WINONA_BATTERY_HIGH = {
            BURNING = "Hehehe!",
            BURNT = "Well, that was mildly entertaining.",
            GENERIC = "More of Winona's weird junk.",
            LOWPOWER = "It's on its last legs.",
            OFF = "Oops. It broke.",
        },
        WINONA_BATTERY_LOW = {
            BURNING = "Hehehe!",
            BURNT = "Well, that was mildly entertaining.",
            GENERIC = "I do not know, it's Winona's junk.",
            LOWPOWER = "It's on its last legs.",
            OFF = "Oops. It broke.",
        },
        WINONA_CATAPULT = {
            BURNING = "Hehehe!",
            BURNT = "Well, that was mildly entertaining.",
            GENERIC = "It flings big stinkin' rocks.",
            OFF = "Is this thing on?",
        },
        WINONA_SPOTLIGHT = {
            BURNING = "Hehehe!",
            BURNT = "Well, that was mildly entertaining.",
            GENERIC = "What an ingenious idea!",
            OFF = "Is this thing on?",
        },
        WINTERHAT = "It's not warm enough for my liking.",
        WINTEROMETER = {
            BURNT = "It can't be that hot out!",
            GENERIC = "It might be prudent to keep an eye on the temperature.",
        },
        WINTERSFEASTFUEL = "The spirit of the season!",
        WINTERSFEASTOVEN = {
            ALMOST_DONE_COOKING = "Is it done yet?!",
            COOKING = "Cooking phase in progress. Stay focused.",
            DISH_READY = "At long last!",
            GENERIC = "Oh, don't tell me I'm the one who has to cook...",
        },
        WINTER_FOOD1 = "Honestly, who DOESN'T eat the head first?",
        WINTER_FOOD2 = "Doesn't look too bad, considering!",
        WINTER_FOOD3 = "Give me two of 'em and I can do a MacTusk impression.",
        WINTER_FOOD4 = "Anyone else sense the overpowering stench of evil?",
        WINTER_FOOD5 = "A log for eating, not for burning.",
        WINTER_FOOD6 = "I'm going to totally stuff my face!!",
        WINTER_FOOD7 = "Sure beats rainwater!",
        WINTER_FOOD8 = "I only like it when it's SCALDING.",
        WINTER_FOOD9 = "Who knew stuff from a bird's butt could be so tasty?",
        WINTER_ORNAMENT = "Bibble-ty bauble-ty boo!",
        WINTER_ORNAMENTBOSS = "Woah, shiny! Nice!",
        WINTER_ORNAMENTFORGE = "I should toast this over an open fire.",
        WINTER_ORNAMENTGORGE = "Huh. It rather looks...goaty!",
        WINTER_ORNAMENTLIGHT = "A tree's not complete without some light.",
        WINTER_TREE = {
            BURNING = "Now we're celebrating!",
            BURNT = "Happy Winter's Feast, everybody.",
            CANDECORATE = "Looks great!",
            YOUNG = "It's a bit shrimpy.",
        },
        WINTER_TREESTAND = {
            BURNT = "Happy Winter's Feast, everybody.",
            GENERIC = "Are we going to grow a tree?",
        },
        WOBSTER_DEN = "It's a rock with Wobsters in it.",
        WOBSTER_MOONGLASS = "They still taste the same, right? Right...?",
        WOBSTER_SHELLER = "What a wascally Wobster.",
        WOBSTER_SHELLER_DEAD = "You should cook up nicely.",
        WOBSTER_SHELLER_DEAD_COOKED = "I can't wait to eat you.",
        WOBYBIG = {
            "She's just a big pushover.",
            "She's just a big pushover.",
        },
        WOBYSMALL = {
            "She's a smelly, weird little dog. I like her!",
            "She's a smelly, weird little dog. I like her!",
        },
        WOLFGANG = {
            ATTACKER = "%s! Take it easy, big guy!",
            FIRESTARTER = "Don't hurt yourself, big guy.",
            GENERIC = "Hi %s!",
            GHOST = "Hey %s, did you know the heart's a muscle?",
            MURDERER = "Murderer confirmed. Keep your guard up.",
            REVIVER = "Woah! %s ripped a spirit straight out of the afterlife!",
        },
        WOLFGANG_WHISTLE = "I can't NOT listen to him when he's holding a shiny whistle!",
        WONKEY = {
            ATTACKER = "Careful, I think it's rabid!",
            FIRESTARTER = "Heh. Good monkey.",
            GENERIC = "I swear this monkey's following me.",
            GHOST = "What happened? Did you fall out of a tree?",
            MURDERER = "Hostile target confirmed. Keep distance.",
            REVIVER = "You're pretty smart for a stinky monkey.",
        },
        WOODCARVEDHAT = "Oooh, what a flammable- I mean, fancy hat!",
        WOODIE = {
            ATTACKER = "You're only fueling the fire, %s!",
            BEAVER = "Calm down, %s. Wanna borrow Bernie?",
            BEAVERGHOST = "%s! That was hilarious!",
            FIRESTARTER = "Um. I think you lit a fire, %s.",
            GENERIC = "Hi %s!",
            GHOST = "I better get a heart for %s.",
            GOOSE = "Ha! You're a hoot, %s. Or is it a honk?",
            GOOSEGHOST = "Don't be such a down-er, I'll get you a heart.",
            MOOSE = "You have to be kidding me.",
            MOOSEGHOST = "Why the long face? Ha!",
            MURDERER = "Murderer confirmed.",
            REVIVER = "Is there maple syrup in your veins, %s? You're so sweet!",
        },
        WOOD_CHAIR = {
            BURNT = "Aww, it's over.",
            GENERIC = "That will definitely burn.",
            OCCUPIED = "You sure you want to sit there? That chair may spontaneously combust.",
        },
        WOOD_TABLE = {
            BURNT = "Yup. It burned.",
            GENERIC = "That will definitely burn.",
            HAS_ITEM = "That will definitely burn.",
        },
        WORM = {
            DIRT = "Is dirt moving?",
            PLANT = "Pretty light.",
            WORM = "Did not expect this!",
        },
        WORMHOLE = {
            GENERIC = "Poke it with a stick!",
            OPEN = "Here we go!",
        },
        WORMHOLE_LIMITED = "Yuck. That won't hold long.",
        WORMLIGHT = "Pretty light.",
        WORMLIGHT_LESSER = "Nice little light.",
        WORMLIGHT_PLANT = "Pretty light.",
        WORMWOOD = {
            ATTACKER = "Hey! Keep those creepin' vines to yourself!",
            FIRESTARTER = "I don't think it's safe for you to be around fire.",
            GENERIC = "Hi %s!",
            GHOST = "I think %s needs some help.",
            MURDERER = "Stay back. Keep a safe distance.",
            REVIVER = "Hey, thanks a bunch there, %s!",
        },
        WORMWOOD_MUTANTPROXY_CARRAT = {
            DEAD = "Ew, it's dead.",
            GENERIC = "I want to roast it!",
            HELD = "It keeps squirming.",
            SLEEPING = "It's snoring.",
        },
        WORMWOOD_MUTANTPROXY_FRUITDRAGON = {
            GENERIC = "They're pretty cute.",
            RIPE = "Mmm... Roast dragonfruit.",
            SLEEPING = "Good night, little guy.",
        },
        WORMWOOD_MUTANTPROXY_LIGHTFLIER = "I'd rather use fire to light my way.",
        WORTOX = {
            ATTACKER = "You're supposed to prank everyone else, not ME.",
            FIRESTARTER = "Don't play tricks, Mr. Imp!",
            GENERIC = "Hi %s!",
            GHOST = "You're looking a little pale there, red.",
            MURDERER = "Hostile entity confirmed.",
            REVIVER = "I guess %s isn't all mischief.",
        },
        WURT = {
            ATTACKER = "You're a real little terror, aren't you?",
            FIRESTARTER = "I... don't think that's allowed...",
            GENERIC = "Hi %s!",
            GHOST = "Yeesh, what happened to you?",
            MURDERER = "Do not approach me.",
            REVIVER = "Thanks for the hand, %s... or, uh, claw?",
        },
        WX78 = {
            ATTACKER = "Better cool it before you blow a gasket, %s!",
            FIRESTARTER = "%s, no!! Fire can hurt you!",
            GENERIC = "Hi %s!",
            GHOST = "I better get a heart for %s.",
            MURDERER = "Hostile unit confirmed. Engage with caution.",
            REVIVER = "Hahaha %s, you care about us!",
        },
        WX78MODULE_BEE = "WX, this module is efficient.",
        WX78MODULE_COLD = "WX, this module should help with cold exposure.",
        WX78MODULE_HEAT = "WX, this module should help with heat control.",
        WX78MODULE_LIGHT = "WX, this module improves visibility.",
        WX78MODULE_MAXHEALTH = "WX, durability upgrade confirmed.",
        WX78MODULE_MAXHEALTH2 = "WX, additional durability upgrade confirmed.",
        WX78MODULE_MAXHUNGER = "WX, energy capacity increase confirmed.",
        WX78MODULE_MAXHUNGER1 = "WX, additional energy capacity increase confirmed.",
        WX78MODULE_MAXSANITY = "WX, stability module confirmed.",
        WX78MODULE_MAXSANITY1 = "WX, additional stability module confirmed.",
        WX78MODULE_MOVESPEED = "WX, mobility module confirmed.",
        WX78MODULE_MOVESPEED2 = "WX, additional mobility module confirmed.",
        WX78MODULE_MUSIC = "WX, audio module confirmed.",
        WX78MODULE_NIGHTVISION = "WX, night-vision module confirmed.",
        WX78MODULE_TASER = "WX, taser module confirmed.",
        WX78_MODULEREMOVER = "Let me try... hold still, WX!",
        WX78_SCANNER = {
            GENERIC = "I can't decide if it's cute or creepy.",
            HUNTING = "I can't decide if it's cute or creepy.",
            SCANNING = "I can't decide if it's cute or creepy.",
        },
        WX78_SCANNER_ITEM = "Heh... how mad do you think WX would get if I painted a mustache on it?",
        WX78_SCANNER_SUCCEEDED = "What are you waiting for, a pat on the head?",
        YARI = "It is excellent at both piercing and slicing across a wide area.",
        YELLOWAMULET = "It seems to absorb the darkness around it.",
        YELLOWGEM = "It sparkles.",
        YELLOWMOONEYE = "Another rock!",
        YELLOWSTAFF = "It's magical.",
        YOTB_BEEFALOSHRINE = {
            BURNT = "That isn't good...",
            EMPTY = "Guess I should give it something.",
            GENERIC = "Got any firecrackers?",
        },
        YOTB_BEEFALO_DOLL_BEAST = {
            GENERIC = "You and Bernie will get along great!",
            YOTB = "Hey judge! Get a load of this!",
        },
        YOTB_BEEFALO_DOLL_DOLL = {
            GENERIC = "You and Bernie will get along great!",
            YOTB = "Hey judge! Get a load of this!",
        },
        YOTB_BEEFALO_DOLL_FESTIVE = {
            GENERIC = "You and Bernie will get along great!",
            YOTB = "Hey judge! Get a load of this!",
        },
        YOTB_BEEFALO_DOLL_FORMAL = {
            GENERIC = "You and Bernie will get along great!",
            YOTB = "Hey judge! Get a load of this!",
        },
        YOTB_BEEFALO_DOLL_ICE = {
            GENERIC = "You and Bernie will get along great!",
            YOTB = "Hey judge! Get a load of this!",
        },
        YOTB_BEEFALO_DOLL_NATURE = {
            GENERIC = "You and Bernie will get along great!",
            YOTB = "Hey judge! Get a load of this!",
        },
        YOTB_BEEFALO_DOLL_ROBOT = {
            GENERIC = "You and Bernie will get along great!",
            YOTB = "Hey judge! Get a load of this!",
        },
        YOTB_BEEFALO_DOLL_VICTORIAN = {
            GENERIC = "You and Bernie will get along great!",
            YOTB = "Hey judge! Get a load of this!",
        },
        YOTB_BEEFALO_DOLL_WAR = {
            GENERIC = "You and Bernie will get along great!",
            YOTB = "Hey judge! Get a load of this!",
        },
        YOTB_PATTERN_FRAGMENT_1 = "Looks like I'll have to put a few of these together to make anything worthwhile.",
        YOTB_PATTERN_FRAGMENT_2 = "Looks like I'll have to put a few of these together to make anything worthwhile.",
        YOTB_PATTERN_FRAGMENT_3 = "Looks like I'll have to put a few of these together to make anything worthwhile.",
        YOTB_POST = "A good place to secure a beefalo.",
        YOTB_POST_ITEM = "I'd better get that set up.",
        YOTB_SEWINGMACHINE = "I never liked sewing.",
        YOTB_SEWINGMACHINE_ITEM = "What a nice pile of kindling!",
        YOTB_STAGE = "You think you can judge me?!",
        YOTB_STAGE_ITEM = "Ughhh, so much wooorrkk...",
        YOTC_CARRATSHRINE = {
            BURNT = "No regrets.",
            EMPTY = "Let me guess, I have to give you something.",
            GENERIC = "How come the rat-thing gets a shrine and I don't?",
        },
        YOTC_CARRAT_GYM_DIRECTION = {
            BURNT = "All's well that burns well.",
            GENERIC = "This should be fun.",
            RAT = "Getting dizzy yet?",
        },
        YOTC_CARRAT_GYM_DIRECTION_ITEM = "A gym-shaped pile of potential kindling.",
        YOTC_CARRAT_GYM_REACTION = {
            BURNT = "All's well that burns well.",
            GENERIC = "Sure, why not.",
            RAT = "Ha! I could watch this all day.",
        },
        YOTC_CARRAT_GYM_REACTION_ITEM = "A gym-shaped pile of potential kindling.",
        YOTC_CARRAT_GYM_SPEED = {
            BURNT = "All's well that burns well.",
            GENERIC = "I need to get my Carrat up to speed.",
            RAT = "Look at those little legs go!",
        },
        YOTC_CARRAT_GYM_SPEED_ITEM = "A gym-shaped pile of potential kindling.",
        YOTC_CARRAT_GYM_STAMINA = {
            BURNT = "All's well that burns well.",
            GENERIC = "Look! It's got a little jump rope!",
            RAT = "Go on, get the berry!",
        },
        YOTC_CARRAT_GYM_STAMINA_ITEM = "A gym-shaped pile of potential kindling.",
        YOTC_CARRAT_RACE_CHECKPOINT = "Check out this checkpoint!",
        YOTC_CARRAT_RACE_CHECKPOINT_ITEM = "That checks out.",
        YOTC_CARRAT_RACE_FINISH = {
            BURNT = "Now that's more like it.",
            GENERIC = "It's really more of a finish circle than a line.",
            I_WON = "Pfft, as if there was any doubt I'd win.",
            SOMEONE_ELSE_WON = "{winner} was just lucky, I'm winning the next one.",
        },
        YOTC_CARRAT_RACE_FINISH_ITEM = "Let's see, where's a good spot for me to win?",
        YOTC_CARRAT_RACE_START = "Mine's going to win, obviously.",
        YOTC_CARRAT_RACE_START_ITEM = "What a gong show.",
        YOTC_CARRAT_SCALE = {
            BURNT = "Ha! Nice.",
            CARRAT = "Meh. I've seen better.",
            CARRAT_GOOD = "Now we're talking!",
            GENERIC = "My Carrat's off the scale! I mean literally.",
        },
        YOTC_CARRAT_SCALE_ITEM = "A nice, scale-shaped pile of potential kindling.",
        YOTC_SEEDPACKET = "Just a bunch of seeds.",
        YOTC_SEEDPACKET_RARE = "Oh wow, special seeds, how exciting.",
        YOTD_ANCHOR = "I wouldn't want my boat to fly away.",
        YOTD_ANCHOR_ITEM = "I've got all the supplies ready to build an anchor.",
        YOTD_DRAGONSHRINE = {
            BURNT = "There's no reversing that.",
            EMPTY = "What do you want? ash or charcoal?",
            GENERIC = "I wonder what assortment of knickknacks I can craft here.",
        },
        YOTD_OAR = "Let's paddle together~",
        YOTD_STEERINGWHEEL = "Isn't it the same?",
        YOTD_STEERINGWHEEL_ITEM = "Isn't it the same?",
        YOTP_FOOD1 = "I'm going to eat it all!",
        YOTP_FOOD2 = "Nah. I'll feed it to something.",
        YOTP_FOOD3 = "A little snack to fill my belly.",
        YOTR_DECOR_1 = {
            GENERAL = "Everything looks better in the firelight.",
            OUT = "Looks like it needs some more fire!",
        },
        YOTR_DECOR_1_ITEM = "The more firelight around here, the better!",
        YOTR_DECOR_2 = {
            GENERAL = "Everything looks better in the firelight.",
            OUT = "Looks like it needs some more fire!",
        },
        YOTR_DECOR_2_ITEM = "The more firelight around here, the better!",
        YOTR_FIGHTRING_BELL = {
            GENERIC = "Alright, who wants to get the stuffing knocked out of them?",
            PLAYING = "Yeah! Get 'em!!",
        },
        YOTR_FIGHTRING_KIT = "Oh, this'll be fun...",
        YOTR_FOOD1 = "Nice, the rabbits brought cake!",
        YOTR_FOOD2 = "It sure tastes... blue?",
        YOTR_FOOD3 = "Eww, it looks like a big jelly eyeball.",
        YOTR_FOOD4 = "I'll just pop those in my mouth, quick as a bunny.",
        YOTR_RABBITSHRINE = {
            BURNT = "Now I'm smiling!",
            EMPTY = "I get the feeling it's hungry for something.",
            GENERIC = "What's it so smiley about?",
        },
        YOTR_TOKEN = "Entry token confirmed. I'll compete with proper etiquette.",
        YOT_CATCOONSHRINE = {
            BURNT = "Smells like scorched fur.",
            EMPTY = "Maybe it would like a feather to play with...",
            GENERIC = "What to make...",
        },
    },
    DESCRIBE_GENERIC = "I have no idea what that is!",
    DESCRIBE_PLANTHAPPY = "It looks... fine?",
    DESCRIBE_PLANTSTRESSED = "I'd guess there's more than one thing bothering it.",
    DESCRIBE_PLANTSTRESSORFAMILY = "What's wrong? Don't tell me it's lonely?",
    DESCRIBE_PLANTSTRESSORHAPPINESS = "What do you want from me?!",
    DESCRIBE_PLANTSTRESSORKILLJOYS = "Ugh, do I really have to weed the garden again?",
    DESCRIBE_PLANTSTRESSORMOISTURE = "Looking nice and crispy!",
    DESCRIBE_PLANTSTRESSORNUTRIENTS = "Ms. Wickerbottom's always going on about nutrients in the soil. Maybe it needs that.",
    DESCRIBE_PLANTSTRESSOROVERCROWDING = "These plants could use some more personal space.",
    DESCRIBE_PLANTSTRESSORSEASON = "Guess it doesn't like the weather.",
    DESCRIBE_PLANTVERYSTRESSED = "Something's got it really upset.",
    DESCRIBE_SMOLDERING = "It's smoldering. Extinguish it before it spreads.",
    DESCRIBE_TOODARK = "I need more light!",
    EAT_FOOD = {
        TALLBIRDEGG_CRACKED = "Ugh. Crunchy.",
        WINTERSFEASTFUEL = "Does anyone else smell a campfire?",
    },
    QUAGMIRE_ANNOUNCE_LOSE = "I think it's angry!!",
    QUAGMIRE_ANNOUNCE_MEALBURNT = "Whoops, burnt it. Heh heh.",
    QUAGMIRE_ANNOUNCE_NOTRECIPE = "Okay, so that didn't work.",
    QUAGMIRE_ANNOUNCE_WIN = "Let's get out of here!",
}
